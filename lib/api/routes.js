...
module.exports = function(parsoidConfig, processLogger) {
	...
	routes.internal = function(req, res, next) {
		...
		if (metrics) {
			metrics.increment('api.version.' + res.locals.apiVersion);
		}
		res.locals.iwp = iwp;
			...
			.then(function(doc) {
				...
				if (metrics) {
					if (startTimers.has('wt2html.wt.parse')) {
						...
						metrics.timing('wt2html.wt.size.output', output.length);
						...
						metrics.timing('wt2html.pageWithOldid.size.output', output.length);
					}
	...
	var html2wt = Promise.method(function(req, res, html) {
		...
		if (original) {
			if (opts.from === 'pagebundle') {
				...
				pb = apiUtils.extractPageBundle(original);
