...
module.exports = function routes(parsoidConfig, processLogger, parsoidOptions, parse) {
	...
	var wt2html = Promise.async(function *(req, res, wt, reuseExpansions) {
		...
		if (typeof wikitext === 'string') {
			...
			if (metrics) {
				metrics.endTiming(
					'wt2html.wt.init', startTimers.get('wt2html.init')
				);
	...
	var pb2pb = Promise.async(function *(req, res) { // eslint-disable-line require-yield
		...
		// No reuse from semantically different content versions.
			...
			if (opts.updates && opts.updates.redlinks) {
			...
			} else if (opts.updates && opts.updates.variant) {
				...
				if (req.headers['content-language']) {
					env.page.pagelanguage = req.headers['content-language'];
				} else if (opts.original && opts.original.revid) {
					// fetch pagelanguage from original pageinfo
					const title = opts.original.title;
					const oldid = opts.original.revid;
					yield TemplateRequest.setPageSrcInfo(env, title, oldid);
				} else {
		...
		if (opts.from === 'wikitext') {
			...
			if (typeof wikitext !== 'string' && opts.wikitext) {
				...
				if (opts.wikitext.headers && opts.wikitext.headers['content-language']) {
					env.page.pagelanguage = opts.wikitext.headers['content-language'];
				}
			}
