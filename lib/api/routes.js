			...
			if (typeof ret.wikitext === 'string') {
				...
				if (revision) {
					p2 = p2.then(function(ret2) {
						...
						DU.applyPageBundle(doc, { parsoid: revision['data-parsoid'].body });
		...
		var p = Promise.method(function() {
			...
			if (opts.original) {
				...
				if (dp) {
					...
					DU.applyPageBundle(ret.doc, { parsoid: dp.body });
				...
				if (opts.original.html) {
					...
					apiUtils.validateDp(opts.original);
					DU.applyPageBundle(env.page.dom.ownerDocument, { parsoid: dp.body });
