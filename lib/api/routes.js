...
module.exports = function routes(parsoidConfig, processLogger, parsoidOptions, parse) {
	...
	var wt2html = Promise.async(function *(req, res, wt, reuseExpansions) {
		...
		if (metrics) {
			...
			if (semver.neq(env.contentVersion, MWParserEnv.prototype.availableVersions[0])) {
	...
	var html2wt = Promise.async(function *(req, res, html) {
			...
			if (hasInlineContentVersion) {
				...
				if (originalVersion !== inlineContentVersion) {
					...
					if (downgrade && opts.from === 'pagebundle') {  // Downgrades are only for pagebundle
						var oldDoc;
						({ doc: oldDoc, pb: origPb } = apiUtils.doDowngrade(downgrade, metrics, original, originalVersion));
						oldBody = oldDoc.body;
					} else {
		...
		if (metrics) {
			...
			if (!hasInlineContentVersion) { metrics.increment('html2wt.original.version.notinline'); }
		}
