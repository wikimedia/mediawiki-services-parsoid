...
module.exports = function routes(parsoidConfig, processLogger, parsoidOptions, parse) {
	...
	var html2wt = Promise.async(function *(req, res, html) {
		...
		var hasInlineContentVersion = (inlineContentVersion !== null);
		if (hasInlineContentVersion) { env.originalVersion = inlineContentVersion; }

		if (original && original.html) {
			var originalVersion = apiUtils.versionFromType(original.html);
			if (originalVersion === null) {
				return apiUtils.fatalRequest(env, 'Content-type of original html is missing.', 400);
			}
			if (hasInlineContentVersion) {
				// Check if we need to downgrade the original to match the inlined
				// content version
				if (originalVersion !== inlineContentVersion) {
					var downgrade = apiUtils.findDowngrade(originalVersion, inlineContentVersion);
					if (downgrade && opts.from === 'pagebundle') {  // Downgrades are only for pagebundle
						oldBody = DU.parseHTML(original.html.body).body;
						origPb = apiUtils.extractPageBundle(original);
						apiUtils.validatePageBundle(origPb, originalVersion);
						downgrade.func(oldBody.ownerDocument, origPb);
					} else {
						return apiUtils.fatalRequest(env,
							util.format(
								'Modified (%s) and original (%s) html are of different type, and no path to downgrade.',
								inlineContentVersion, originalVersion
							),
						400);
					}
				}
				...
				env.originalVersion = originalVersion;
			}
		}
		...
		if (original) {
			...
			if (original.html) {
				if (!oldBody) { oldBody = DU.parseHTML(original.html.body).body; }
				if (opts.from === 'pagebundle') {
	...
	var pb2pb = Promise.async(function *(req, res) { // eslint-disable-line require-yield
		...
		var downgrade = apiUtils.findDowngrade(env.originalVersion, env.contentVersion);
		if (downgrade) {
			console.assert(revision === opts.original);
			return apiUtils.doDowngrade(downgrade, env, revision, res, contentmodel);
