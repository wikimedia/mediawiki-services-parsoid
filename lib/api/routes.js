...
module.exports = function(parsoidConfig, processLogger) {
	...
	var wt2html = Promise.method(function(req, res, wt) {
		...
		return p.then(function(wikitext) {
			...
			if (env.conf.parsoid.devAPI && req.query.follow_redirects) {
			...
			.then(function(doc) {
				var out;
				if (pageBundle) {
					out = DU.extractDpAndSerialize(res.locals.bodyOnly ? doc.body : doc, {
				...
				if (opts.format === 'lint') {
					apiUtils.jsonResponse(res, env.lintLogger.buffer);
				} else {
					var contentmodel = env.page.meta.revision.contentmodel;
					apiUtils.wt2htmlRes(env, res, out.html, out.pb, contentmodel);
	...
	var html2wt = Promise.method(function(req, res, html) {
		...
		env.scrubWikitext = apiUtils.shouldScrub(req, env.scrubWikitext);
