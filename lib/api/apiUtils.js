...
apiUtils.wikitextContentType = function(env) {
	return 'text/plain; charset=utf-8; profile="mediawiki.org/specs/wikitext/' + env.conf.parsoid.WIKITEXT_VERSION + '"';
};

apiUtils.htmlContentType = function(env) {
	return 'text/html; charset=utf-8; profile="mediawiki.org/specs/html/' + env.conf.parsoid.HTML_VERSION + '"';
};

apiUtils.dataParsoidContentType = function(env) {
	return 'application/json; charset=utf-8; profile="mediawiki.org/specs/data-parsoid/' + env.conf.parsoid.DATA_PARSOID_VERSION + '"';
};
