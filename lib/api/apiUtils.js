...
var apiUtils = module.exports = {
	/** @property {string} */
	WIKITEXT_CONTENT_TYPE: 'text/plain; charset=utf-8; profile="mediawiki.org/specs/wikitext/1.0.0"',
	/** @property {string} */
	HTML_CONTENT_TYPE:     'text/html; charset=utf-8; profile="mediawiki.org/specs/html/1.1.0"',
	/** @property {string} */
	DATA_PARSOID_CONTENT_TYPE: 'application/json; charset=utf-8; profile="mediawiki.org/specs/data-parsoid/0.0.1"',
};
...
apiUtils.startHtml2wt = Promise.method(function(req, res, html) {
	...
	var maxHTMLSize = env.conf.parsoid.limits.html2wt.maxHTMLSize;
	if (html.length >= maxHTMLSize) {
		throw new Error("HTML size bigger than " + maxHTMLSize);
	}
...
apiUtils.startWt2html = function(req, res, wt) {
	...
	var maxWikitextSize = env.conf.parsoid.limits.wt2html.maxWikitextSize;
	if (wt && wt.length >= maxWikitextSize) {
		throw new Error("Wikitext size bigger than " + maxWikitextSize);
	}
