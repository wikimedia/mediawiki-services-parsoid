 ...
 * change information.
 */
DOMDiff.prototype.diff = function(nodeA, nodeB) {
 ...
 */
DOMDiff.prototype.dataMWEquals = function(nodeA, dmwA, nodeB, dmwB) {
 ...
 * @private
 */
DOMDiff.prototype._dataMWEquals = function(nodeA, dmwA, nodeB, dmwB, options) {
	...
	for (var i = 0; i < keysA.length; i++) {
		...
		if (!vA || !vB) {
			...
			if (!this.treeEquals(DU.ppToDOM(vA, { markNew: true }), DU.ppToDOM(vB, { markNew: true }), true)) {
				return false;
			}
 ...
 * @return {boolean}
 */
DOMDiff.prototype.treeEquals = function(nodeA, nodeB, deep) {
...
function nextNonTemplateSibling(node) {
	return DU.isEncapsulationWrapper(node) ? DU.skipOverEncapsulatedContent(node) : node.nextSibling;
}
 ...
 */
DOMDiff.prototype.doDOMDiff = function(baseParentNode, newParentNode) {
	var dd = this;
	...
	while (baseNode && newNode) {
		...
		if (!this.treeEquals(baseNode, newNode, false)) {
			...
			if (!foundDiff && DU.isContentNode(newNode)) {
				...
				while (lookaheadNode) {
					...
					if (DU.isContentNode(lookaheadNode) &&
						...
						foundDiff = true;
						break;
					} else if (!DU.emitsSolTransparentSingleLineWT(lookaheadNode)) {
			...
			if (!foundDiff) {
				if (!DU.isElt(savedNewNode)) {
					...
					if (!DU.isEncapsulationWrapper(baseNode) &&
		...
		} else if (!DU.isEncapsulationWrapper(baseNode) && !DU.isEncapsulationWrapper(newNode)) {
...
/** @private */
DOMDiff.prototype.markNode = function(node, mark, blockNodeDeleted) {
	var meta;
	if (mark === 'deleted') {
		// insert a meta tag marking the place where content used to be
		meta = DU.prependTypedMeta(node, 'mw:DiffMarker/' + mark);
	} else {
		if (DU.isElt(node)) {
			DU.setDiffMark(node, this.env, mark);
