...
function nextNonTemplateSibling(node) {
	return DU.hasParsoidAboutId(node) ? DU.skipOverEncapsulatedContent(node) : node.nextSibling;
}
...
DDP.doDOMDiff = function(baseParentNode, newParentNode) {
	...
	while (baseNode && newNode) {
		...
		if (!this.treeEquals(baseNode, newNode, false)) {
			...
			if (!foundDiff) {
				if (!DU.isElt(savedNewNode)) {
					...
					if (!DU.hasParsoidAboutId(baseNode) &&
						!DU.hasParsoidAboutId(savedNewNode)) {
		...
		} else if (!DU.hasParsoidAboutId(baseNode) && !DU.hasParsoidAboutId(newNode)) {
