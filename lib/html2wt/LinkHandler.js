...
function serializeAsWikiLink(node, state, linkData) {
	var contentParts;
	var contentSrc = '';
	var isPiped = false;
...
function serializeAsExtLink(node, state, linkData) {
	var env = state.env;
	var wiki = env.conf.wiki;
	var target = linkData.target;
	var dp = DU.getDataParsoid(node);
...
var linkHandler = function(node, state) {
	...
	if (linkType !== null && linkData.target.value !== null) {
		...
		if (/^mw:WikiLink$/.test(linkType) || Util.solTransparentLinkRegexp.test(linkType)) {
			...
			serializeAsWikiLink(node, state, linkData);
		} else if (linkType === 'mw:ExtLink') {
			// [..] links, autolinks, ISBN, RFC, PMID
			serializeAsExtLink(node, state, linkData);
		} else {
...
var figureHandler = function(node, state) {
	var env = state.env;
	...
	if (!imgElt) {
		env.log("error", "In WSP.figureHandler, node does not have any img elements:", node.outerHTML);
	...
	if (resource.value === null) {
		...
		if (!src) {
			env.log("error", "In WSP.figureHandler, img does not have resource or src:", node.outerHTML);
			state.emitChunk('', node);
			return;
		}
		if (/^https?:/.test(src)) {
	...
	wikitext += ']]';
	state.emitChunk(new WikiLinkText(wikitext, node, env.conf.wiki, 'mw:Image'), node);
};
