...
var serializeAsWikiLink = Promise.async(function *(node, state, linkData) {
	...
	if (escapedTgt && escapedTgt.invalidLink) {
		...
		if (isPiped && requiresEscaping) {
			...
			pipedText = '|' + state.serializer.wteHandlers.escapeLinkContent(state, contentSrc, false, node);
		} else if (isPiped) {
	...
	[
		...
		if (o.value && o.value.fromsrc) {
			...
			if (o.value && /^(link|alt)$/.test(o.name)) {
				...
				value = state.serializer.wteHandlers.escapeLinkContent(state, value, false, node);
			}
			nopts.push({
				ck: o.name,
				v: value,
