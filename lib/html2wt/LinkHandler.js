...
var getLinkRoundTripData = Promise.method(function(env, node, state) {
	...
	if (rel) {
		...
		var typeMatch = rel.match(/\b(mw:(WikiLink|ExtLink|Media|PageProp)[^\s]*)\b/);
		if (typeMatch) {
	...
	if (rtData.type === null && !node.querySelector('img, video')) {
	...
	return state.serializer.serializedAttrVal(node, 'href').then(function(target) {
		...
		if (state.inModifiedContent || DU.hasDiffMark(node, env, 'subtree-changed')) {
		...
		if (/\bmw:MediaLink\b/.test(rtData.type)) {
			// No provision for editing namespace alias
			// If present, original namespace alias is always used
			var mediaPrefix;
			if (dp.sa && dp.sa.namespace) {
				mediaPrefix = dp.sa.namespace;
			} else {
				mediaPrefix = wiki.namespaceNames[wiki.namespaceIds.media];
			}

			// Unless edited, get original shadowed info for 'fileName'
			// and update rtData.target. href/target is derived from the file name.
			rtData.target = DU.getShadowInfo(node, 'fileName', rtData.href.replace(/.*\//, ''));
			rtData.target.value = rtData.href = mediaPrefix + ":" + rtData.target.value;
		...
		if (interWikiMatch
			...
			} else if (oldPrefix &&  // Should we preserve the old prefix?
...
var isSimpleWikiLink = function(env, dp, target, linkData) {
	...
	if (contentString !== undefined
		...
		|| (!hrefHasProto &&
				...
				(contentString === Util.decodeURI(linkData.href) ||
				// normalize with underscores for comparison with href
				env.normalizedTitleKey(contentString, true) === Util.decodeURI(linkData.href)))
		);
	}
	...
	if (linkData.type === 'mw:PageProp/Category') {
		...
		if (targetParts) {
			target.value = targetParts[1]
				.replace(/^(\.\.?\/)*/, '')
				.replace(/_/g, ' ');
			contentParts = splitLinkContentString(targetParts[2].replace(/%23/g, '#'), dp);
		...
		if (linkData.content.string === undefined) {
			linkData.content.string = Util.decodeEntities(target.value);
		}
		p = Promise.resolve();
	} else {
		p = Promise.resolve();
		...
		if (magicLinkMatch) {
			...
			if (serialized[0] === '[') {
				...
				state.emitChunk(magicLinkMatch[0] === 'ISBN' ?
					...
					new ExtLinkText(serialized, node, wiki, 'mw:ExtLink'), node);
			} else {
	...
	return getLinkRoundTripData(env, node, state).then(function(linkData) {
		...
		if (linkType !== null && linkData.target.value !== null) {
			...
			if (isFigure) {
				...
				return state.serializeLinkChildrenToString(node, handler)
				.then(function(str) {
					var chunk;
					if (!hrefStr) {
...
function eltNameFromMediaType(type) {
	switch (type) {
		// We use <video> for audio in order to be able to style the player.
		case 'mw:Audio':
		case 'mw:Video':
			return 'VIDEO';
		default:
...
var figureHandler = Promise.method(function(node) {
	...
	.then(function(resource) {
		...
		return figureHandlerImpl(node, state, resource, elt, eltName, format,
			rdfaType, linkElt, outerElt, captionElt);
	});
});

figureHandlerImpl = Promise.method(function(node, state, resource, elt,
		eltName, format, rdfaType, linkElt, outerElt, captionElt) {
	var nopts = [];
	...
	if (linkElt && linkElt.hasAttribute('href')) {
		linkPromise = state.serializer.serializedImageAttrVal(outerElt, linkElt, 'href')
		.then(function(link) {
			if (!link.fromsrc) {
		...
		linkPromise = state.serializer.getAttributeValueAsShadowInfo(outerElt, 'href');
	}
	...
	var captionPromise = captionElt ?
		state.serializeCaptionChildrenToString(captionElt,
	...
	return Promise.join(
		...
		state.serializer.serializedImageAttrVal(outerElt, elt, 'alt'),
		// Fetch the lang (if any)
		state.serializer.serializedImageAttrVal(outerElt, elt, 'lang')
	)
	.spread(function(link, _caption, alt, lang) {
		caption = _caption;
		...
		var paramFromDataMw = function(o) {
			...
			if (v !== undefined) {
				return state.serializer.getAttributeValue(outerElt, o.ck, mwAliases[o.alias])
				.then(function(ak) {
					nopts.push({
						ck: o.ck,
						ak: ak,
	...
	.then(function(opt) {
		...
		return Promise.join(
			...
			state.serializer.serializedImageAttrVal(outerElt, elt, 'height'),
			// Get the user-specified width from wikitext
			state.serializer.serializedImageAttrVal(outerElt, elt, 'width')
		);
	})
