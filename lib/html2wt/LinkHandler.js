		...
		if (target.modified || !target.fromsrc) {
			...
			if (categoryMatch) {
				...
				if (ns === wiki.canonicalNamespaces.category) {
					...
					if (!(
						nextNode && DU.isElt(nextNode) && DU.hasNodeName(nextNode, "link") &&
...
var figureHandler = Promise.async(function *(node) {
	...
	return (yield figureHandlerImpl(
		node, state, resource, elt, eltName, format,
		rdfaType, linkElt, outerElt, captionElt
	));
});

figureHandlerImpl = Promise.async(function *(node, state, resource, elt,
