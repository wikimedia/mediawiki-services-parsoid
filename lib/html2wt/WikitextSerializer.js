...
WSP._serializeHTMLTag = function(node, wrapperUnmodified) {
	...
	var sAttribs = this._serializeAttributes(node, token);
...
WSP._serializeHTMLEndTag = function(node, wrapperUnmodified) {
	if (wrapperUnmodified) {
		var dsr = DU.getDataParsoid(node).dsr;
...
WSP._serializeAttributes = function(node, token) {
...
WSP._buildExtensionWT = function(node, dataMW) {
	...
	var attrStr = this._serializeAttributes(node, extTok);
	...
	if (!dataMW.body) {
		...
		if (typeof dataMW.body.html === 'string' ||
			...
			env.log("error", "extension src unavailable for: " + node.outerHTML);
		}
		...
		return {
			handle: function() {
				...
				if (/(?:^|\s)mw:Transclusion(?=$|\s)/.test(typeOf)) {
					...
					if (dataMW.parts) {
						src = self._buildTemplateWT(node, dataMW.parts);
					} else if (dp.src) {
					...
					if (dataMW.name) {
						if (dp.autoInsertedRefs && state.rtTestMode) {
							...
							src = self._buildExtensionWT(node, dataMW);
						}
					} else if (dp.src) {
						self.env.log("error", "data-mw missing in: " + node.outerHTML);
						src = dp.src;
