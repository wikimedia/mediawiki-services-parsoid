...
WS.prototype.serializePart = Promise.async(function *(node, type, part, tplData) {
...
WS.prototype.serializeFromParts = Promise.async(function *(node, srcParts) {
	var env = this.env;
	...
	for (const part of srcParts) {
		...
		if (tplarg) {
			buf += yield this.serializePart(node, 'templatearg', tplarg, null);
			continue;
		}
		...
		try {
			...
			fetched = true;
			buf += yield this.serializePart(node, type, tpl, tplData);
		} catch (err) {
			if (fetched && tplData === null) {
				...
				buf += yield this.serializePart(node, type, tpl, null);
			}
		}
	}
	return buf;
