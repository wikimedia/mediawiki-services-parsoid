...
var LanguageVariantHandler = require('./LanguageVariantHandler.js');
 ...
 */
WS = WikitextSerializer = function(options) {
...
WS.prototype.linkHandler = LinkHandlersModule.linkHandler;
WS.prototype.figureHandler = LinkHandlersModule.figureHandler;

WS.prototype.languageVariantHandler = LanguageVariantHandler.languageVariantHandler;

// Separator handling
WS.prototype.updateSeparatorConstraints = SeparatorsModule.updateSeparatorConstraints;
WS.prototype.buildSep = SeparatorsModule.buildSep;
...
WS.prototype.getAttributeValue = Promise.async(function *(node, key, value) {
	var tplAttrs = DU.getDataMw(node).attribs;
	if (tplAttrs) {
...
WS.prototype.serializedAttrVal = Promise.async(function *(node, name) {
	return yield this.serializedImageAttrVal(node, node, name);
});

WS.prototype.getAttributeValueAsShadowInfo = Promise.async(function *(node, key) {
...
WS.prototype.serializedImageAttrVal = Promise.async(function *(dataMWnode, htmlAttrNode, key) {
	var v = yield this.getAttributeValueAsShadowInfo(dataMWnode, key);
	return v || DU.getAttributeShadowInfo(htmlAttrNode, key);
});

WS.prototype._serializeHTMLTag = Promise.async(function *(node, wrapperUnmodified) {
...
WS.prototype._serializeHTMLEndTag = Promise.method(function(node, wrapperUnmodified) {
	if (wrapperUnmodified) {
		var dsr = DU.getDataParsoid(node).dsr;
...
WS.prototype._serializeAttributes = Promise.async(function *(node, token, isWt) {
	...
	for (const kv of attribs) {
		...
		if (k === "id" && /H[1-6]/.test(node.nodeName)) {
			if (DU.getDataParsoid(node).reusedId === true) {
				vInfo = token.getAttributeShadowInfo(k);
				out.push(k + '=' + '"' + vInfo.value.replace(/"/g, '&quot;') + '"');
			}
			continue;
...
WS.prototype._handleLIHackIfApplicable = function(node) {
...
WS.prototype.serializePart = Promise.async(function *(state, buf, node, type, part, tplData, prevPart, nextPart) {
	...
	var forceTrim = (format !== null) || DU.isNewElt(node);
	...
	if (formatEOL) {
		if (nextPart === null) {
			...
			let next = DU.nextNonDeletedSibling(node);
			while (next && DU.isComment(next)) {
				next = DU.nextNonDeletedSibling(next);
			}
			if (!DU.isText(next) || !/^\n/.test(next.nodeValue)) {
				buf += '\n';
			}
...
WS.prototype.serializeFromParts = Promise.async(function *(state, node, srcParts) {
	var env = this.env;
...
WS.prototype.serializeExtensionStartTag = Promise.async(function *(node, state) {
...
WS.prototype.defaultExtensionHandler = Promise.async(function *(node, state) {
	var dataMw = DU.getDataMw(node);
 ...
 * @private
 */
WS.prototype._getDOMHandler = function(node) {
 ...
 * @private
 */
WS.prototype._serializeText = function(res, node, omitEscaping) {
	...
	if (omitEscaping) {
		...
		if (!state.inIndentPre) {
			...
			if (!state.inHTMLPre &&
				...
				(!DU.allChildrenAreText(node.parentNode) || doubleNewlineCount > 1)) {
 ...
 * @private
 */
WS.prototype._serializeTextNode = Promise.method(function(node) {
 ...
 */
WS.prototype.emitWikitext = function(res, node) {
	this._serializeText(res, node, true);
};

WS.prototype._getDOMAttribs = function(attribs) {
...
WS.prototype._serializeDOMNode = Promise.async(function *(node, domHandler) {
	...
	if (state.selserMode
			...
			|| (dp.dsr[1] === dp.dsr[0] &&
				(/^(P|BR)$/.test(node.nodeName) || DU.getDataMw(node).autoGenerated))
 ...
 * @private
 */
WS.prototype._serializeNode = Promise.async(function *(node) {
...
WS.prototype._stripUnnecessaryHeadingNowikis = function(line) {
	var state = this.state;
...
WS.prototype._stripUnnecessaryIndentPreNowikis = function() {
	var env = this.env;
		...
		if (reqd) {
			for (var j = 0; j < htmlTags.length; j++) {
				...
				if (!Consts.HTML.HTML5Tags.has(tagName)) {
					...
					reqd = true;
					break;
				} else if (Util.isBlockTag(tagName)) {
...
// - any single ' char before </i> or </b> does not need <nowiki/> protection.
WS.prototype._stripUnnecessaryQuoteNowikis = function(line) {
	...
	for (var j = 1; j < n; j += 2) {
		...
		// FIXME: This ignore business should happen for ALL extension
		// content, not just ref and nowiki
		// Ignore <ref>..</ref> sections -- FIXME: Leaky
 ...
 * WARNING: You probably want to use {@link DOMUtils.serializeDOM} instead.
 */
WS.prototype.serializeDOM = Promise.async(function *(body, selserMode) {
	console.assert(DU.isBody(body), 'Expected a body node.');
	// `editedDoc` is simply body's ownerDocument.  However, since we make
	// recursive calls to WS.prototype.serializeDOM with elements from dom fragments
	...
	if (psd.dumpFlags && psd.dumpFlags.has("dom:post-normal")) {
		DU.dumpDOM(body, 'DOM: post-normal', { storeDiffMark: true, env: this.env });
	}
