...
const { Normalizer } = require('./normalizeDOM.js');
...
const { tagHandlers, htmlElementHandler, _getEncapsulatedContentHandler } = require('./DOMHandlers.js');
...
const { WikitextEscapeHandlers } = require('./escapeWikitext.js');
...
class WikitextSerializer {
	constructor(options) {
		...
		this.wteHandlers = new WikitextEscapeHandlers(this.env, this);
...
WikitextSerializer.prototype._getDOMHandler = function(node) {
	...
	if (WTUtils.isFirstEncapsulationWrapperNode(node)) {
		return _getEncapsulatedContentHandler();
	}
...
WikitextSerializer.prototype._getDOMAttribs = function(attribs) {
