...
var LanguageVariantHandler = require('./LanguageVariantHandler.js').LanguageVariantHandler;
...
WSP.languageVariantHandler = LanguageVariantHandler;
...
WSP.serializePart = Promise.method(function(node, type, part, tplData) {
	...
	return Promise.map(tplKeysFromDataMw, function(k) {
		...
		if (tplData && Array.isArray(tplData.paramOrder)) {
			...
			paramOrder = tplData.paramOrder.reduce(function(prev, next) {
				var aliases = params && params[next] && params[next].aliases;
...
WSP.serializeFromParts = Promise.method(function(node, srcParts) {
	...
	return Promise.reduce(srcParts, function(buf, part) {
		...
		if (tplarg) {
			return self.serializePart(node, 'templatearg', tplarg, null)
			.then(function(str) {
				return buf + str;
			});
		}
		...
		return p.then(function(apiResp) {
			...
			if (fetched && tplData === null) {
				...
				return self.serializePart(node, type, tpl, null);
			}
		}).then(function(str) {
			return buf + str;
		});
...
WSP.serializeExtensionStartTag = Promise.method(function(node, state) {
	...
	return this._serializeAttributes(node, extTok, true).then(function(attrStr) {
		var src = '<' + extName;
		if (attrStr) {
			src += ' ' + attrStr;
		}
