...
WSP._handleLIHackIfApplicable = function(node, state) {
	...
	if (liHackSrc !== undefined &&
		...
		state.emitChunk(liHackSrc, node);
	}
};
...
WSP._htmlElementHandler = function(node, state, wrapperUnmodified) {
	...
	this._handleLIHackIfApplicable(node, state);
 ...
 */
WSP._getDOMHandler = function(node, state) {
	...
	if (DU.isFirstEncapsulationWrapperNode(node)) {
		return {
			handle: function() {
				...
				self.emitWikitext(src, state, node);
			...
			return {
				handle: function() {
					...
					if (dp.src.match(/^\n+$/)) {
						...
						self.emitWikitext(dp.src, state, node);
					}
				},
			};
		}
			...
			return {
				handle: function() {
					if (contentSrc === dp.srcContent) {
						...
						self.emitWikitext(contentSrc, state, node);
					}
				},
			};
		}
 ...
 */
WSP._serializeTextNode = function(node, state) {
 ...
 */
WSP.emitWikitext = function(text, state, node) {
	...
	switch (node.nodeType) {
		case node.ELEMENT_NODE:
				...
				if (domHandler && domHandler.handle) {
					...
					try {
						if (state.selserMode && DU.hasInsertedDiffMark(node, this.env)) {
							...
							nextNode = domHandler.handle(node, state, wrapperUnmodified);
			...
			if (next) {
				this.updateSeparatorConstraints(state,
						node, domHandler,
						next, this._getDOMHandler(next, state));
			}
		...
		case node.TEXT_NODE:
			if (!this.handleSeparatorText(node, state)) {
				...
				this._serializeTextNode(node, state);
				next = DU.nextNonSepSibling(node) || node.parentNode;
				if (next) {
					this.updateSeparatorConstraints(state,
							node, {},
...
WSP.serializeDOMSync = function(body, selserMode) {
	...
	if (state.hasIndentPreNowikis) {
		...
		state.out = stripUnnecessaryIndentPreNowikis(this.env, state.out);
	}
