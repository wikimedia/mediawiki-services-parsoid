...
WSP._serializeTemplate = function(node, tpl, isTpl, tplData) {
	...
	return Promise.map(tplKeysFromDataMw, function(k) {
		...
		var argIndex = 1;
		var numericIndex = 1;
		var numPositionalArgs = Array.reduce(dpArgInfo, function(n, pi) {
		...
		if (/.*"mediawiki.org\/specs\/data-parsoid\/0.0.1"$/.test(env.page.dpContentType)) {
		...
		var useInlineFormat = (format === 'inline' || DU.isNewElt(node));
		if (useBlockFormat) {
		...
		for (var i = 0; i < argBuf.length; i++) {
			...
			if (name !== null) {
				...
				if (name === '') {
					...
					// Default to inline-arg spacing 'foo=bar'
				...
				val = spc[0] + name + spc[1] + '=' + spc[2] + val + spc[3];
			}

			if (useBlockFormat) {
				// Add a space after the pipe only if required
				if (val[0] !== ' ') {
					buf += ' ';
				}
				buf += val;
				if (!TRAILING_COMMENT_OR_WS_AFTER_NL_REGEXP.test(val)) {
					buf += '\n';
				}
			} else if (useInlineFormat && name !== null) {
				// Trim values in inline-format mode only if
				// we are serializing in named-arg format
				buf += val.trim();
			} else {
				buf += val;
			}
...
WSP.defaultExtensionHandler = Promise.method(function(node, state) {
	...
	return this.serializeExtensionStartTag(node, state).then(function(src) {
		if (!dataMw.body) {
			...
			state.env.log('error', 'extension src unavailable for: ' + node.outerHTML);
		}
