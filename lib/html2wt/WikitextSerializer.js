...
WSP.languageVariantHandler = LanguageVariantHandler.languageVariantHandler;
...
WSP.getAttributeValue = Promise.async(function *(node, key, value) {
	var tplAttrs = DU.getDataMw(node).attribs;
	if (tplAttrs) {
...
WSP.serializedAttrVal = Promise.async(function *(node, name) {
	return yield this.serializedImageAttrVal(node, node, name);
});

WSP.getAttributeValueAsShadowInfo = Promise.async(function *(node, key) {
...
WSP.serializedImageAttrVal = Promise.async(function *(dataMWnode, htmlAttrNode, key) {
	var v = yield this.getAttributeValueAsShadowInfo(dataMWnode, key);
	return v || DU.getAttributeShadowInfo(htmlAttrNode, key);
});

WSP._serializeHTMLTag = Promise.async(function *(node, wrapperUnmodified) {
...
WSP._serializeAttributes = Promise.async(function *(node, token, isWt) {
...
WSP.serializePart = Promise.method(function(node, type, part, tplData) {
	...
	var tplKeysFromDataMw = Object.keys(part.params).map(function(k) {
	...
	var kvMap = new Map();
	return Promise.map(tplKeysFromDataMw, (k) => {
		var param = part.params[k];
		...
		if (param.wt !== undefined) {
			...
			p = this.serializeHTML({ env: env }, param.html);
		}
		...
		var numPositionalArgs = dpArgInfo.reduce((n, pi) => {
			return (part.params[pi.k] !== undefined && !pi.named) ? n + 1 : n;
		}, 0);

		return argOrder.reduce((argBuf, param) => {
			var kv = kvMap.get(param);
			// Add nowiki escapes for the arg value, as required
			var escapedValue = this.wteHandlers.escapeTplArgWT(kv.value, {
		...
		for (var i = 0; i < argBuf.length; i++) {
			...
			if (name === null) {
				...
				var spc = (dpArgInfoMap.get(arg.dpKey) || {}).spc;
				if (spc && (!format || Util.COMMENT_REGEXP.test(spc[3]))) {
					var nl = formatParamName.startsWith('\n') ? '\n' : '';
			...
			var trailing = TRAILING_COMMENT_OR_WS_AFTER_NL_REGEXP.test(buf);
...
WSP.serializeFromParts = Promise.method(function(node, srcParts) {
	var env = this.env;
	var useTplData = DU.isNewElt(node) || DU.hasDiffMarkers(node, env);
	return Promise.reduce(srcParts, (buf, part) => {
		var tplarg = part.templatearg;
		if (tplarg) {
			return this.serializePart(node, 'templatearg', tplarg, null)
			.then(function(str) {
				return buf + str;
			});
		}
		...
		return p.then((apiResp) => {
			...
			fetched = true;
			return this.serializePart(node, type, tpl, tplData);
		}).catch((err) => {
			if (fetched && tplData === null) {
				...
				return this.serializePart(node, type, tpl, null);
			}
		}).then(str => buf + str);
	}, '');
});

WSP.serializeExtensionStartTag = Promise.async(function *(node, state) {
...
WSP.defaultExtensionHandler = Promise.async(function *(node, state) {
	var dataMw = DU.getDataMw(node);
...
WSP._serializeDOMNode = Promise.async(function *(node, domHandler) {
 ...
 */
WSP._serializeNode = Promise.async(function *(node) {
 ...
 */
WSP.serializeDOM = Promise.async(function *(body, selserMode) {
