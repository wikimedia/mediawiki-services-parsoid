...
class DOMNormalizer {
	...
	normalizeNode(node) {
		...
		// they serialize to as many newlines as the count of <p></p> nodes.
			...
			// Give the br the dsr info from the p
			// This is mainly to support roundtrip testing where we don't expect to
			// have introduced any new nodes without dsr info
			// In the normal usage of the normalizer, we mark the br as inserted below
			DOMDataUtils.getDataParsoid(br).dsr = DOMDataUtils.getDataParsoid(node).dsr;
