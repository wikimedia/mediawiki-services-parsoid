	...
	return {
		...
		handle: function(node, state) {
			...
			if (node.childNodes.length) {
				...
				state.serializeChildren(node, headingHandler);
			} else {
	...
	return {
		handle: function(node, state) {
			...
			if (node.childNodes.length === 0) {
				...
				state.serializeChildren(node, null);
				WTSUtils.emitEndTag(quotes, node, state);
			}
		},
	};
...
var TagHandlers = {
	...
	table: {
		handle: function(node, state, wrapperUnmodified) {
			...
			state.emitChunk(serializeTableTag(wt, '', state, node, wrapperUnmodified), node);
			...
			if (!state.sep.constraints) {
				...
				// in SSP.pushSep after a separator is emitted. However, for {|\n|},
	...
	tr: {
		handle: function(node, state, wrapperUnmodified) {
			...
			if (DU.previousNonSepSibling(node) || dp.startTagSrc) {
				...
				WTSUtils.emitStartTag(res, node, state);
			}
			state.serializeChildren(node);
		},
		sepnls: {
	...
	pre_html: {
		handle: function(node, state) {
			return state.serializer._htmlElementHandler(node, state);
		},
		sepnls: {
			before: id({}),
			after: id({}),
		...
		handle: function(node, state) {
			...
			if (isRecognizedSpanWrapper(type)) {
				if (type === 'mw:Nowiki') {
					...
					state.serializer.figureHandler(node, state);
					...
					if (DU.isText(node.firstChild)) {
						state.emitChunk(Util.entityEncodeAll(node.firstChild.nodeValue),
							node.firstChild);
					} else {
						state.serializeChildren(node);
					}
					...
					if (/(^|\s)mw:Placeholder(\s|$)/ &&
						...
						state.emitChunk(' '.repeat(node.firstChild.nodeValue.length),
							node.firstChild);
					} else {
						state.serializer._htmlElementHandler(node, state);
					}
	...
	figure: {
		handle: function(node, state) {
			return state.serializer.figureHandler(node, state);
		},
		sepnls: {
	...
	img: {
		handle: function(node, state) {
			if (node.getAttribute('rel') === 'mw:externalImage') {
				state.serializer.emitWikitext(node.getAttribute('src') || '', state, node);
			} else {
				return state.serializer.figureHandler(node, state);
			}
		},
	...
	a:  {
		handle: function(node, state) {
			return state.serializer.linkHandler(node, state);
		},
