...
var tagHandlers = JSUtils.mapObject({
	...
	th: {
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			let previousTrailingSpace = '';
			// If there isn't whitespace already emitted for the previous cell,
			// see if we need to emit whitespace there.
			const previousCell = DU.previousNonDeletedSibling(node);
			if (previousCell && !/\s$/.test(state.currLine.text)) {
				previousTrailingSpace = inWideTH ? getTrailingSpace(state, previousCell, '') : '';
			}
			...
			WTSUtils.emitStartTag(previousTrailingSpace + thTag + leadingSpace,
				node,
				state
			);
			...
			if (!DU.firstNonDeletedChild(node)) {
	...
	td: {
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			const previousCell = DU.previousNonDeletedSibling(node);
			if (previousCell && !/\s$/.test(state.currLine.text)) {
				previousTrailingSpace = inWideTD ? getTrailingSpace(state, previousCell, '') : '';
			}
			const leadingSpace = getLeadingSpace(state, node, '');
			WTSUtils.emitStartTag(previousTrailingSpace + tdTag + leadingSpace, node, state);
