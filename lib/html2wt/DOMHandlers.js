	...
	return {
		...
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			if (node.hasChildNodes()) {
				yield state.serializeChildren(node, undefined, fc);
			} else {
...
function wtListEOL(node, otherNode) {
	...
	} else if (DU.isBlockNode(node.parentNode) && DU.lastNonSepChildNode(node.parentNode) === node) {
		return { min: 1, max: 2 };
	...
	return {
		...
		sepnls: {
			before: function(node, otherNode) {
				...
				if (DU.isBlockNode(node.parentNode) && DU.firstNonSepChildNode(node.parentNode) === node) {
					return { min: 1, max: 2 };
				} else if (DU.isFormattingElt(otherNode)) {
					return { min: 1, max: 1 };
				} else {
...
function isPbr(br) {
	return DU.getDataParsoid(br).stx !== 'html' && br.parentNode.nodeName === 'P' && DU.firstNonSepChildNode(br.parentNode) === br;
}
		...
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			if (isRecognizedSpanWrapper(type)) {
				if (type === 'mw:Nowiki') {
					...
					/* ignore contents, this is just a legacy compat ID */
	...
	br: {
		...
		sepnls: {
			before: function(node, otherNode, state) {
				...
				if (isPPTransition(DU.previousNonSepSibling(node.parentNode))) {
					// <p>..</p><p><br/>.. => at least 3 newlines at this point
					// Whether we need 4 depends on what follows this <p><br/>
					if (isPbr(node)) {
						return { min: Math.max(3, (c.min || 0)), force: true };
					}
				} else {
					if (isPbrP(node)) {
						// <h2>..</h2><p><br/><p> => 2 newlines after heading
						// (or list, table, etc.)
						return { min: Math.max(2, (c.min || 0)), force: true };
					} else if (isPbr(node)) {
						// <h2>..</h2><p><br/> => 1 newline after heading
						// (or list, table, etc.)
						return { min: Math.max(1, (c.min || 0)), force: true };
					}
				}
			...
			after: function(node, otherNode, state) {
				...
				var c = state.sep.constraints || { min: 0 };
				if (isPPTransition(DU.nextNonSepSibling(node.parentNode))) {
					if (isPbrP(node)) {
						// <p><br/></p><p>..</p> => at least 4 newlines before
						// content of the *next* p-tag
						return { min: Math.max(4, (c.min || 0)), force: true };
					} else if (isPbr(node)) {
						// <p><br/>..<p><p>..</p> => at least 2 newlines after
						// content of *this* p-tag
						return { min: Math.max(2, (c.min || 0)), force: true };
					}
				} else {
					if (isPbrP(node)) {
						// <p><br/><p><h2>..</h2> => 3 newlines before heading
						// (or list, table, etc.)
						return { min: Math.max(3, (c.min || 0)), force: true };
					} else if (isPbr(node)) {
						// <p><br/><p><h2>..</h2> => 2 newlines before heading
						// (or list, table, etc.)
						return { min: Math.max(2, (c.min || 0)), force: true };
					}
				}
	...
	return {
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			if (/(?:^|\s)(?:mw:Transclusion|mw:Param)(?=$|\s)/.test(typeOf)) {
				if (dataMw.parts) {
					src = yield self.serializeFromParts(node, dataMw.parts);
				} else if (dp.src !== undefined) {
