...
function getLeadingSpace(state, node, newEltDefault, fc) {
	let space = '';
	if (!fc) {
		fc = DU.firstNonDeletedChildNode(node);
	}
	if (DU.isNewElt(node)) {
		if (fc && (!DU.isText(fc) || !fc.nodeValue.match(/^\s/))) {
			space = newEltDefault;
		}
	} else if (state.selserMode && (!fc || !DU.isElt(fc))) {
...
function getTrailingSpace(state, node, newEltDefault, lc) {
	let space = '';
	if (!lc) {
		lc = DU.lastNonDeletedChildNode(node);
	}
	if (DU.isNewElt(node)) {
		if (lc && (!DU.isText(lc) || !lc.nodeValue.match(/\s$/))) {
			space = newEltDefault;
		}
	} else if (state.selserMode && (!lc || !DU.isElt(lc))) {
		const dsr = DU.getDataParsoid(node).dsr;
		if (Util.isValidDSR(dsr, true)) {
...
function buildHeadingHandler(headingWT) {
	return {
		forceSOL: true,
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			let fc = DU.firstNonDeletedChildNode(node);
			// Skip fallback ID if present.
			if (fc && DU.isFallbackIdSpan(fc)) {
				fc = DU.nextNonDeletedSibling(fc);
			}
			...
			let lc = DU.lastNonDeletedChildNode(node);
			// Skip fallback ID if present.
			if (lc && DU.isFallbackIdSpan(lc)) {
				lc = DU.previousNonDeletedSibling(lc);
			}
			space = getTrailingSpace(state, node, ' ', lc);
...
var tagHandlers = JSUtils.mapObject({
	...
	th: {
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			if (!DU.firstNonDeletedChildNode(node)) {
				var nextTd = DU.nextNonDeletedSibling(node);
				if (nextTd && DU.getDataParsoid(nextTd).stx === 'row') {
