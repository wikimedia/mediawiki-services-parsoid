	...
	return {
		...
		sepnls: {
			before: function(node, otherNode) {
				...
				var parentIsListItem = DU.isListItem(node.parentNode);
				if (DU.isBody(otherNode)) {
					return { min: 0, max: 0 };
				} else if (DU.isText(otherNode) && parentIsListItem) {
					// A list nested inside a list item
					// <li> foo <dl> .. </dl></li>
					return { min: 1, max: 1 };
				} else if (DU.isListOrListItem(otherNode) && parentIsListItem) {
					return { min: 1, max: 2 };
				// A list in a block node (<div>, <td>, etc) doesn't need a leading empty line
				// if it is the first non-separator child (ex: <div><ul>...</div>)
				} else if (DU.isBlockNode(node.parentNode) && DU.firstNonSepChildNode(node.parentNode) === node) {
					return { min: 1, max: 2 };
				} else if (DU.isFormattingElt(otherNode)) {
					return { min: 1, max: 1 };
				} else {
