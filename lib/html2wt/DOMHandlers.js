	...
	span: {
		handle: Promise.method(function(node, state, wrapperUnmodified) {
			...
			if (isRecognizedSpanWrapper(type)) {
				if (type === 'mw:Nowiki') {
					// src seems to have been added to nowiki spans for
					// T44353 which looks like it predates selser.  We also
					// seem to added src for self-closed (currently metas)
					// nowikis to preserve syntactic variations.  Possibly
					// worth dropping for both cases.
					if (dp.src !== undefined && contentSrc === dp.src) {
						return emitPlaceholderSrc(node, state);
					}
					state.emitChunk('<nowiki>', node);
