...
WEHP.hasWikitextTokens = function(state, onNewline, options, text) {
	...
	for (var i = 0, n = tokens.length; i < n; i++) {
		...
		if (Util.isHTMLTag(t)) {
			...
			if ((tc === pd.TagTk || tc === pd.EndTagTk) &&
				state.env.conf.wiki.extensionTags.has(t.name.toLowerCase())) {
				return true;
			}
		...
		if (tc === pd.TagTk) {
			...
			if (t.name in {td: 1, tr: 1, th: 1} && !Util.isHTMLTag(t) && state.wikiTableNesting === 0) {
				continue;
			}
 ...
 * ---------------------------------------------------------------- */
WEHP.escapedText = function(state, sol, origText, fullWrap, dontWrapIfUnnecessary) {
	try {
		...
		state.env.log("error/html2wt/wtescape", "Error escaping text", origText);
	}
};
 ...
 */
WEHP.escapeTplArgWT = function(arg, opts) {
