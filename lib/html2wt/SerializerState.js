 ...
 */
SSP.serializeChildren = Promise.method(function(node, wtEscaper, firstChild) {
	// SSS FIXME: Unsure if this is the right thing always
	if (wtEscaper) { this.wteHandlerStack.push(wtEscaper); }

	var serializeNextChild = Promise.method((child) => {
	...
	return serializeNextChild(firstChild || node.firstChild).then(() => {
