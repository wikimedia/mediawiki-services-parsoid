...
SingleLineContext.prototype.enforce = function() {
	this._stack.push(true);
};

SingleLineContext.prototype.enforced = function() {
	return this._stack.length > 0 && JSUtils.lastItem(this._stack);
};

SingleLineContext.prototype.disable = function() {
	this._stack.push(false);
};

SingleLineContext.prototype.pop = function() {
	this._stack.pop();
};
...
/**
 */
SerializerState.prototype.initMode = function(selserMode) {
	this.useWhitespaceHeuristics = semver.gte(this.env.originalVersion, '1.7.0');
 ...
 */
SerializerState.prototype.appendSep = function(src) {
 ...
 */
SerializerState.prototype.updateSep = function(node) {
 ...
 */
SerializerState.prototype.resetCurrLine = function(node) {
	this.currLine = {
		text: '',
		chunks: [],
		firstNode: node,
...
/**
 */
SerializerState.prototype.flushLine = function() {
 ...
 */
SerializerState.prototype.serializeChildren = Promise.async(function *(node, wtEscaper, firstChild) {
 ...
 */
SerializerState.prototype.getOrigSrc = function(start, end) {
 ...
 */
SerializerState.prototype.updateModificationFlags = function(node) {
 ...
 */
SerializerState.prototype.sepIntroducedSOL = function(sep) {
 ...
 */
SerializerState.prototype.pushToCurrLine = function(text, node) {
 ...
 */
SerializerState.prototype.emitSep = function(sep, node, debugPrefix) {
 ...
 */
SerializerState.prototype.emitSepForNode = function(node) {
	var again = (node === this.sep.lastSourceNode);
	var origSepUsable = !again &&
		this.prevNodeUnmodified && !DU.nextToDeletedBlockNodeInWT(this.prevNode, true) &&
		this.currNodeUnmodified && !DU.nextToDeletedBlockNodeInWT(node, false);
 ...
 */
SerializerState.prototype.emitChunk = function(res, node) {
	...
	if (this.escapeText) {
		...
		if (this.selserMode
			...
			var pChild = DU.firstNonSepChild(node);
 ...
 * @private
 */
SerializerState.prototype._kickOffSerialize = Promise.async(function *(node, wtEscaper) {
	this.updateSep(node);
	this.currNodeUnmodified = false;
 ...
 */
SerializerState.prototype._serializeChildrenToString = Promise.async(function *(node, wtEscaper, inState) {
...
SerializerState.prototype.serializeLinkChildrenToString = Promise.method(function(node, wtEscaper) {
	return this._serializeChildrenToString(node, wtEscaper, 'inLink');
});

SerializerState.prototype.serializeCaptionChildrenToString = Promise.method(function(node, wtEscaper) {
	return this._serializeChildrenToString(node, wtEscaper, 'inCaption');
});

SerializerState.prototype.serializeIndentPreChildrenToString = Promise.method(function(node, wtEscaper) {
