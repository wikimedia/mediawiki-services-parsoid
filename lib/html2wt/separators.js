	...
	if (sepNlsHandlerB) {
		...
		if (b.min !== undefined) {
			if (nlConstraints.max !== undefined && nlConstraints.max < b.min) {
				...
				env.log("warn", "Incompatible constraints 1:", nodeA.nodeName,
		...
		if (b.max !== undefined) {
			if (nlConstraints.min !== undefined && nlConstraints.min > b.max) {
				// Conflict, warn and let nodeB win.
				env.log("warn", "Incompatible constraints 2:", nodeA.nodeName,
						nodeB.nodeName, loggableConstraints(nlConstraints));
	...
	if (origSepUsable) {
		...
		if (Util.isValidDSR(dsrA) && Util.isValidDSR(dsrB)) {
			...
			if (dsrA[0] <= dsrB[0]) {
				...
				this.env.log("warn", "dsr backwards: should not happen!");
			}
