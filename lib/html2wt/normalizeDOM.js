	...
	while (true) {  // eslint-disable-line
		...
		if (insertedSubtree) {
			if (this.inInsertedContent) {
				...
				console.warn("Node:", DU.isElt(node) ? node.outerHTML : node.textContent);
				console.warn("Node's parent:", node.parentNode.outerHTML);
				DU.dumpDOM(node.ownerDocument.body, '-- DOM triggering nested inserted dom-diff flags --');
			}
