...
Normalizer.prototype.addDiffMarks = function(node, mark, dontRecurse) {
	...
	if (!DU.isNewElt(node)) {
		...
		if (mark === 'inserted' || mark === 'deleted') {
			DU.addDiffMark(node.parentNode, env, 'children-changed');
		}
	}
...
function isInsertedContent(node, env) {
	while (true) {
		if (DU.hasInsertedDiffMark(node, env)) {
			return true;
		}
		if (DU.isBody(node)) {
			return false;
...
Normalizer.prototype.normalizeNode = function(node) {
	...
	// they serialize to as many newlines as the count of <p></p> nodes.
	} else if (node.nodeName === 'P' && !DU.isLiteralHTMLNode(node) &&
		...
		if (next && next.nodeName === 'P' && !DU.isLiteralHTMLNode(next)) {
...
Normalizer.prototype.processNode = function(node, recurse) {
	...
	while (true) {  // eslint-disable-line
		...
		while (node && DU.isFirstEncapsulationWrapperNode(node)) {
		...
		if (insertedSubtree) {
			...
			// either be removed OR fixed up to remove uses of DU.ppToXML
