...
Normalizer.prototype.moveFormatTagOutsideATag = function(node) {
	if (this.inRtTestMode || node.nodeName !== 'A') {
		...
		// If there are no format tags to swap, we are done
		// do not normalize WikiLinks with color, style or class attributes
		if (firstChild && !DU.isText(firstChild) &&
			!firstChild.getAttribute('color') && !firstChild.getAttribute('style') &&
			!firstChild.getAttribute('class')) {
			var hrefName = node.getAttribute('href');
			if (fcNextSibling === null) {
				var child;
				...
				if (hrefName.replace(/\.\//, '') === node.textContent) {
					while ((child = DU.firstNonDeletedChildNode(node)) && DU.isFormattingElt(child)) {
...
Normalizer.prototype.normalizeNode = function(node) {
	...
	// Anchors
		...
		node = this.moveFormatTagOutsideATag(node);
		return node;
	...
	// Table cells
		...
		if (dp.stx === 'html' ||
			(DU.firstNonSepChildNode(node.parentNode) !== node && dp.stx === 'row')) {
			return node;
		}
