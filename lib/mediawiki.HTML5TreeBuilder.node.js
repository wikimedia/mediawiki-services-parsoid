...
TreeBuilder.prototype.processToken = function(token) {
	...
	switch (token.constructor) {
		...
		case SelfclosingTagTk:
			...
			// But, <ref> marker metas should be fostered since
			// they end up generating HTML content at the marker site.
			if (tName === "meta") {
				var tTypeOf = token.getAttribute( "typeof" );
				if (tTypeOf && tTypeOf.match(/^mw:/) && !tTypeOf.match(/^mw:Extension\/ref\/Marker\b/)) {
					// transclusions state
