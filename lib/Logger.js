 ...
 */
var Logger = function(env, dontRegisterDefaultBackend) {
	this._env = env;
	this._logRequestQueue = [];
	this._backends = new Map();
	this._defaultLogLevels = ["error", "warning", "fatal", "fatal\/request"];
	this._traceTestRegExp = this._traceTest();

	if (!dontRegisterDefaultBackend) {
 ...
 * @method
 *
 * Enables us to turn logging on and off for different logTypes,
 * by changing defaultLogTypes.
 * @param {String} logType
 */
Logger.prototype.changeLogTypes = function (newLogLevels) {
	this._defaultLogLevels = Array.prototype.slice.call(arguments);
	this._traceTestRegExp = this._traceTest();
};
 ...
 */
Logger.prototype._traceTest = function() {
	var traceTypes = "";
	var debugOn = "";
	var escapedTraceFlags;

	if (this._env.conf.parsoid.traceFlags !== null) {
		escapedTraceFlags = this._env.conf.parsoid.traceFlags.map(Util.escapeRegExp);
	...
	return new RegExp(this._defaultLogLevels.join("|") + traceTypes + debugOn);
};
