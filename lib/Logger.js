...
var Logger = function(env) {
	...
	this.traceTest = (function() {
		...
		if (self.env.conf.parsoid.traceFlags !== null) {
			escapedTraceFlags = self.env.conf.parsoid.traceFlags.map(Util.escapeRegExp);
			traceTypes = "|trace\/(" + escapedTraceFlags.join("|") + ")";
		}

		if (self.env.conf.parsoid.debug) {
			debugOn = "|debug";
		}

		return new RegExp("error|warning|fatal" + traceTypes + debugOn);
	})();
};
...
Logger.prototype.routeToBackends = function(logType, obj, cb) {
	...
	async.parallel(applicableBackends, function(logType, processFatalCB){
