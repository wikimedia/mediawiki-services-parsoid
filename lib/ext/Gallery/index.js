			...
			case child.ELEMENT_NODE:
				...
				if (elt) {
					...
					if (resource !== null) {
						...
						if (alt !== null) {
							content += '|alt=' + state.serializer.wteHandlers.escapeLinkContent(state, alt, false, child);
						}
					}
				} else {
				...
				if (a) {
					...
					if (href !== null && href !== resource) {
						content += '|link=' + state.serializer.wteHandlers.escapeLinkContent(state, href.replace(/^\.\//, ''), false, child);
					}
				}
