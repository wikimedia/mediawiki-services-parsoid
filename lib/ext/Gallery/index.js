...
var pCaption = Promise.async(function *(data) {
	...
	DOMUtils.visitDOM(doc.body, DOMDataUtils.storeDataAttribs);
	return doc.body;
});

var pLine = Promise.async(function *(data, obj) {
	const { state, opts } = data;
	const env = state.manager.env;
	...
	if (gallerytext) {
		// Store before `migrateChildrenBetweenDocs` in render
		DOMUtils.visitDOM(gallerytext, DOMDataUtils.storeDataAttribs);
	}
...
const toDOM = function(state, content, args) {
	...
	const opts = new Opts(state.manager.env, attrs);
	...
	.then(function(ret) {
		...
		DOMUtils.visitDOM(doc.body, DOMDataUtils.loadDataAttribs);
		return doc;
	});
};
