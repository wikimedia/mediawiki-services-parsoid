...
const DU = ParsoidExtApi.DOMUtils;

const dummyDoc = DU.parseHTML('');
...
// FIXME: We could expand the parseWikitextToDOM helper to let us
// pass in a handler that post-processes the DOM immediately,
// instead of in the end.
const domPostProcessor = function(node, env, options, atTopLevel) {
	if (!atTopLevel) {
		return;
	}
	...
	while (c) {
		if (DU.isElt(c)) {
			if (/\bmw:Extension\/poem\b/.test(c.getAttribute('typeof'))) {
				...
				domPostProcessor(c, env, options, atTopLevel);
			}
		}
		c = c.nextSibling;
	}
