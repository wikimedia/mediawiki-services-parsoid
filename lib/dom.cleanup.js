...
function stripMarkerMetas(rtTestMode, node, atTopLevel) {
	...
	// of those metas before deleting it. Also check if the node has data-mw, in
	// which case we also have to keep it, except if it's also a mw:extension/ref/Marker
	...
	if (metaType
		&& ((metaTestRE.test(metaType) && !node.getAttribute("property")
				&& (!DU.getNodeData(node).mw || metaType.match(/mw:Extension\/ref\/Marker/)))
	...
	if ( DU.isElt(node) ) {
		...
		if (dp) {
			...
			if (validDataMW && Util.isValidDSR(dp.dsr)) {
				dp.src = undefined;
			} else if (/(?:^|\s)mw:(Transclusion|Extension)(?=$|\s)/.test(node.getAttribute("typeof")) &&
				!dp.tsr) {
