...
function cleanupAndSaveDataParsoid( node, env, atTopLevel, tplInfo ) {
	...
	if ( atTopLevel ) {
		...
		if ((tplInfo &&
			// Always keep for the first node.
			tplInfo.first !== node &&
			...
			// which content model the text came from to emit the right
			// newline separators. For example, "a\n\nb" and "<p>a</p><p>b/p>"
			// both generate identical html but serialize to different wikitext.
			!dp.stx) ||
			env.discardDataParsoid
		) {
