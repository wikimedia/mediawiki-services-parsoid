...
function ApiRequest(env, title) {
	...
	// public one.
	this.mwApiServer = env.conf.parsoid.mwApiServer;
}
...
ApiRequest.prototype.request = function(requestOpts) {
	...
	// As for the proxying case, we allow subclasses to override this when they
	// set the proxy variable to `null` or to a different value.
	// See ParsoidConfig.initMwApiMap
	if (mwApiServer && options.proxy === undefined) {
