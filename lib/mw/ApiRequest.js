...
ApiRequest.prototype.request = function(requestOpts) {
	...
	return request(options, this._requestCB.bind(this, Date.now()));
};
...
ApiRequest.prototype._requestCB = function(startTime, error, response, body) {
	var s;
	if (this.traceTime) {
		this.env.bumpIOTime(this.constructor.name, Date.now() - startTime);
	...
	if (this.traceTime) {
		this.env.bumpTimeUse("API response processing time", Date.now() - s);
...
BatchRequest.prototype._handleJSON = function(error, data) {
	...
	if (this.traceTime) {
		...
		if (data['parsoid-batch-time']) {
			...
			this.env.bumpTimeUse('Batch CPU', Math.round(data['parsoid-batch-time'] * 1000));
		}
	}
 ...
 * @param {MWParserEnvironment} env
 */
var ConfigRequest = function(env) {
	...
	if (dims) {
		if (dims.width) {
			...
			if (dims.page !== undefined) {
				...
				apiArgs[prefix + 'urlparam'] = util.format('page%d-%dpx', dims.page, dims.width);
			}
		}
		if (dims.height) {
		...
		if (dims.seek !== undefined) {
			apiArgs[prefix + 'urlparam'] = util.format('seek=%d', dims.seek);
		}
	}
