...
PegTokenizer.prototype.initTokenizer = function() {
	...
	tokenizerSource = tokenizerSource
		...
		.replace(/(var key    = peg\$currPos [^,]+)/g, "$1 + stops.key")
...
PegTokenizer.prototype._processText = function( text, fullParse ) {
	...
	if (fullParse) {
		...
		this.tokenizeAsync(text, 0, pipelineOffset, null, chunkCB);
	}
};

PegTokenizer.prototype.tokenizeAsync = function( text, startOffset, pipelineOffset, peg, cb ) {
	...
	if (ret.eof) {
		...
		setImmediate(function() {
			pegTokenizer.tokenizeAsync( text, ret.newOffset, pipelineOffset, peg, cb );
		});
	}
};
