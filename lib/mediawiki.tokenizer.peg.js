...
PegTokenizer.prototype._processText = function( text, fullParse ) {
	...
	if (fullParse) {
		...
		this.tokenizeAsync(text, 0, pipelineOffset, chunkCB);
	}
};

PegTokenizer.prototype.tokenizeAsync = function( text, startOffset, pipelineOffset, cb ) {
	var ret,
		pegTokenizer = this,
		args = {
			cb: cb,
	...
	if (ret.eof) {
		...
		setImmediate(function() {
			pegTokenizer.tokenizeAsync( text, ret.newOffset, pipelineOffset, cb );
		});
	}
};
