...
PegTokenizer.prototype.initTokenizer = function() {
	...
	// Bucket the cache by offset so that clearing it is easier.
	// Include the stops key in the cache key;
	tokenizerSource = tokenizerSource
		.replace(/      var key    = peg\$currPos \* \d+ \+ (\d+),\n          cached = peg\$cache\[key\];/g,
			"      var key    = \"$1\" + stops.key;\n" +
			"      var bucket = peg$currPos;\n" +
			"      if ( !peg$cache[bucket] ) { peg$cache[bucket] = {}; }\n" +
			"      var cached = peg$cache[bucket][key];")
		.replace(/peg\$cache\[key\] =/g, "peg$cache[bucket][key] =");

	// Keep these constant throughout an entire tokenization pass.
	tokenizerSource = tokenizerSource
		.replace(/(peg\$FAILED =) {}/, "$1 options.peg$FAILED || {}")
		.replace(/(peg\$cache =) {}/, "$1 options.peg$cache || {}");
	...
	if (!PARSOID_DUMP_TOKENIZER) {
		...
		tokenizerSource = tokenizerSource
			...
			.replace(/new ctor\(\);/, function(m) {
				return m + "\n    /* jscs:enable requireCapitalizedConstructors */";
			});
		tokenizerSource =
			'/* jshint loopfunc:true, latedef:false, nonstandard:true, -W100 */\n' +
			'/* jscs:disable disallowMultipleVarDecl, validateIndentation */\n' +
			'\'use strict\';\n' +
			'require(\'./core-upgrade.js\');\n' +
...
PegTokenizer.prototype._processText = function( text, fullParse ) {
	...
	if (fullParse) {
		...
		this.tokenizeAsync(text, 0, 0, pipelineOffset, null, chunkCB);
	}
};

PegTokenizer.prototype.tokenizeAsync = function( text, prevOffset, startOffset, pipelineOffset, peg, cb ) {
	// Initialize a some peg vars scoped to this chain
	if ( !peg ) {
		peg = { cache: {}, FAILED: {} };
	}
	...
	if (ret.eof) {
		...
		setImmediate(function() {
			pegTokenizer.tokenizeAsync( text, startOffset, ret.newOffset, pipelineOffset, peg, cb );
		});
	}
};
 ...
 */
PegTokenizer.prototype.tokenize = function(text, rule, args, throwErr) {
	if ( !this.tokenizer ) {
		this.initTokenizer();
	}
