...
function ibSiblingHandler(state, text, opts) {
	var node = opts.node;

	// Ensure that this is a *new* text-node.
	if (!node.previousSibling ||
		!DU.isMarkerMeta(node.previousSibling, "mw:DiffMarker")) {
		return false;
	}

	if (text.match(/'$/)) {
		var next = DU.nextNonDeletedSibling(node);
		return next && Consts.WTQuoteTags.has(next.nodeName);
	} else if (text.match(/^'/)) {
		var prev = DU.previousNonDeletedSibling(node);
		return prev && Consts.WTQuoteTags.has(prev.nodeName);
	}
...
WEHP.escapeWikiText = function(state, text, opts) {
	...
	// i/b tags siblings
	if (ibSiblingHandler(state, text, opts)) {
		state.env.log("trace/wt-escape", "---i/b tag siblings---");
		return this.escapedText(state, false, text, true);
	}
