...
WEHP.tdHandler = function(tdNode, state, text, opts) {
	...
	return (!opts.node || state.currLine.firstNode === tdNode) &&
		(text.match(/\|/) || (
			...
			// we don't need to escape it.  We also don't need to escape <td><small>-</small></td>;
			// make sure every node on the path is the first child of its parent & is zero-width.
			text.match(/^[\-+]/) &&
	...
	try {
		...
		if (fullWrap) {
			...
			var smartNowikier = function(open, close, str, i, numToks) {
				...
				// sucked into a nowiki (40 is an arbitrary number)
...
WEHP.escapeTplArgWT = function(state, arg, opts) {
	...
	function escapeStr(str, pos) {
		...
		if (genericMatch || (!serializeAsNamed && /[=]/.test(str))) {
			...
			// Replace | with {{!}} if we haven't set it inside nowiki yet.
			buf += str.replace(/\|/g, "{{!}}");
		} else {
			buf += str;
		}
