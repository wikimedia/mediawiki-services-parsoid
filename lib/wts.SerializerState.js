...
var DU = require('./mediawiki.DOMUtils.js').DOMUtils,
	...
	escapeLine = require('./wts.ConstrainedText.js').escapeLine;
...
SSP.emitSepAndOutput = function(res, node, cb, logPrefix) {
	 ...
	 * -------------------------------------------------------------------------- */
	var origSepUsable =
		this.prevNodeUnmodified && !DU.nextToDeletedBlockNodeInWT(this.sep.lastSourceNode, true) &&
	...
	if (this.escapeText) {
			...
			&& node.nodeName === 'P' && !DU.isLiteralHTMLNode(node))
		{
			...
			if (pChild && DU.isText(pChild)) {
				var match = res.match(/^((?:<!--(?:[^\-]|-(?!->))*-->|<includeonly>.*?<\/includeonly>)*)([ \*#:;{\|!=].*)$/);
				if (match && match[2]) {
					if (/^([\*#:;]|{\||.*=$)/.test(match[2]) ||
					...
					{
						res = ConstrainedText.cast((match[1]||"") + "<nowiki>" + match[2][0] + "</nowiki>" + match[2].substring(1), node);
					}
				}
			}
		}
...
SSP.serializeChildrenToString = function(node, wtEscaper, onSOL) {
	...
	var bits = '',
		oldSep = this.sep,
		oldChunks = this.currLine.chunks,
