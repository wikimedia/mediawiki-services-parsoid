...
Util.registerLoggingBackends = function(defaultLogLevels, logger, parsoidConfig) {
	...
	logger.registerDefaultBackend(defaultRE);
	...
	if (parsoidConfig.traceFlags) {
		logger.registerTracer(buildTraceOrDebugFlag(parsoidConfig.traceFlags, "trace"));
	}
	if (parsoidConfig.debugFlags) {
		logger.registerTracer(buildTraceOrDebugFlag(parsoidConfig.debugFlags, "debug"));
	}
	if (parsoidConfig.linting){
		var linter = new Linter();
		logger.registerBackend(/lint(\/.*)?/, linter.linterBackend.bind(linter));
		logger.registerBackend(/end\/parse/, linter.emitLintOutput.bind(linter));
		logger.registerBackend(/end(\/(?!parse).*)?/, linter.logLintOutput.bind(linter));
