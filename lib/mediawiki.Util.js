...
var Util = {
	...
	setDebuggingFlags: function(obj, opts) {
		...
		if (opts.trace !== undefined) {
			if ( opts.trace === true) {
				...
				obj.traceFlags = (obj.traceFlags || []).concat(this.splitFlags(opts.trace));
			}
		}
	...
	setTemplatingAndProcessingFlags: function(obj, opts) {
		...
		['fetchConfig', 'fetchTemplates', 'rtTestMode', 'addHTMLTemplateParameters'].forEach(function(c) {
			if (opts[c] !== undefined) {
		...
		if (opts.addHTMLTemplateParameters !== undefined) {
			obj.addHTMLTemplateParameters = Util.booleanOption(opts.addHTMLTemplateParameters);
		}
		if (opts.lint) {
			obj.linting = true;
			obj.linterAPI = null;
	...
	*/
	...
	isSolTransparentLinkTag: function(token) {
		...
		return (tc === pd.SelfclosingTagTk || tc === pd.TagTk || tc === pd.EndTagTk) &&
			token.name === 'link' &&
			/mw:PageProp\/(?:Category|redirect|Language)/.test(token.getAttribute('rel'));
	},
	...
	KVtoHash: function ( kvs, convertValuesToString ) {
		...
		for ( var i = 0, l = kvs.length; i < l; i++ ) {
			var kv = kvs[i],
				...
				val = convertValuesToString ? this.tokensToString(kv.v) : kv.v;
...
Util.registerLoggingBackends = function(defaultLogLevels, logger, parsoidConfig, linter) {
	...
	var loggerBackend, tracerBackend;
	loggerBackend = (typeof(parsoidConfig.loggerBackend) === 'function') ?
		parsoidConfig.loggerBackend : logger.getDefaultBackend();
	logger.registerBackend(defaultRE, loggerBackend);
	...
	tracerBackend = (typeof(parsoidConfig.tracerBackend) === 'function') ?
					parsoidConfig.tracerBackend : logger.getDefaultTracerBackend();
	if (parsoidConfig.traceFlags) {
		logger.registerBackend(buildTraceOrDebugFlag(parsoidConfig.traceFlags, "trace"),
			tracerBackend);
	}
	if (parsoidConfig.debugFlags) {
		logger.registerBackend(buildTraceOrDebugFlag(parsoidConfig.debugFlags, "debug"),
			tracerBackend);
	}
	if (parsoidConfig.linting){
		logger.registerBackend(/lint(\/.*)?/, linter.linterBackend.bind(linter));
		logger.registerBackend(/end(\/.*)/, linter.logLintOutput.bind(linter));
	}
};
