...
PreHandler.prototype.onNewline = function (token, manager, cb) {
	...
	env.log("debug/pre", "----------");
	env.log("trace/pre", "NL  |", PreHandler.STATE_STR[this.state], "|", function() { return JSON.stringify(token); });
	...
	env.log("debug/pre", "saved:", this.tokens);
	env.log("debug/pre", "ret:", function() { return JSON.stringify(ret); });
...
PreHandler.prototype.onAny = function ( token, manager, cb ) {
	...
	env.log("debug/pre","----------");
	env.log("trace/pre", "any |", PreHandler.STATE_STR[this.state], "|", function() { return JSON.stringify(token); });
