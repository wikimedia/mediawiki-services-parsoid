...
SSP.doSerializeDOM = function( err, body, cb, finalcb ) {
	...
	if ( err || (!this.env.page.dom && !this.env.page.domdiff) || !this.env.page.src ) {
		...
		if ( this.timer ) {
			this.timer.timing( 'html2wt.serialize.full', '', ( startTimers.get( 'html2wt.serialize.full' ) - Date.now()) );
		}
	} else {
		...
		if ( this.timer ) {
			this.timer.timing( 'html2wt.domDiff', '', ( startTimers.get( 'html2wt.domDiff' ) - Date.now()) );
		}
		...
		if ( !diff.isEmpty ) {
			...
			if ( this.timer ) {
				this.timer.timing( 'html2wt.serialize.selser', '', ( startTimers.get( 'html2wt.serialize.selser' ) - Date.now()) );
			}
