...
SSP.doSerializeDOM = function( err, body, cb, finalcb ) {
	...
	if ( err || (!this.env.page.dom && !this.env.page.domdiff) || !this.env.page.src ) {
		...
		if ( this.timer ) {
			this.timer.timing( 'html2wt.serialize.full', '', ( Date.now() - startTimers.get( 'html2wt.serialize.full' )) );
		}
	} else {
		...
		if ( this.timer ) {
			this.timer.timing( 'html2wt.domDiff', '', ( Date.now() - startTimers.get( 'html2wt.domDiff' )) );
		}
		...
		if ( !diff.isEmpty ) {
			...
			if ( this.timer ) {
				this.timer.timing( 'html2wt.serialize.selser', '', ( Date.now() - startTimers.get( 'html2wt.serialize.selser' )) );
			}
