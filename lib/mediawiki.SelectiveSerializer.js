...
var ParsoidCacheRequest = require('./mediawiki.ApiRequest.js').ParsoidCacheRequest;
 ...
 * @param {Error} err
 * @param {Node} body
 * @param {Function} cb The callback for when we've finished serializing the DOM.
 */
SSP.doSerializeDOM = function( err, body, cb ) {
	...
	if ( err || (!this.env.page.dom && !this.env.page.domdiff) || !this.env.page.src ) {
		...
		if ( this.timer ) {
			this.timer.timing( 'html2wt.full.serialize', '', ( Date.now() - startTimers.get( 'html2wt.full.serialize' )) );
		}
	} else {
		...
		if ( this.timer ) {
			this.timer.timing( 'html2wt.selser.domDiff', '', ( Date.now() - startTimers.get( 'html2wt.selser.domDiff' )) );
		}
		...
		if ( !diff.isEmpty ) {
			...
			if ( this.timer ) {
				this.timer.timing( 'html2wt.selser.serialize', '', ( Date.now() - startTimers.get( 'html2wt.selser.serialize' )) );
			}
 ...
 * @param {Function} cb The callback for after we've serialized the entire document.
 * @param {Error} err
