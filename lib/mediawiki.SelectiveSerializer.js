...
SSP.doSerializeDOM = function( err, body, cb, finalcb ) {
	...
	if ( err || (!this.env.page.dom && !this.env.page.domdiff) || !this.env.page.src) {
		...
		if ( this.timer ) {
			startTimers.set( 'html2wt.serialize.partial', Date.now() );
		...
		if ( !diff.isEmpty ) {
			...
			if ( this.timer ) {
				this.timer.timing( 'html2wt.serialize.partial', '', startTimers.get( 'html2wt.serialize.partial' ) );
				this.timer.count( 'html2wt.serialize.partial.count', '' );
			}
		} else {
