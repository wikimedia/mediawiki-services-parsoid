	...
	if (options.tunnelFosteredContent && WikitextConstants.HTML.FosterablePosition.has(node.nodeName)) {
		...
		while (child) {
			...
			if (DOMUtils.isText(child)) {
				tunneledContent.push({ idx: idx, text: true, content: child.data });
				node.replaceChild(ownerDoc.createComment(''), child);
			} else if (DOMUtils.isElt(child) && allowedTags.indexOf(child.nodeName) === -1) {
				tunneledContent.push({ idx: idx, content: XMLSerializer.serialize(child).html });
				node.replaceChild(ownerDoc.createComment(''), child);
			}
			child = next;
			idx++;
		...
		if (tunneledContent.length > 0) {
			node.setAttribute('data-fostered', JSON.stringify(tunneledContent));
		}
	}
