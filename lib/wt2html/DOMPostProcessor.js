	...
	processors = processors.concat([
		{
			...
			handlers: [
				...
				{
					nodeName: 'td',
					action: (node, env) => tableFixer.stripDoubleTDs(node, env),
				},
				{
					nodeName: 'td',
					action: (node, env) => tableFixer.handleTableCellTemplates(node, env),
				},
				{
					nodeName: 'th',
					action: (node, env) => tableFixer.handleTableCellTemplates(node, env),
...
function processDumpAndGentestFlags(atTopLevel, psd, body, shortcut, opts, preOrPost) {
	...
	} else if (atTopLevel && psd.generateFlags && psd.generateFlags.handlers) {
		opts.quiet = true;
...
DOMPostProcessor.prototype.doPostProcess = Promise.async(function *(document) {
	...
	var haveDumpFlags = psd.dumpFlags || psd.generateFlags;
	...
	if (this.atTopLevel && psd.generateFlags && psd.generateFlags.handlers) {
		...
		DOMDataUtils.getDataParsoid(document.body).dsr = [0, env.page.src.length, 0, 0];
	}

	const pipelineConfig = this.env.conf.parsoid.pipelineConfig;
	const phpDOMTransformers = pipelineConfig && pipelineConfig.wt2html && pipelineConfig.wt2html.DOM || null;
	for (var i = 0; i < this.processors.length; i++) {
		...
		try {
			...
			if (haveDumpFlags) {
				...
				processDumpAndGentestFlags(this.atTopLevel, psd, document.body, pp.shortcut, opts, 'pre');
			}
			...
			if (haveDumpFlags) {
				processDumpAndGentestFlags(this.atTopLevel, psd, document.body, pp.shortcut, opts, 'post');
			}
			if (tracePP) {
