...
function DOMPostProcessor(env, options) {
	...
	dataParsoidLoader.addHandler(null, (...args) => PrepareDOM.prepareDOM(this.seenDataIds, ...args));
	...
	const fixupsVisitor = new DOMTraverser();
	...
	const headingsVisitor = new DOMTraverser();
	headingsVisitor.addHandler(null, Headings.genAnchors);
	addPP('heading gen anchor', 'headings',
		(node, env, opts, atTopLevel) => headingsVisitor.traverse(node, env, opts, atTopLevel),
		true
	);
	...
	const headingVisitor = new DOMTraverser();
	...
	const cleanupVisitor = new DOMTraverser();
...
DOMPostProcessor.prototype.doPostProcess = Promise.async(function *(document) {
	...
	const phpDOMTransformers = pipelineConfig && pipelineConfig.phpDOMTransformers || null;
	for (var i = 0; i < this.processors.length; i++) {
		...
		try {
			...
			if (phpDOMTransformers && phpDOMTransformers.indexOf(pp.name) >= 0) {
