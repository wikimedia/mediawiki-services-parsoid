...
function DOMPostProcessor(env, options) {
	...
	addPP('processTreeBuilderFixups', processTreeBuilderFixups);
	addPP('normalize', (body) => { body.normalize(); });
	addPP('pWrap', pWrap, true);
	...
	env.conf.wiki.extConfig.domProcessors.forEach(function(extProcs) {
		addPP(extProcs.extName, extProcs.procs.wt2htmlPostProcessor);
	});
	...
	// Language conversion
	addPP('LanguageConverter', (rootNode, env, options) => {
	...
	addPP('stripMarkerMetas', domVisitor.traverse.bind(domVisitor));
	...
	addPP('handleEmptyElts+cleanupAndSaveDP', domVisitor.traverse.bind(domVisitor));
	...
	// (Optional) downgrade
	addPP('downgrade2to1', function(rootNode, env, options) {
		if (semver.lt(env.contentVersion, '2.0.0')) {
	...
	for (var i = 0; i < this.processors.length; i++) {
		...
		try {
			...
			if (tracePP) {
				...
				env.bumpTimeUse(resourceCategory, (ppEnd - ppStart), 'DOM');
			}
		} catch (e) {
			env.log('fatal', e);
			return;
