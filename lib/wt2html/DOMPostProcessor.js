...
function DOMPostProcessor(env, options) {
	...
	// 4. Deduplicate template styles (should be post-dom-fragment expansion)
	domVisitor.addHandler('style', dedupeStyles.dedupe);
...
function generateTest(psd, body, shortcut, opts, preOrPost) {
	if (psd.generateFlags.handlers) {
		psd.generateFlags.handlers.forEach(function(handler) {
			if (handler === ('dom:' + shortcut)) {
				...
				DU.dumpDOM(body, 'DOM: ' + preOrPost + '-' + shortcut, opts);
			}
		});
	}
}
	...
	for (var i = 0; i < this.processors.length; i++) {
		...
		try {
			...
			if (this.atTopLevel) {
				...
				} else if (psd.generateFlags) {
					generateTest(psd, body, pp.shortcut, opts, 'pre');
				}
			}
			...
			if (this.atTopLevel) {
				if (psd.dumpFlags && psd.dumpFlags.has('dom:post-' + pp.shortcut)) {
					...
					generateTest(psd, body, pp.shortcut, opts, 'post');
				}
			}
			if (tracePP) {
