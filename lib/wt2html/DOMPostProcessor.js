...
function DOMPostProcessor(env, options) {
	...
	dataParsoidLoader.addHandler(null, (node, env) => this.prepareDOM(node, env));
	...
	headingVisitor.addHandler(null, (node, env) => {
	...
	markerMetasVisitor.addHandler('meta',
		(node, env) => CleanUp.stripMarkerMetas(env.conf.parsoid.rtTestMode, node, env)
	);
