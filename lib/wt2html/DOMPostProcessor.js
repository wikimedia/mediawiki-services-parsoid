...
function DOMPostProcessor(env, options) {
	...
	env.conf.wiki.nativeExtPostProcessors.forEach(function(extPP) {
...
DOMPostProcessor.prototype.doPostProcess = function(document) {
	...
	for (var i = 0; i < this.processors.length; i++) {
		...
		try {
			...
			if (tracePP) {
				...
				ppStart = new Date();
				env.log("trace/time/dompp", prefix + "; " + ppName + " start");
			}
			pp.proc(document.body, env, this.options, this.atTopLevel);
			if (tracePP) {
				ppEnd = new Date();
				env.log("trace/time/dompp", prefix + "; " + ppName + " end; time = " + (ppEnd - ppStart));
			}
		} catch (e) {
			env.log('fatal', e);
			return;
	...
	if (tracePP) {
		endTime = new Date();
		env.log("trace/time/dompp", prefix + "; end=" + endTime + "; time = " + (endTime - startTime));
	}
