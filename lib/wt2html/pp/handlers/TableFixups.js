...
class TableFixups {
	...
	stripDoubleTDs(node, env) {
		...
		if (!WTUtils.isLiteralHTMLNode(node) &&
			...
			var nodeSrc = WTUtils.getWTSource(env.topFrame, node);
			if (!dataMW.parts) {
				dataMW.parts = [];
			}
	...
	hoistTransclusionInfo(env, child, tdNode) {
		...
		if (dp.dsr[0] < childDP.dsr[0]) {
			parts.unshift(env.topFrame.srcText.substring(dp.dsr[0], childDP.dsr[0]));
		}
		...
		if (childDP.dsr[1] < dp.dsr[1]) {
			parts.push(env.topFrame.srcText.substring(childDP.dsr[1], dp.dsr[1]));
		}
	 ...
	 */
	reparseTemplatedAttributes(env, node, templateWrapper) {
	...
	handleTableCellTemplates(node, env) {
