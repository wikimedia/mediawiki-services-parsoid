...
function split(n) {
	if (!DU.isElt(n)) {
		// text/comments: p-wrappable
		return [ { pwrap: true, node: n } ];
	} else if (!isSplittableTag(n)) {
	...
	while (c) {
		...
		if (DU.isBlockNode(c)) {
			...
			split(c).forEach(function(v) {
				...
				} else if (DU.isComment(n) || (DU.isText(n) && /^\s*$/.test(n.nodeValue))) {
					if (p) {
						p.appendChild(n);
					} else {
						root.insertBefore(n, next);
...
function pWrapDOM(doc) {
	pWrap(doc.body);
	pWrapInsideTag(doc.body, 'BLOCKQUOTE');
}

if (typeof module === "object") {
	module.exports.pWrapDOM = pWrapDOM;
}
