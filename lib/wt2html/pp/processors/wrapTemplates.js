...
function recordTemplateInfo(env, compoundTpls, compoundTplId, tpl, argInfo) {
...
function findTopLevelNonOverlappingRanges(document, env, docRoot, tplRanges) {
	...
	for (var l = 0; l < numRanges; l++) {
		...
		// Extract argInfo
		var tmp = DU.getDataParsoid(r.startElem).tmp;
		...
		if (enclosingRangeId) {
			...
			if (argInfo) {
				...
				recordTemplateInfo(env, compoundTpls, enclosingRangeId, r, argInfo);
			}
			...
			if (argInfo) {
				recordTemplateInfo(env, compoundTpls, prev.id, r, argInfo);
			}
		} else {
			...
			// Update compoundTpls
			if (argInfo) {
				recordTemplateInfo(env, compoundTpls, r.id, r, argInfo);
			}
		}
		...
		if (encapInfo.valid) {
			...
			tplArray = tplArray.map(function(a) {
				if (a.wt) {
					...
					infoIndex++;
					var type = startElem.getAttribute('typeof');
					return /(^|\s)mw:Param(\s|$)/.test(type)
						? { templatearg: a.args }
						: { template: a.args };
				}
			});
			...
			var origDmw = DU.getDataMw(encapInfo.target);

			// This is to let us handle {{1x|<ref>foo</ref>}} where
			// the Cite extension needs the data-mw from the <ref> tag.
			if (Object.keys(origDmw).length > 0) {
				encapInfo.dp.nestedDmw = origDmw;
			}
 ...
 * @private
 */
function findWrappableTemplateRanges(doc, env, rootNode, tpls) {
	var tplRanges = [];
...
function wrapTemplates(body, env, options) {
	...
	if (psd.dumpFlags && psd.dumpFlags.has("dom:pre-encap")) {
		DU.dumpDOM(body, 'DOM: pre-encapsulation', { env: env, dumpFragmentMap: true });
	}

	wrapTemplatesInTree(body.ownerDocument, env, body);

	if (psd.dumpFlags && psd.dumpFlags.has("dom:post-encap")) {
		DU.dumpDOM(body, 'DOM: post-encapsulation', { env: env, dumpFragmentMap: true });
	}
}
