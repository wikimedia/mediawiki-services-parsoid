...
class WrapTemplates {
	 ...
	 * @private
	 */
	getDOMRange(env, doc, startElem, endMeta, endElem) {
		var range = {
			startElem: startElem,
			endElem: endMeta,
	...
	recordTemplateInfo(env, compoundTpls, compoundTplId, tpl, argInfo) {
		...
		if (tplArray.length > 0) {
			...
			if (prevTplInfo.dsr[1] < dsr[0]) {
				tplArray.push({ wt: env.topFrame.srcText.substring(prevTplInfo.dsr[1], dsr[0]) });
			}
		}
	...
	findTopLevelNonOverlappingRanges(document, env, docRoot, tplRanges) {
		var n, r, ranges;
	...
	encapsulateTemplates(doc, env, tplRanges, tplArrays) {
		var numRanges = tplRanges.length;
		for (var i = 0; i < numRanges; i++) {
			...
			if (encapInfo.valid) {
				...
				if (firstTplInfo.dsr[0] > dp1.dsr[0]) {
					...
					tplArray = [{ wt: env.topFrame.srcText.substring(dp1.dsr[0], firstTplInfo.dsr[0]) }].concat(tplArray);
				}

				// Add any trailing wikitext
				var lastTplInfo = lastItem(tplArray);
				if (lastTplInfo.dsr[1] < dp1.dsr[1]) {
					tplArray.push({ wt: env.topFrame.srcText.substring(lastTplInfo.dsr[1], dp1.dsr[1]) });
				}
			...
			if (encapInfo.valid) {
				...
				encapInfo.dp.src = env.topFrame.srcText.substring(encapInfo.dp.dsr[0], encapInfo.dp.dsr[1]);
			}
	 ...
	 * @private
	 */
	findWrappableTemplateRanges(doc, env, rootNode, tpls) {
		var tplRanges = [];
		...
		while (elem) {
			...
			if (DOMUtils.isElt(elem)) {
				...
				var metaMatch = type ? type.match(Util.TPL_META_TYPE_REGEXP) : null;
				...
				if (metaMatch && (DOMDataUtils.getDataParsoid(elem).tsr || /\/End(?=$|\s)/.test(type))) {
					...
					if (!/\/End(?=$|\s)/.test(metaType)) {
						if (aboutRef) {
							...
							if (aboutRef.end) {
								...
								tplRanges.push(this.getDOMRange(env, doc, elem, aboutRef.end, aboutRef.end));
							} else {
								// should not happen!
						...
						if (aboutRef) {
							...
							tplRanges.push(this.getDOMRange(env, doc, sm, em, ee));
						} else {
							tpls[about] = { end: elem };
						}
					}
	...
	wrapTemplatesInTree(document, env, node) {
		var tplRanges = this.findWrappableTemplateRanges(document, env, node, {});
