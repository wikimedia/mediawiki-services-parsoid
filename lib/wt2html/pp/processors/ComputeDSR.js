...
class ComputeDSR {
	tsrSpansTagDOM(n, parsoidData) {
		...
		return !(
			...
			/\bmw:(Placeholder|LanguageVariant|DisplaySpace)\b/.test(n.getAttribute('typeof')) ||
	...
	computeTagWidths(widths, node, dp) {
		if (dp.extTagOffsets) {
			return [dp.extTagOffsets[1] - dp.extTagOffsets[0],
	 ...
	 * @param {Object} opts
	 */
	computeNodeDSR(env, node, s, e, dsrCorrection, opts) {
		function trace() {
			var args = arguments;
			...
			if (cType === node.TEXT_NODE) {
				...
				if (cs !== null || ce !== null) {
					...
					env.log("trace/dsr", function() {
						...
						dp.dbsrc = env.topFrame.srcText.substring(cs, ce);
						return str;
					});
				}
					...
					while (newCE !== null && sibling && !WTUtils.isTplStartMarkerMeta(sibling)) {
						...
						if (nType === node.TEXT_NODE) {
							...
							env.log("trace/dsr", function() {
								...
								// debug info
								if (siblingDP.dsr[1]) {
									siblingDP.dbsrc = env.topFrame.srcText.substring(newCE, siblingDP.dsr[1]);
								}
								return str;
							});
	...
	run(rootNode, env, options) {
		...
		var endOffset = options.sourceOffsets ? options.sourceOffsets[1] : env.topFrame.srcText.length;
