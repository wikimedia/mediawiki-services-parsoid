...
getNextMatchingNode = function(node, match) {
	...
	if (node.nextSibling) {
		return leftMostDescendent(DOMUtils.nextNonSepSibling(node), match);
	}
...
function logTreeBuilderFixup(env, c, dp, tplInfo) {
	...
	if (!Util.isVoidElement(cNodeName) &&
		...
		if (dp.autoInsertedEnd === true && (tplInfo || dsr[2] > 0)) {
			...
			} else if (!WTUtils.hasLiteralHTMLMarker(dp) && DOMUtils.isQuoteElt(c) && (ancestor = getHeadingAncestor(c.parentNode))) {
...
function logFosteredContent(env, node, dp, tplInfo) {
	...
	var templateInfo = findEnclosingTemplateName(env, tplInfo);
	var lintObj = {
		dsr: findLintDSR(templateInfo, tplInfo, DOMDataUtils.getDataParsoid(maybeTable).dsr),
		templateInfo: templateInfo
	};
...
function logBadPWrapping(env, node, dp, tplInfo) {
	if (!DOMUtils.isBlockNode(node) && DOMUtils.isBlockNode(node.parentNode) && hasNoWrapCSS(node)) {
...
function logTidyWhitespaceBug(env, node, dp, tplInfo) {
	...
	while (node && !DOMUtils.isBlockNode(node) && !/^(H|B|WB)R$/.test(node.nodeName)) {
		if (DOMUtils.isText(node) || !hasNoWrapCSS(node)) {
	...
	nowrapNodes.forEach(function(o, i) {
		if (o.tidybug && i < n && !nowrapNodes[i + 1].hasLeadingWS) {
			var lintObj = {
				dsr: findLintDSR(templateInfo, tplInfo, DOMDataUtils.getDataParsoid(o.node).dsr),
				templateInfo: templateInfo,
				params: {
...
function logPHPParserBug(env, node, dp, tplInfo) {
	...
	var templateInfo = findEnclosingTemplateName(env, tplInfo);
	var lintObj = {
		dsr: findLintDSR(templateInfo, tplInfo, DOMDataUtils.getDataParsoid(node).dsr),
		templateInfo: templateInfo,
		params: {
			name: 'table',
...
/**
 */
function linter(body, env, options) {
