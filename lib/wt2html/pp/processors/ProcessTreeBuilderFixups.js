...
class ProcessTreeBuilderFixups {
	...
	addPlaceholderMeta(env, node, dp, name, opts) {
		...
		if (!src) {
			if (dp.tsr) {
				src = env.topFrame.srcText.substring(dp.tsr[0], dp.tsr[1]);
			} else if (opts.tsr) {
				src = env.topFrame.srcText.substring(opts.tsr[0], opts.tsr[1]);
						...
						if ((prevSibling && prevSibling.nodeName.toLowerCase() !== expectedName) ||
							...
							if (c && dp.stx !== 'html'
								...
								if (dp.tsr
									...
									origTxt = env.topFrame.srcText.substring(dp.tsr[0], dp.tsr[1]);
	...
	// is indeed empty.
	removeAutoInsertedEmptyTags(env, node) {
		var c = node.firstChild;
		while (c !== null) {
