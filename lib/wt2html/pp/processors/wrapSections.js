	...
	state.templatesToExamine.forEach(function(tplInfo) {
		...
		if (ancestor === s1) {
			...
			if (tplInfo.last.nextSibling) {
				// Append the content of the section after the last node to data-mw.parts
				var newTplEndOffset = getDSR(s2, false); // will succeed because it traverses non-tpl content
			...
			var newS2 = s2Ancestors[i - 1]; // i >= 1 since we know s2 is not s1's ancestor
			...
			var dsr1 = getDSR(newS1, true); // will succeed because it traverses non-tpl content
			var dsr2 = getDSR(newS2, false); // will succeed because it traverses non-tpl content
...
function wrapSections(rootNode, env, options, atTopLevel) {
	if (!atTopLevel || (!env.conf.parsoid.wrapSections && !env.wrapSections)) {
		return;
	}
