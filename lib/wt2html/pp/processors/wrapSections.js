		...
		if (!s1 || ancestor === s1) {
			...
			if (tplInfo.last.nextSibling) {
				...
				if (/mw:Transclusion/.test(tplInfo.first.getAttribute('typeof'))) {
					...
					dmw.body.extsrc += state.getSrc(tplEndOffset, newTplEndOffset);
				}
				// Update DSR
			...
			if (/mw:Transclusion/.test(tplInfo.first.getAttribute('typeof'))) {
				...
				dmw.body.extsrc = state.getSrc(dsr1, tplDsr[0]) + dmw.body.extsrc + state.getSrc(tplDsr[1], dsr2);
				DU.setDataMw(newS1, dmw);
...
function wrapSections(rootNode, env, options, atTopLevel) {
	...
	if (env.conf.parsoid.dumpFlags && (env.conf.parsoid.dumpFlags.indexOf("dom:pre-sections") !== -1)) {
