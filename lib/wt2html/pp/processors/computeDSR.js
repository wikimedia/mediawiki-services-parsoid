...
function computeNodeDSR(env, node, s, e, dsrCorrection, opts) {
	...
	function traceNode(n, i, cs, ce) {
		env.log("trace/dsr", function() {
			return "     CHILD: <" + n.parentNode.nodeName + ":" + i +
				">=" +
				(DU.isElt(n) ? '' : (DU.isText(n) ? '#' : '!')) +
				(DU.isElt(n) ?
					(n.nodeName === 'META' ? n.outerHTML : n.nodeName) :
	...
	for (var i = numChildren - 1; i >= 0; i--) {
		...
		if (cType === node.TEXT_NODE) {
			...
			if (cs !== null || ce !== null) {
				if (ce < 0) {
					if (!fosteredNode) {
						env.log("warning/dsr/negative", "Negative DSR for node: " + node.nodeName + "; resetting to zero");
					}
					ce = 0;
				}
			...
			if (ce !== null &&
				...
				while (newCE !== null && sibling && !DU.isTplStartMarkerMeta(sibling)) {
					...
					if (nType === node.TEXT_NODE) {
						...
						newCE = newCE + DU.decodedCommentLength(sibling);
	...
	if (s !== null && s !== undefined && cs !== s && !acceptableInconsistency(opts, node, cs, s)) {
		env.log("warning/dsr/inconsistent", "DSR inconsistency: cs/s mismatch for node:",
