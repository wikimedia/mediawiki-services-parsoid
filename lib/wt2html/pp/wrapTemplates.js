...
function getDOMRange(env, doc, startElem, endMeta, endElem) {
	...
	if (DU.isFosterablePosition(tcStart) && (
		...
		(DU.isTplMarkerMeta(tcStart) && DU.isTplMarkerMeta(tcStart.nextSibling)) ||
		(DU.isTplMarkerMeta(tcStart) && !DU.isElt(tcStart.nextSibling)))) {
		...
		while (n !== docRoot) {
			...
			if (ranges) {
				if (n !== r.start) {
					...
					var sTpls = ranges;
					var eTpls = DU.getNodeData(r.end).tmp_tplRanges;
					...
					for (var j = 0; j < sKeys.length; j++) {
						...
						if (other !== r.id
							...
							if (!nestedRangesMap[r.id]
								|| sTpls[other].startOffset < sTpls[nestedRangesMap[r.id]].startOffset) {
			...
			if (metaMatch && (DU.getDataParsoid(elem).tsr || /\/End(?=$|\s)/.test(type))) {
				...
				if (!/\/End(?=$|\s)/.test(metaType)) {
					...
					if (tableNode) {
						...
						// include the table.
						// TODO: implement
						env.log("warning/template", 'foster-parented content following!');
