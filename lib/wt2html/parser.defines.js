...
var TokenUtils; // TokenUtils module var for circular dependency avoidance
var requireTokenUtils = function() {
	if (!TokenUtils) {
		TokenUtils = require('../utils/TokenUtils.js').TokenUtils; // (circular dep)
	}
...
class Token {
	...
	getAttribute(name) {
		requireTokenUtils();
		return TokenUtils.lookup(this.attribs, name);
	}
	...
	addSpaceSeparatedAttribute(name, value) {
		requireTokenUtils();
		var curVal = TokenUtils.lookupKV(this.attribs, name);
		var vals;
		if (curVal !== null) {
