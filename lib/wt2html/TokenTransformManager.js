...
AsyncTokenTransformManager.prototype.transformTokens = function(tokens, parentCB) {
	// Trivial case
	if (tokens.length === 0) {
		return { tokens: tokens, asyncAccum: null }; // jscs:ignore jsDoc
	}
	...
	if (traceTime) {
		this.env.bumpTimeUse("AsyncTTM (Partial)", (new Date() - startTime - tokenTimes));
	}
...
SyncTokenTransformManager.prototype.onChunk = function(tokens) {
	...
	if (traceTime) {
		this.env.bumpTimeUse("SyncTTM", (new Date() - startTime - tokenTimes));
	}
...
Frame.prototype.expand = function(chunk, options) {
	...
	if (outType === 'tokens/x-mediawiki/expanded') {
		...
		if (lastItem(chunk).constructor === EOFTk) {
			...
			var newChunk = JSUtils.pushArray(chunk, this._eofTkList);
