...
const { KV, EOFTk, Params } = require('./parser.defines.js');
const fs = require('fs');
...
function verifyTokensIntegrity(env, ret) {
	...
	console.assert(ret.token === undefined);
	console.assert(ret.tokens === undefined || Array.isArray(ret.tokens));
	return ret;
}
...
class TokenTransformManager extends events.EventEmitter {
	...
	addTransform(transformation, debugName, rank, type, name) {
		...
		if (type === 'any') {
			...
			var key = TokenTransformManager.tokenTransformersKey(type, name);
	...
	_getTransforms(token, minRank) {
		...
		var key = TokenTransformManager.tokenTransformersKey(tkType, token.name);
...
class AsyncTokenTransformManager extends TokenTransformManager {
	...
	checkForEOFTkErrors(tokens, atEnd) {
		if (this.frame.depth === 0 && tokens && tokens.length) {
			var last = atEnd && JSUtils.lastItem(tokens);
	...
	transformTokens(tokens, parentCB) {
		...
		while (workStack.length > 0) {
			var curChunk = JSUtils.lastItem(workStack);
...
class SyncTokenTransformManager extends TokenTransformManager {
	...
	generateTest(transformer, token, res) {
		var generateFlags = this.env.conf.parsoid.generateFlags;
		var handlerName = generateFlags.handler;
		if (handlerName === transformer.name.slice(0, handlerName.length)) {
			var streamHandle = generateFlags.streamHandle;
			...
			var resultString = (this.pipelineId + "-gen/" + handlerName + " ".repeat(20)).slice(0, 27);
			var inputToken = resultString + " | IN  | " + JSON.stringify(token) + "\n";
			streamHandle.write(inputToken);
			if (res.tokens) {
				var outputTokens = resultString + " | OUT | " + JSON.stringify(res.tokens) + "\n";
	...
	onChunk(tokens) {
		...
		while (workStack.length > 0) {
			var curChunk = JSUtils.lastItem(workStack);
			var minRank = curChunk.rank || this.phaseEndRank - 1;
			var token = curChunk[curChunk.eltIndex++];
			if (curChunk.eltIndex === curChunk.length) {
				// remove processed chunk
				workStack.pop();
			...
			while (j < numTransforms && !this.pipelineModified) {
				...
				if (resT !== token) {
					modified = true;
					if (this.env.conf.parsoid.generateFlags) {
			...
			if ((!modified) && (!this.pipelineModified)) {
				console.assert(res.tokens[0] === token);
...
class TokenAccumulator {
	...
	concatTokens(tokens) {
		...
		var lastChunk = JSUtils.lastItem(this.siblingChunks);
		if (!tokens.rank) {
	...
	receiveToksFromChild(ret) {
		...
		var async = ret.async || this.waitForSibling || (this.siblingChunks.length > 0);
