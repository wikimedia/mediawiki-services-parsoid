		...
		if (Array.isArray(v)) {
			...
			if (n === 0 || (n === 1 && v[0].constructor === String)) {
				...
				this.frame.expand(v, {
					...
					type: this._toType,
				}).then(function(i, tokens) {
					this._returnAttributeValue(i, tokens);
				}.bind(this, i)).done();
			}
		} else {
		...
		if (Array.isArray(k)) {
			...
			if (n === 0 || (n === 1 && k[0].constructor === String)) {
				...
				this.frame.expand(k, {
					...
					this._returnAttributeKey(i, tokens);
				}.bind(this, i)).done();
			}
		} else {
			kv.k = k;
...
AttributeTransformManager.prototype._returnAttributeValue = function(ref, tokens) {
	this.manager.env.log('debug', 'check _returnAttributeValue: ', ref,  tokens);
...
AttributeTransformManager.prototype._returnAttributeKey = function(ref, tokens) {
	this.manager.env.log('debug', 'check _returnAttributeKey: ', ref,  tokens);
