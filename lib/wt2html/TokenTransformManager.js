...
const { NlTk, KV, EOFTk } = require('../tokens/TokenTypes.js');
...
class SyncTokenTransformManager extends TokenTransformManager {
	...
	onChunk(tokens) {
		...
		const env = this.env;
		const genFlags = env.conf.parsoid.generateFlags;
		...
		this.transformers.forEach(function(transformer, i) {
			...
			while (tokens.length > 0) {
				...
				if (traceFlags) {
					env.log(
						self.traceType, self.pipelineId, transformer.constructor.name,
				...
				if (traceTime) {
					...
					if (token.constructor === NlTk) {
						...
						traceName = self.traceNames[i][0];
					} else if (token.constructor === EOFTk) {
						res = transformer.onEnd(token);
						traceName = self.traceNames[i][1];
					} else {
						res = transformer.onTag(token);
						traceName = self.traceNames[i][2];
					}
					const t = JSUtils.elapsedTime(s);
				...
				if (!modified && !res.skip && transformer.active) {
					if (traceTime) {
						const s = JSUtils.startTime();
						const traceName = self.traceNames[i][3];
