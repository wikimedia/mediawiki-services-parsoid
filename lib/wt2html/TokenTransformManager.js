...
function generateTest(context, transformer, token, res) {
	var generateFlags = context.env.conf.parsoid.generateFlags;
	...
	if (handlerName === transformer.name.slice(0, handlerName.length)) {
		...
		var resultString = (context.pipelineId + "-gen/" + handlerName + " ".repeat(20)).slice(0, 27);
		var inputToken = resultString + " | IN  | " + JSON.stringify(token) + "\n";
		streamHandle.write(inputToken);
		if (res.token && res.token !== token) {
			var outputToken = resultString + " | OUT | " + JSON.stringify(res.token) + "\n";
			streamHandle.write(outputToken);
		} else if (res.tokens) {
	...
	while (workStack.length > 0) {
		...
		while (j < numTransforms && !this.pipelineModified) {
			...
			if (this.env.conf.parsoid.generateFlags) {
				generateTest(this, transformer, token, res);
			}
			var resT = res.token || (res.tokens && !res.tokens.rank && res.tokens.length === 1 && res.tokens[0]);
			if (resT !== token) {
				break;
			}
			j++;
