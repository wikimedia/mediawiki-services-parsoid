...
AccumChain.prototype = {
	...
	init: function(parentCB) {
		...
		localAccum.append = function(chunk) {
			// All tokens in firstAccum are fully processed
			self.firstAccum.push.apply(self.firstAccum, chunk);
		};
 ...
 *
 * @return {Promise} resolved with expanded attribute array of KV when
 *   expansion is complete
 */
AttributeTransformManager.prototype.process = Promise.async(function *(attributes) {
	...
	this.kvs.length = attributes.length; // Allocate result array
	yield Promise.all(attributes.map(Promise.async(function *(cur, i) {
		var k = cur.k;
		var v = cur.v;
		var tokens;
		var n;
		...
		var kv = this.kvs[i] = new KV([], [], cur.srcOffsets);
		...
		if (Array.isArray(v)) {
			...
			if (n === 0 || (n === 1 && v[0].constructor === String)) {
				...
				// transform the value
				tokens = yield this.frame.expand(v, {
					wrapTemplates: this.options.wrapTemplates,
					inTemplate: this.options.inTemplate,
					type: this._toType,
				});
				this.manager.env.log(
					'debug', 'check _returnAttributeValue: ', i,  tokens
				);
				kv.v = Util.stripEOFTkfromTokens(tokens);
			}
		} else {
		...
		if (Array.isArray(k)) {
			...
			if (n === 0 || (n === 1 && k[0].constructor === String)) {
				...
				// transform the key
				tokens = yield this.frame.expand(k, {
					wrapTemplates: this.options.wrapTemplates,
					inTemplate: this.options.inTemplate,
					type: this._toType,
				});
				this.manager.env.log(
					'debug', 'check _returnAttributeKey: ', i,  tokens
				);
				kv.k = Util.stripEOFTkfromTokens(tokens);
			}
		} else {
			kv.k = k;
		}
	...
	if (this.callback) { this.callback(this.kvs); }
	return this.kvs;
});
