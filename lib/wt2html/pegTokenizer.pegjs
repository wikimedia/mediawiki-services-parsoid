...
opt_lang_variant_flags
  = f:( space_or_newline* ff:lang_variant_flags space_or_newline*
        "|" { return ff; } )? {
    ...
    if (useVariants) {
      ...
      return { flags: flags, original: flagList };
    }
  }
...
lang_variant_option_list
  = o:lang_variant_option rest:( ";" oo:lang_variant_option { return oo; })*
    ...
    {
      var r = [ o ].concat(rest);
      if (tr) { r.push({ semi: true }); }
      return r;
    }
  / lvtext:lang_variant_text { return [{ text: lvtext }]; }

lang_variant_option
  = space_or_newline* lang:lang_variant_name space_or_newline* ":"
    lvtext:(lang_variant_nowiki / lang_variant_text_no_semi)
    {
      return { lang: lang, text: lvtext, bidir: true };
    }
  / from:(lang_variant_nowiki / lang_variant_text_no_semi_or_arrow)
    ...
    {
      return { from: from, lang: lang, to: to };
    }
