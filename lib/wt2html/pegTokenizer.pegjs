 ...
 */
{
    ...
    var isExtTag = function(name) {
        var lName = name.toLowerCase();
        return env.conf.wiki.extConfig.tags.has(lName);
    };
...
tplarg_or_template
  = &'{{'
    ...
    &{
      ...
      if (templatedepth + 1 < env.conf.parsoid.maxDepth) {
        return true;
      } else {
        return false;
      }
...
lang_variant_text_no_semi
  = lvtext:lang_variant_text<semicolon>
    { return lvtext; }

lang_variant_text_no_semi_or_arrow
  = lvtext:lang_variant_text_no_semi<arrow> { return lvtext; }
...
xmlish_tag
  = "<" tag:(xmlish_tag_opened<extTag> / xmlish_tag_opened<extTag=false>)
    { return tag; }
 ...
 */
block_tag
  = "<" tag:(block_tag_opened<extTag> / block_tag_opened<extTag=false>)
    { return tag; }
...
block_tag_opened
  = end: "/"?
    ...
    ">" {
      var tsr = tsrOffsets();
      tsr[0]--; // For "<" matched at the start of block_tag rule
      var t = tu.buildXMLTag(name, name.toLowerCase(), attribs, end, !!selfclose, tsr);
...
row_syntax_table_args
  = as:table_attributes<tableCellArg> s:space* p:pipe !pipe {
        return [as, s, p];
    }
