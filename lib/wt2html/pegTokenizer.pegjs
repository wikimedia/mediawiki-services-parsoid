...
tplarg_or_template
  = &'{{' &{
      ...
      if (stops.onStack('templatedepth') === false ||
          stops.onStack('templatedepth') < env.conf.parsoid.maxDepth) {
        return true;
      } else {
        return false;
      }
...
tplarg_or_template_guarded
  = &{
      const templatedepth = stops.onStack('templatedepth');
      return stops.push('templatedepth', templatedepth === false ? 1 : templatedepth + 1);
    }
    r:( &('{{' &('{{{'+ !'{') tplarg) a:(template/broken_template) { return a; }
      ...
      stops.pop('templatedepth');
      return r;
    }
    / & { return stops.pop('templatedepth'); }

tplarg_or_template_or_bust "tplarg_or_template_or_bust"
    = r:(tplarg_or_template / .)+ { return tu.flattenIfArray(r); }

template
  = stopLen:("" { return stops.push('preproc', tu.PREPROC_BRACE_BRACE); })
...
broken_template
  = &"{{" &{ return stops.push('preproc', tu.PREPROC_BROKEN); }
...
lang_variant
  = stopLen:("" { return stops.push('preproc', tu.PREPROC_BRACE_DASH); })
...
wikilink
  = stopLen:("" { return stops.push('preproc', tu.PREPROC_BRACKET_BRACKET); })
...
inlineline_break_on_colon
  = & {return stops.push('colon', true);}
    ill:inlineline
    & { stops.pop('colon'); return true;}
    { return ill; }
    / & { return stops.pop('colon'); }
...
table_start_tag "table_start_tag"
  = sc:(space / comment)*
    ...
    ta:(table_attributes / &{ assert(false); return false; })
...
table_row_tag
  = // avoid recursion via nested_block_in_table
    ...
    p:pipe dashes:$"-"+
    & { return stops.push('table', false); }
    a:(table_attributes / &{ assert(false); return false; })
