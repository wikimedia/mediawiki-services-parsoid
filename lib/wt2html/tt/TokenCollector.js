...
class TokenCollector extends TokenHandler {
	constructor(manager, options) {
		...
		this.manager.addTransform(
			(token, manager, cb) => this._onDelimiterToken(token, manager, cb),
	 ...
	 * @private
	 */
	_onDelimiterToken(token, frame, cb) {
		...
		if (tc === TagTk) {
			if (this.scopeStack.length === 0) {
				...
				this.manager.addTransform(
					(token, manager, cb) => this._onAnyToken(token, manager, cb),
