...
const { NlTk, TagTk, SelfclosingTagTk, EndTagTk } = require('../../tokens/TokenTypes.js');
			...
			if ([TagTk, SelfclosingTagTk].indexOf(t.constructor) !== -1) {
				...
				if (TokenUtils.isDOMFragmentType(t.getAttribute('typeof'))) {
					...
					if (t.constructor === TagTk) {
						...
						console.assert(tokens[i].constructor === EndTagTk);
					}
					continue;
				}
		...
		for (i = 0, l = oldAttrs.length; i < l; i++) {
			...
			if (expandedK) {
				...
				if (Array.isArray(oldA.k)) {
					if (!(expandedK.constructor === String && /(^|\s)mw:maybeContent(\s|$)/.test(expandedK))) {
						...
						if (expandedA.v === '' && !this.options.standalone) {
							...
							const kStr = TokenUtils.tokensToString(expandedK).trim();
				...
				if ((reparsedKV && (updatedK.hasGeneratedContent || metaTokens.length > 0)) ||
					...
					const key = expandedK.constructor === String ? expandedK : TokenUtils.tokensToString(expandedK);
					if (!tmpDataMW) {
						tmpDataMW = new Map();
					}
					tmpDataMW.set(key, {
