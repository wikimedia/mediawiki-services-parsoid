...
const { NlTk, TagTk, SelfclosingTagTk } = require('../parser.defines.js');
...
class AttributeExpander extends TokenHandler {
	...
	static splitTokens(env, token, nlTkPos, tokens, wrapTemplates) {
		const buf = [];
	...
	static stripMetaTags(env, tokens, wrapTemplates) {
		...
		for (let i = 0, l = tokens.length; i < l; i++) {
			...
			if ([TagTk, SelfclosingTagTk].indexOf(t.constructor) !== -1) {
				...
				if (Util.isDOMFragmentType(t.getAttribute('typeof'))) {
					const nodes = env.fragmentMap.get(t.dataAttribs.html);
					const str = nodes.reduce(function(prev, next) {
	...
	*buildExpandedAttrsG(token, expandedAttrs) {
		...
		let nlTkPos = -1;
		let i, l;
		const nlTkOkay = Util.isHTMLTag(token) || !Util.isTableTag(token);
			...
			if (expandedK) {
				...
				if (Array.isArray(oldA.k)) {
					if (!(expandedK.constructor === String && /(^|\s)mw:maybeContent(\s|$)/.test(expandedK))) {
						...
						if (nlTkPos !== -1) {
							...
							updatedK = AttributeExpander.splitTokens(env, token, nlTkPos, expandedK, wrapTemplates);
						...
						if (expandedA.v === '' && !this.options.standalone) {
							...
							const kStr = Util.tokensToString(expandedK).trim();
							const rule = nlTkOkay ? 'generic_newline_attributes' : 'table_attributes';
							const kvs  = /=/.test(kStr) ? this.tokenizer.tokenizeAs(kStr, rule) : new Error('null');
				...
				if (expandedK.constructor === String && Array.isArray(oldA.v)) {
					if (!expandedK.match(/^mw:/)) {
						...
						if (nlTkPos !== -1) {
							// Scenario 1 from the documentation comment above.
							updatedV = AttributeExpander.splitTokens(env, token, nlTkPos, attrValTokens, wrapTemplates);
				...
				if ((reparsedKV && (updatedK.hasGeneratedContent || metaTokens.length > 0)) ||
					...
					const key = expandedK.constructor === String ? expandedK : Util.tokensToString(expandedK);
					if (!tmpDataMW) {
						tmpDataMW = new Map();
					}
					tmpDataMW.set(key, {
		...
		if (!token.getAttribute('about') && tmpDataMW && tmpDataMW.size > 0) {
			...
			const eVals = yield Util.expandValuesToDOM(
