...
AttributeExpander.prototype.onToken = function(token, frame, cb) {
	...
	if ((token.constructor === TagTk || token.constructor === SelfclosingTagTk) &&
		...
		).process(token.attribs).then(
			expandedAttrs => this._returnAttributes(token, expandedAttrs)
		).then(
			t => cb(t)
		).done();
	} else {
 ...
 * @private
 */
AttributeExpander.prototype._returnAttributes = Promise.async(function *(token, expandedAttrs) {
	...
	if (modified) {
		...
		if (!token.getAttribute('about') && tmpDataMW && tmpDataMW.size > 0) {
			...
			var newToks = metaTokens.concat([token], postNLToks);
			if (metaTokens.length === 0) {
				// No more attribute expansion required for token after this
				newToks.rank = skipRank;
			}
			return { tokens: newToks };
		}
