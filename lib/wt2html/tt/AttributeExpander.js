...
AttributeExpander.prototype.onToken = function(token, frame, cb) {
	...
	if ((token.constructor === TagTk || token.constructor === SelfclosingTagTk) &&
		...
		(token.name !== 'meta' ||
		!/mw:(TSRMarker|Placeholder|Transclusion|Param|Includes|DOMFragment\/sealed\/ref)/.test(token.getAttribute('typeof')))
	) {
		var atm = new AttributeTransformManager(
			this.manager,
			{ wrapTemplates: this.options.wrapTemplates, inTemplate: this.options.inTemplate }
		);
...
function stripMetaTags(env, tokens, wrapTemplates) {
	...
	for (var i = 0, l = tokens.length; i < l; i++) {
		...
		if ([TagTk, SelfclosingTagTk].indexOf(t.constructor) !== -1) {
			...
			if (/^mw:DOMFragment(\/sealed\/\w+)?$/.test(t.getAttribute('typeof'))) {
