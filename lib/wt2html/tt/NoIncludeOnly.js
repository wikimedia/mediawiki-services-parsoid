...
class OnlyInclude extends TokenHandler {
	...
	constructor(manager, options) {
		...
		if (this.options.isInclude) {
			...
			this.manager.addTransform(
				(token, manager) => this.onAnyInclude(token, manager),
				'OnlyInclude:onAnyInclude', OnlyInclude.RANK(), 'any');
		} else {
			// Just convert onlyinclude tokens into meta tags with rt info.
			this.manager.addTransform(
				(token, manager) => this.onOnlyInclude(token, manager),
				'OnlyInclude:onOnlyInclude', OnlyInclude.RANK(), 'tag', 'onlyinclude');
		}
	}

	onOnlyInclude(token, manager) {
	...
	onAnyInclude(token, manager) {
		...
		curriedBuildMetaToken = function(isEnd, tsr, src) {
			return buildMetaToken(manager, tagName, isEnd, tsr, src);
		};
		...
		if (isTag && token.name === 'onlyinclude') {
			...
			// meta.rank = OnlyInclude.RANK();
			return { tokens: [ meta ] };
		} else {
			if (this.inOnlyInclude) {
				// token.rank = OnlyInclude.RANK();
				return { tokens: [ token ] };
			} else {
