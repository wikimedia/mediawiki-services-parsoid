...
class PreHandler extends TokenHandler {
	...
	// Handler ranks
	static NL_RANK() { return  2.051; }
	static ANY_RANK() { return  2.052; }
	static END_RANK() { return  2.053; }
	static SKIP_RANK() { return  2.054; } // should be higher than all other ranks above
	...
	constructor(manager, options) {
		...
		if (!this.options.inlineContext && !this.options.inPHPBlock) {
			this.manager.addTransform(
				(token, manager, cb) => this.onNewline(token, manager, cb),
				'PreHandler:onNewline', PreHandler.NL_RANK(), 'newline');
			this.manager.addTransform(
				(token, manager, cb) => this.onEnd(token, manager, cb),
				'PreHandler:onEnd', PreHandler.END_RANK(), 'end');
			this.reset(true);
		}
	}
	...
	reset(addAnyHandler) {
		...
		if (addAnyHandler) {
			this.manager.addTransform(
				(token, manager, cb) => this.onAny(token, manager, cb),
				'PreHandler:onAny', PreHandler.ANY_RANK(), 'any');
		}
	}

	moveToIgnoreState() {
		this.state = PreHandler.STATE_IGNORE();
		this.manager.removeTransform(PreHandler.ANY_RANK(), 'any');
	}
	...
	getResultAndReset(token) {
		...
		ret.rank = PreHandler.SKIP_RANK(); // prevent this from being processed again
		return ret;
	}
	...
	onAny(token, manager, cb) {
		var env = this.manager.env;

		env.log("trace/pre", this.manager.pipelineId, "any   |",
			PreHandler.STATE_STR()[this.state], "|", function() { return JSON.stringify(token); });
