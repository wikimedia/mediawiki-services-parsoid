...
PreHandler.prototype.reset = function(addAnyHandler) {
...
PreHandler.prototype.encounteredBlockWhileCollecting = function(token) {
	...
	if (this.tokens.length > 0) {
		...
		while (i > 0 && Util.isSolTransparent(env, this.tokens[i])) { i--; }
...
function getUpdatedPreTSR(tsr, token) {
	...
	if (tc === CommentTk) {
		...
		// (see DU.decodedCommentLength() -- but that takes a node not a token)
		tsr = token.dataAttribs.tsr ? token.dataAttribs.tsr[1] : (tsr === -1 ? -1 : DU.decodeComment(token.value).length + 7 + tsr);
	...
	if (tc === EOFTk) {
		...
		switch (this.state) {
			case PreHandler.STATE_SOL:
				...
				} else if (Util.isSolTransparent(env, token)) {
					// continue watching ...
			...
			case PreHandler.STATE_PRE:
				if (Util.isSolTransparent(env, token)) { // continue watching
					this.solTransparentTokens.push(token);
				} else if (Util.isTableTag(token) ||
					(Util.isHTMLTag(token) && Util.isBlockTag(token.name))) {
					ret = this.getResultAndReset(token);
					this.moveToIgnoreState();
				} else {
			...
			case PreHandler.STATE_MULTILINE_PRE:
				...
				} else if (Util.isSolTransparent(env, token)) { // continue watching
					this.solTransparentTokens.push(token);
				} else {
					ret = this.processPre(token);
					this.moveToIgnoreState();
