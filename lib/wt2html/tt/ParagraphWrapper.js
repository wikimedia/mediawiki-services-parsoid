...
coreutil.inherits(ParagraphWrapper, TokenHandler);
...
function removeUselessPWrappers(res) {
	...
	for (var i = 0, n = res.length; i < n; i++) {
		var t = res[i];
		if (i + 2 < n && t.constructor === TagTk && t.name === 'p' && !Util.isHTMLTag(t) &&
			Consts.HTML.FormattingTags.has((res[i + 1].name || '').toUpperCase()) &&
			res[i + 2].constructor === EndTagTk && res[i + 2].name === 'p' && !Util.isHTMLTag(res[i + 2])) {
			// Init newRes
			if (newRes === null) {
...
ParagraphWrapper.prototype.processPendingNLs = function(isBlockToken) {
	...
	if (newLineCount >= 2) {
		while (newLineCount >= 3) {
			...
			var topTag = this.tableTags.length > 0 ? lastItem(this.tableTags) : null;
