...
class ParagraphWrapper extends TokenHandler {
	constructor(manager, options) {
		...
		// Disable p-wrapper
		this.switchedOff = this.options.inlineContext || this.options.inPHPBlock;
		this.reset();
	}
	...
	onAny(token) {
		...
		if (tc === TagTk && token.name === 'pre' && !TokenUtils.isHTMLTag(token)) {
			if (this.inBlockElem) {
				...
				return { tokens: this._processBuffers(token, true), skip: true };
			}
			...
			if (this.newLineCount === 0) {
				...
				return { tokens: this._flushBuffers(), skip: true };
			} else {
			...
			if (this.newLineCount === 0) {
				...
				return { tokens: this._processBuffers(token, false), skip: true };
			}
		} else {
