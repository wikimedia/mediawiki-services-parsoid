...
class ExtensionHandler extends TokenHandler {
	...
	onExtension(token, frame, cb) {
		...
		if (nativeExt && nativeExt.toDOM) {
			...
			if (p) {
				...
				cb({ async: true });
				p.nodify(this.parseExtensionHTML.bind(this, token, cb));
			} else {
			...
			const toks = PipelineUtils.encapsulateExpansionHTML(env, token, cachedExpansion, { setDSR: true });
			cb({ tokens: toks });
			...
			this.fetchExpandedExtension(
				...
				(err, html) => {
					...
					this.parseExtensionHTML(token, cb, err, err ? null : DU.parseHTML(html));
				}
			);
		} else {
	...
	_onDocument(state, cb, doc) {
		...
		if (opts.isForeignContent) {
			...
			DU.addSpanWrappers(body.childNodes);
			...
			// Set data-mw
			DU.setDataMw(firstNode, Object.assign(state.wrapperDataMw || {}, argDict));
		...
		const toks = PipelineUtils.buildDOMFragmentTokens(env, state.token, body, opts);
