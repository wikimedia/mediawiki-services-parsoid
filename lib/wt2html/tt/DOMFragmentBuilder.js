...
const { TagTk,EOFTk,SelfclosingTagTk,EndTagTk } = require('../parser.defines.js');
...
class DOMFragmentBuilder extends TokenHandler {
	constructor(manager, options) {
		...
		this.manager.addTransform(
			(scopeToken, frame, cb) => this.buildDOMFragment(scopeToken, frame, cb),
			'buildDOMFragment',
	...
	buildDOMFragment(scopeToken, frame, cb) {
	...
	wrapDOMFragment(cb, scopeToken, opts, dom) {
		// Pass through pipeline options
		const toks = PipelineUtils.buildDOMFragmentTokens(this.manager.env, scopeToken, dom.body, false, opts);
