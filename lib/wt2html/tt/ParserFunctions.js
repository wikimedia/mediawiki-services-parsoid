...
ParserFunctions.prototype.expandKV = function(kv, cb, defaultValue, type, trim) {
	...
	if (kv === undefined) {
		...
		var self = this;
		var getCB = function(v) {
			cb({ tokens: self._rejoinKV(trim, kv.k, v) });
		};
		kv.v.get({
			type: type,
			cb: getCB,
...
ParserFunctions.prototype.pf_localurl = function(token, frame, cb, args) {
	...
	var self = this;
	args = args.slice(1);
	Promise.all(args.map(function(item) {
		return new Promise(function(resolve, reject) {
			// FIXME: we are swallowing all errors
			// FIXME: this is the only use of Util.buildAsyncOutputBufferCB
			var resCB = Util.buildAsyncOutputBufferCB(resolve);
...
ParserFunctions.prototype.pf_directionmark = function(token, frame, cb, args) {
	...
	cb({ tokens: [ Util.decodeEntities(mark) ] });
};
...
ParserFunctions.prototype.pf_fullurl = function(token, frame, cb, args) {
	var target = args[0].k;
	cb({ tokens: [ target || this.env.conf.wiki.articlePath + this.env.page.name.split('/').map(encodeURIComponent).join('/') || "http://example.com/fixme/" ] });
};
...
ParserFunctions.prototype.pf_anchorencode = function(token, frame, cb, args) {
	...
	var tokens = target.split(/([\{\}\[\]|]|''|ISBN|RFC|PMID|__)/g).map(function(s,i) {
		if ((i % 2) === 0) { return s; }
		var charEntity = function(c) {
			var enc = Util.entityEncodeAll(c);
			return [
				new defines.TagTk(
					'span',
					[new defines.KV('typeof', 'mw:Entity')],
					{ src: enc, srcContent: c }
				),
				c,
				new defines.EndTagTk('span', [], {})
			];
		};
		if (s === "''") { return [ charEntity(s[0]), charEntity(s[1]) ]; }
		return [ charEntity(s[0]), s.slice(1) ];
	});
	cb({ tokens: tokens });
};
	...
	cb({
		tokens: [
			new defines.SelfclosingTagTk('meta', [
				new defines.KV('property', 'mw:PageProp/categorydefaultsort'),
