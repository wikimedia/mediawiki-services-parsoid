...
const { TagTk, EndTagTk, SelfclosingTagTk } = require('../../tokens/TokenTypes.js');
...
class QuoteTransformer extends TokenHandler {
	constructor(manager, options) {
		...
		this.manager.addTransform(
			(token, prevToken, cb) => this.onQuote(token, prevToken),
	 ...
	 */
	onQuote(token, prevToken) {
		var qlen = token.value.length;
		this.manager.env.log("trace/quote", this.manager.pipelineId, "QUOTE |", () => JSON.stringify(token));

		if (!this.isActive) {
			const processQuotes = (token, prevToken, cb) => this.processQuotes(token);
			const onAny = (token, prevToken, cb) => this.onAny(token);
			this.manager.addTransform(
				processQuotes,
	...
	processQuotes(token) {
		...
		if ((numitalics % 2 === 1) && (numbold % 2 === 1)) {
			...
			for (i = 1; i < this.chunks.length; i += 2) {
				...
				// If the previous token is a string, use that as context.
				// If not a string (comment, tag, link, transclusion etc),
				// we are effectively treating context as a "multi-letter-word".
				var ctxPrevToken = '';
				if (prevChunk[prevChunk.length - 1].constructor === String) {
					ctxPrevToken = prevChunk[prevChunk.length - 1];
				}
