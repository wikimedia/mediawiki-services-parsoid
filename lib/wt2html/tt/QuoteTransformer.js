 ...
 */
QuoteTransformer.prototype.onQuote = function(token, tokenManager, prevToken) {
	var qlen = token.value.length;
	this.manager.env.log("trace/quote", this.manager.pipelineId, "QUOTE |", () => JSON.stringify(token));

	if (!this.isActive) {
		this.manager.addTransform(this.processQuotes.bind(this),
			'QuoteTransformer:processQuotes', this.quoteAndNewlineRank, 'newline');
		// Treat 'th' just the same as a newline
		this.manager.addTransform(this.processQuotes.bind(this),
			'QuoteTransformer:processQuotes', this.quoteAndNewlineRank, 'tag', 'td');
		// Treat 'td' just the same as a newline
		this.manager.addTransform(this.processQuotes.bind(this),
			'QuoteTransformer:processQuotes', this.quoteAndNewlineRank, 'tag', 'th');
		// Treat end-of-input just the same as a newline
		this.manager.addTransform(this.processQuotes.bind(this),
			'QuoteTransformer:processQuotes:end', this.quoteAndNewlineRank, 'end');
...
QuoteTransformer.prototype.onAny = function(token) {
 ...
 */
QuoteTransformer.prototype.processQuotes = function(token) {
