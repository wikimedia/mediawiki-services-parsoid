...
var { PegTokenizer } = require('../tokenizer.js');
var { WikitextConstants } = require('../../config/WikitextConstants.js');
var { Sanitizer } = require('./Sanitizer.js');
var { Util } = require('../../utils/Util.js');
var TokenHandler = require('./TokenHandler.js');
var { DOMUtils } = require('../../utils/DOMUtils.js');
var { JSUtils } = require('../../utils/jsutils.js');
var Promise = require('../../utils/promise.js');
var { KV, EOFTk, TagTk, SelfclosingTagTk, EndTagTk, Token } =
...
class WikiLinkHandler extends TokenHandler {
	...
	getWikiLinkTargetInfo(token, hrefKV) {
		...
		if (/^:/.test(info.href)) {
			if (env.conf.parsoid.linting) {
				...
				if (this.options.inTemplate) {
					...
					// match `env.makeLink`.
					var name = Sanitizer.sanitizeTitleURI(
		...
		var hrefBits = WikiLinkHandler._hrefParts(info.href);
		if (hrefBits) {
	...
	*onRedirectG(token, frame) {
	...
	static bailTokens(env, token, isExtLink) {
		...
		if (isExtLink) {
			...
			var mwc = token.getAttribute('mw:content');
	...
	*onWikiLinkG(token, frame) {
		...
		if (env.conf.wiki.hasValidProtocol(target.href)) {
			...
			var src = '[' + token.attribs.slice(1).reduce((prev, next) => {
	 ...
	 * target structure.
	 */
	_wikiLinkHandler(token, frame, target, isRedirect) {
	...
	* ------------------------------------------------------------ */
	...
	addLinkAttributesAndGetContent(newTk, token, target, buildDOMFragment) {
		...
		var newAttrData = WikiLinkHandler.buildLinkAttrs(attribs, true, null, [new KV('rel', 'mw:WikiLink')]);
		...
		if (content.length > 0) {
			...
			if (buildDOMFragment) {
				...
				content = [ Util.getDOMFragmentToken(out, offsets, { inlineContext: true, token: token }) ];
			} else {
				content = out;
			}
		} else {
	 ...
	 */
	*renderWikiLinkG(token, frame, target) { // eslint-disable-line require-yield
	 ...
	 */
	*renderCategoryG(token, frame, target) {
		var tokens = [];
		...
		var saniContent = Sanitizer.sanitizeTitleURI(strContent).replace(/#/g, '%23');
	 ...
	 */
	*renderLanguageLinkG(token, frame, target) { // eslint-disable-line require-yield
		...
		var title = Sanitizer.sanitizeTitleURI(Util.decodeURIComponent(target.href));
		var absHref = target.language.url.replace("$1", title);
	 ...
	 */
	*renderInterwikiLinkG(token, frame, target) { // eslint-disable-line require-yield
		// The prefix is listed in the interwiki map
		...
		// We set an absolute link to the article in the other wiki/language
		var title = Sanitizer.sanitizeTitleURI(Util.decodeURIComponent(target.href));
		var absHref = target.interwiki.url.replace("$1", title);
	...
	static handleSize(env, opts, info) {
		...
		if (ratio !== null && ratio > 1) {
			...
			var format = WikiLinkHandler.getFormat(opts);
	...
	handleImage(opts, info, _, dataMw, optSources) {
		...
		var size = WikiLinkHandler.handleSize(this.env, opts, info);
	...
	static addSources(info, opts, dataMw, hasDimension) {
		var frag = WikiLinkHandler.parseFrag(info, opts, dataMw);
	...
	handleVideo(opts, info, manualinfo, dataMw, optSources) {
		...
		var sources = WikiLinkHandler.addSources(info, opts, dataMw, true);
		var tracks = WikiLinkHandler.addTracks(info);
	...
	handleAudio(opts, info, manualinfo, dataMw, optSources) {
		...
		var sources = WikiLinkHandler.addSources(info, opts, dataMw, false);
		var tracks = WikiLinkHandler.addTracks(info);

		var end = new EndTagTk('audio');
		var elt = [start].concat(sources, tracks, end);
	...
	*_requestInfoG(reqs, errorHandler) {
		...
		try {
			...
			infos = result.map((r, i) => {
				var info = WikiLinkHandler.extractInfo(env, r);
				if (!info) {
					info = errorHandler();
	...
	*handleInfoG(token, opts, optSources, errs, info, manualinfo) {
			...
			if (opts.link) {
				...
				if (this.urlParser.tokenizeURL(opts.link.v)) {
					...
					if (link !== null) {
						...
						var pos = token.dataAttribs.optList.reduce((prv, cur, ind) => {
		...
		var wrapperInfo = WikiLinkHandler.getWrapperInfo(opts, info);
		...
		if (optSources && !dataAttribs.uneditable) {
			var inVals = optSources.map(e => e[1]);
			var outVals = yield Util.expandValuesToDOM(
	 ...
	 */
	*renderFileG(token, frame, target) {
		var title = target.title;
		...
		var content = WikiLinkHandler.buildLinkAttrs(token.attribs, true, null, null).contentKVs;
		...
		while (optKVs.length > 0) {
			...
			if (oText.constructor !== String) {
				...
				var maybeOText = WikiLinkHandler.stringifyOptionTokens(oText, '', env);
				if (maybeOText !== null) {
					oText = maybeOText;
				}
			}
		...
		var result = yield this._requestInfo(reqs, WikiLinkHandler.errorInfo.bind(null, env, opts));
	...
	linkToMedia(token, target, errs, info) {
		...
		var imgHref = WikiLinkHandler.getPath({ url: info.url });
		...
		var tokens = [link].concat(content, [new EndTagTk('a')]);
		return { tokens: tokens };
	}

	*renderMediaG(token, frame, target) {
		var env = this.manager.env;
		var title = target.title;
		var reqs = [{
			promise: env.batcher
		...
		var result = yield this._requestInfo(reqs, () => {
			return {
...
class ExternalLinkHandler extends TokenHandler {
	constructor(manager, options) {
		...
		this.manager.addTransform(this.onUrlLink.bind(this),
			'ExternalLinkHandler:onUrlLink', ExternalLinkHandler.rank(), 'tag', 'urllink');
		this.manager.addTransform(this.onExtLink.bind(this),
			'ExternalLinkHandler:onExtLink', ExternalLinkHandler.rank() - 0.001, 'tag', 'extlink');
		this.manager.addTransform(this.onEnd.bind(this),
			'ExternalLinkHandler:onEnd', ExternalLinkHandler.rank(), 'end');
	...
	onUrlLink(token, frame, cb) {
		var tagAttrs, builtTag;
		var env = this.manager.env;
		var origHref = token.getAttribute('href');
	...
	onExtLink(token, manager, cb) {
		...
		var env = this.manager.env;
		var origHref = token.getAttribute('href');
		var hasExpandedAttrs = /mw:ExpandedAttrs/.test(token.getAttribute('typeof'));
		var href = Util.tokensToString(origHref);
		var hrefWithEntities = Util.tokensToString(origHref, false, {
			includeEntities: true,
		});
		var content = token.getAttribute('mw:content');
		var dataAttribs = Util.clone(token.dataAttribs);
		...
		if (rdfaType && magLinkRe.test(rdfaType)) {
			...
			(!hasExpandedAttrs && typeof origHref === 'string') ||
				this.urlParser.tokenizeURL(hrefWithEntities)
		) {
			rdfaType = 'mw:ExtLink';
			if (
				content.length === 1 &&
			...
			content = Util.getDOMFragmentToken(content, dataAttribs.tsr ? dataAttribs.contentOffsets : null, { inlineContext: true, token: token });
