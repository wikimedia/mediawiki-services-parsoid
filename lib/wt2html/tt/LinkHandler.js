	...
	*onRedirectG(token, frame) {
	...
	static bailTokens(env, token, isExtLink) {
		...
		if (/mw:ExpandedAttrs/.test(token.getAttribute("typeof"))) {
			...
			dft = PipelineUtils.buildDOMFragmentTokens(env, token, body, null, {
				inlineContext: true,
				tsr: tsr,
			});
		} else {
	...
	static getOptionInfo(optStr, env) {
		...
		if (imgOption && key === null) {
			...
			if (bits && key) {
				...
				// Note that we deliberately do entitiy decoding
	...
	static stringifyOptionTokens(tstream, prefix, env) {
		...
		for (let i = 0; i < tstream.length; i++) {
			...
			if (currentToken.constructor === String) {
				...
				if (
					(currentToken.name === 'span' && currentToken.getAttribute('typeof') === 'mw:Nowiki') ||
					currentToken.name === 'mw-quote'
				) {
					if (isWhitelistedOpt()) {
						// just recurse inside
