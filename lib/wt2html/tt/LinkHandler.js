...
WikiLinkHandler.prototype.onWikiLink = function(token, frame, cb) {
	...
	return this._wikiLinkHandler(token, frame, cb, target, isRedirect);
};
 ...
 * target structure.
 */
WikiLinkHandler.prototype._wikiLinkHandler = function(token, frame, cb, target, isRedirect) {
	var title = target.title;
	if (title) {
		if (isRedirect) {
			return this.renderWikiLink(token, frame, cb, target);
		}
		if (title.getNamespace().isMedia()) {
			// Render as a media link.
			return this.renderMedia(token, frame, cb, target);
		}
		if (!target.fromColonEscapedText) {
			if (title.getNamespace().isFile()) {
				// Render as a file.
				return this.renderFile(token, frame, cb, target);
			}
			if (title.getNamespace().isCategory()) {
				// Render as a category membership.
				return this.renderCategory(token, frame, cb, target);
			}
		}
	...
	// language and interwiki links
		...
		if (target.interwiki) {
			return this.renderInterwikiLink(token, frame, cb, target);
			...
			if (noLanguageLinks) {
				...
				return this.renderLanguageLink(token, frame, cb, target);
			}
		}
	}
...
WikiLinkHandler.prototype.renderInterwikiLink = function(token, frame, cb, target) {
	...
	if (target.href[0] !== "#") {
		var titleAttr = target.interwiki.prefix + ':' +
				Util.decodeURI(target.href.replace(/#[\s\S]*/, '').replace(/_/g, ' '));
...
function handleSize(env, opts, info) {
	...
	if (info.mediatype === 'AUDIO') {
		height = /* height || */ 20;  // Arguably, audio should respect a defined height
...
function getWrapperInfo(opts) {
	...
	if (isInline || true) {  // eslint-disable-line
...
// Internal Helper
WikiLinkHandler.prototype._requestInfoAndRenderLink = function(reqs, errorHandler, renderer) {
	var env = this.manager.env;
...
WikiLinkHandler.prototype.renderFile = function(token, frame, cb, target) {
	...
	if (!env.conf.parsoid.fetchImageInfo) {
		...
		return this.handleInfo(cb, token, opts, optSources, [err], errorInfo(env, opts));
	}
...
WikiLinkHandler.prototype.linkToMedia = function(cb, token, title, errs, info) {
	var nsText, fileName;
	var hrefParts = token.getAttribute('href')[0].match(/^(.+?)_*:_*(.*)$/);
	if (hrefParts) {
		nsText = hrefParts[1];
	...
	link.setShadowInfo('namespace', title.getNamespace().getNormalizedText(), nsText);
