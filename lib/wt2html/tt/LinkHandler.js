...
class WikiLinkHandler extends TokenHandler {
	...
	*onWikiLinkG(token, frame) {
		const env = this.manager.env;
		const hrefKV = TokenUtils.lookupKV(token.attribs, 'href');
		let target;
	...
	*renderCategoryG(token, frame, target) {
		...
		TokenUtils.lookupKV(newTk.attribs, 'rel').v = 'mw:PageProp/Category';
		...
		if (strContent && strContent !== '' && strContent !== target.href) {
			const hrefkv = TokenUtils.lookupKV(newTk.attribs, 'href');
			hrefkv.v += '#';
	...
	*renderLanguageLinkG(token, frame, target) { // eslint-disable-line require-yield
		...
		TokenUtils.lookupKV(newTk.attribs, 'rel').v = 'mw:PageProp/Language';
	...
	*renderInterwikiLinkG(token, frame, target) { // eslint-disable-line require-yield
		...
		TokenUtils.lookupKV(newTk.attribs, 'rel').v = 'mw:WikiLink/Interwiki';
	...
	*renderFileG(token, frame, target) {
		...
		const opts = {
			title: {
				v: title,
				src: TokenUtils.lookupKV(token.attribs, 'href').vsrc,
			},
			size: {
				v: {
					height: null,
