...
WikiLinkHandler.prototype.getWikiLinkTargetInfo = function(token) {
	...
	if (hrefBits) {
		...
		if (ns !== undefined) {
			info.title = env.makeTitleFromURLDecodedStr(title.replace(/^[^:]+:/, ''), ns);
		} else if (interwikiInfo && interwikiInfo.localinterwiki !== undefined) {
			if (hrefBits[2] === '') {
				// Empty title => main page (T66167)
				info.title = env.makeTitleFromURLDecodedStr(env.conf.wiki.mainpage, 0);
			} else {
	...
	if (hasImageLink) {
		if (opts.link) {
			...
			if (this.urlParser.tokenizeURL(opts.link.v)) {
				...
				title = this.env.makeTitleFromText(opts.link.v);
				innerContain.addNormalizedAttribute('href', this.env.makeLink(title), opts.link.src);
			}
...
WikiLinkHandler.prototype.renderFile = function(token, frame, cb, target) {
	...
	if (!env.conf.parsoid.fetchImageInfo) {
		return this.handleImageInfo(cb, token, title, opts, optSources, 'Fetch of image info disabled.');
	} else if (!title) {
		return this.handleImageInfo(cb, token, title, opts, optSources, 'Error creating thumbnail: "' + opts.manualthumb.v + '"');
	}
	...
	env.batcher.imageinfo(
		title.getKey(),
		opts.size.v
	).nodify(this.handleImageInfo.bind(this, cb, token, title, opts, optSources));
};
