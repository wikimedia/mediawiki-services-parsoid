	...
	if (/^:/.test(info.href)) {
		if (env.conf.parsoid.linting) {
			...
			if (this.options.inTemplate) {
				...
				// match `env.makeLink`.
				var name = Sanitizer.sanitizeTitleURI(env.page.relativeLinkPrefix +
...
WikiLinkHandler.prototype.linkToMedia = function(token, target, errs, info) {
	...
	link.addNormalizedAttribute('resource',
		this.env.makeLink(target.title), target.hrefSrc);
...
ExternalLinkHandler.prototype.onExtLink = function(token, manager, cb) {
	...
	} else if ((!hasExpandedAttrs && typeof origHref === 'string') ||
