	...
	if (hrefBits) {
		...
		if (ns !== undefined) {
			...
			if (hrefBits[2] === '') {
				...
				// Recurse!
				var hrefKV = new KV('href', (/:/.test(info.href) ? ':' : '') + info.href);
				hrefKV.vsrc = info.hrefSrc;
				info = this.getWikiLinkTargetInfo(new TagTk('a', [hrefKV], token.dataAttribs));
...
// Internal Helper
WikiLinkHandler.prototype._requestInfoAndRenderLink = function(reqs, manualthumb, errorHandler, renderer) {
	var env = this.manager.env;
	Promise.all(reqs)
...
WikiLinkHandler.prototype.renderFile = function(token, frame, cb, target) {
	...
	var reqs = [
		env.batcher.imageinfo(title.getKey(), dims).then(wrapResp(title)),
	];
...
WikiLinkHandler.prototype.linkToMedia = function(cb, token, title, errs, info) {
	...
	link.setAttribute('title',
		// Ugliness galore!
		imgHrefFileName.replace(/_/g, ' ') === fileName.replace(/_/g, ' ') ? fileName : imgHrefFileName);
	...
	var content = token.getAttribute('mw:maybeContent') || token.getAttribute('href');
...
WikiLinkHandler.prototype.renderMedia = function(token, frame, cb, target) {
	...
	var title = target.title;
	var reqs = [
		env.batcher.imageinfo(title.getKey(), { height: null, width: null })
		.then(function(data) {
