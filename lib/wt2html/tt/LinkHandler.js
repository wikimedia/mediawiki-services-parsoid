...
const { KV, EOFTk, TagTk, SelfclosingTagTk, EndTagTk, Token } =
...
class WikiLinkHandler extends TokenHandler {
	...
	*onRedirectG(token, frame) {
		...
		const r = yield this.onWikiLink(wikiLinkTk, frame);
	...
	*onWikiLinkG(token, frame) {
		const env = this.manager.env;
		const hrefKV = Util.lookupKV(token.attribs, 'href');
		let target;
	...
	*renderCategoryG(token, frame, target) {
		...
		const strContent = Util.tokensToString(content);
		...
		if (strContent && strContent !== '' && strContent !== target.href) {
			const hrefkv = Util.lookupKV(newTk.attribs, 'href');
			hrefkv.v += '#';
		...
		if (content.length === 1) {
			...
			const outVals = yield Util.expandValuesToDOM(
				this.manager.env,
				this.manager.frame,
				inVals,
	...
	*handleInfoG(token, opts, optSources, errs, info, manualinfo) {
		...
		if (opts.caption !== undefined) {
			if (!isInline) {
				...
				const captionDOM = yield Util.promiseToProcessContent(
	...
	*renderFileG(token, frame, target) {
		...
		if (cachedMedia) {
			const wrapperTokens = DU.encapsulateExpansionHTML(env, token, cachedMedia, {
				noAboutId: true,
				setDSR: true,
			});
		...
		const result = yield this._requestInfo(reqs, WikiLinkHandler.errorInfo.bind(null, env, opts));
	...
	linkToMedia(token, target, errs, info) {
		...
		let content = Util.tokensToString(token.getAttribute('href')).replace(/^:/, '');
