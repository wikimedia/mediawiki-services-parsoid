...
TemplateHandler.prototype.onTemplate = function(token, frame, cb) {
	...
	if (env.conf.parsoid.usePHPPreProcessor) {
		...
		).process(token.attribs).then(function(attribs) {
			// could be inlined
			this._expandTemplate(
				state, frame, tgt, accumReceiveToksFromSibling, attribs
			);
		}.bind(this)).done();
	}
};
...
TemplateHandler.prototype.resolveTemplateTarget = function(state, targetToks) {
	...
	if (canonicalFunctionName !== undefined) {
		...
		if (i > -1) {
			...
			var firstTok = normalizeTarget(targetToks[i]).replace(re, '');
			...
			// going to generate incorrect output, but it won't crash.
			pfArgToks = targetToks;
		}
		...
		if (env.conf.parsoid.addHTMLTemplateParameters) {
			...
			if (params.length) {
				...
				async.each(params, getParamHTML.bind(this), function(err) {
					...
					env.log('debug', 'TemplateHandler._encapsulateTemplate', chunk, err);
					cb({ tokens: chunk });
				});
				cb({ tokens: [], async: true });
				return;
