...
class TemplateHandler extends TokenHandler {
	...
	constructor(manager, options) {
		...
		this.manager.addTransform(
			(token, frame, cb) => this.onTemplate(token, frame, cb),
			"TemplateHandler:onTemplate", TemplateHandler.RANK(), 'tag', 'template');
		// Template argument expansion
		this.manager.addTransform(
			(token, frame, cb) => this.onTemplateArg(token, frame, cb),
	...
	resolveTemplateTarget(state, targetToks) {
	 ...
	 */
	_expandTemplate(state, frame, resolvedTgt, cb, attribs) {
		var env = this.manager.env;
	 ...
	 */
	_startTokenPipeline(state, frame, cb, tplArgs, err, src) {
				...
				for (i = 0, n = argInfo.paramInfos.length; i < n; i++) {
					...
					if (paramTokens &&
							(paramTokens.constructor === String ||
								TemplateHandler.isSimpleParam(paramTokens))) {
						param.html = param.wt;
	...
	onTemplateArg(token, frame, cb) {
		...
		this.fetchArg(attribs[0].k, ret => this.lookupArg(args, attribs, newCB, cb, ret), cb);
	}

	fetchArg(arg, argCB, asyncCB) {
