...
TemplateHandler.prototype.resolveTemplateTarget = function(state, targetToks) {
	function toStringOrNull(tokens) {
		var maybeTarget = Util.tokensToString(tokens, true, { retainNLs: true });
	...
	var normalizeTarget = function(txt) {
		// Without a trim(), we get bug T147742 because the prefix === target
	...
	if ((magicWordAlias && this.parserFunctions && this.parserFunctions['pf_' + magicWordAlias]) ||
		...
		if (typeof firstTok === 'string') {
			...
			firstTok = normalizeTarget(firstTok).replace(new RegExp("^" + prefix, 'i'), '');
