...
Sanitizer.sanitizeTagAttrs = function(env, tagName, token, attrs) {
	...
	var allowRdfa = SanitizerConstants.globalConfig.allowRdfaAttrs;
	var allowMda  = SanitizerConstants.globalConfig.allowMicrodataAttrs;
	var html5Mode = SanitizerConstants.globalConfig.html5Mode;
	var xmlnsRE   = SanitizerConstants.XMLNS_ATTRIBUTE_RE;
	var evilUriRE = SanitizerConstants.EVIL_URI_RE;
	...
	if (Util.isHTMLTag(token) && (
		...
		if (attribs && attribs.length > 0) {
			...
			if (token.constructor === TagTk || token.constructor === SelfclosingTagTk) {
				this.sanitizeTagAttrs(env, null, token, attribs);
			} else {
 ...
 */
Sanitizer.sanitizeTitleURI = function(title) {
	...
	if (anchor !== null) {
		title += '#' + Sanitizer.escapeIdForLink(anchor);
	}
	return title;
};
 ...
 * Sanitize a URI?
 */
Sanitizer.sanitizeURI = function(s) {
	if (!Sanitizer) {
		Sanitizer = require('../wt2html/tt/Sanitizer.js').Sanitizer;
	}
	...
	s = host + path;
	if (anchor !== null) {
		s += '#' + Sanitizer.escapeIdForLink(anchor);
	}
	return s;
};
 ...
 */
function SanitizerHandler(manager, options) {
...
// constants
SanitizerHandler.prototype.anyRank = 2.91;

// Register this transformer with the TokenTransformer
SanitizerHandler.prototype.register = function(manager) {
	this.manager = manager;
	manager.addTransform(this.onAny.bind(this), "SanitizerHandler:onAny", this.anyRank, 'any');
};
