 ...
 */
function Sanitizer(env, inTemplate) {
	this.env = env;
 ...
 */
Sanitizer.prototype.sanitizeToken = function(token) {
	var i, l, k, v, kv;
	...
	if (Util.isHTMLTag(token) && (
		...
		if (!this.inTemplate && token.dataAttribs.tsr) {
			// Just get the original token source, so that we can avoid
			// whitespace differences.
			token = token.getWTSource(this.env);
 ...
 *   (PHP also supports a deprecated 'html5-legacy' mode)
 * @return {String}
 */
Sanitizer.escapeIdInternal = function(id, mode) {
	switch (mode) {
		...
		case 'legacy':
			...
			id = id.replace(/%/g, '.');
			break;
		// PHP core also defines 'html5-legacy' for historical reasons.
		// We don't need it for Parsoid.
		default:
...
Sanitizer.prototype.sanitizeTagAttrs = function(token, attrs) {
	...
	for (var i = 0; i < n; i++) {
		...
		if (k === 'href' || k === 'src' || k === 'poster') { // T163583
			...
			// XXX interwiki links (when we mark them) should probably be
			// treated as wikilinks, rather than external links.
			var mode = (k === 'href' && rel && rel.value === 'mw:WikiLink') ?
				'wikilink' : 'external';
...
function SanitizerHandler(manager, options) {
	...
	this.sanitizer = new Sanitizer(manager.env, options.inTemplate);
}
