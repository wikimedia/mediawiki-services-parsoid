...
var SanitizerConstants = {
	 ...
	 */
	IDN_RE: idnRE,
	IDN_RE_G: JSUtils.rejoin(idnRE, { flags: 'g' }),
...
Sanitizer.prototype.sanitizeHref = function(href) {
	...
	if (bits) {
		...
		host = bits[2];
		path = bits[3];
		if (proto && !this.manager.env.conf.wiki.hasValidProtocol(proto)) {
...
Sanitizer.prototype.onAnchor = function(token) {
	...
	if (hrefKV !== null) {
		...
		var newHref = this.sanitizeHref(origHref);
		if (newHref === null) {
...
Sanitizer.prototype.onAny = function(token) {
	...
	if (Util.isHTMLTag(token) &&
		...
		if (origToken.name === 'section') {
			...
			var sectionAttribs = [];
			origToken.attribs.forEach(function(a) {
				var content;
				if (a.k === 'begin') {
					sectionType += '/begin';
					content = a.v;
				...
				if (content) {
					sectionAttribs.push({k: 'content', v: content});
				}
			});
			sectionAttribs = [{k: 'typeof', v: sectionType}].concat(sectionAttribs);
			token = new SelfclosingTagTk('meta', sectionAttribs, {
				src: token,
				tsr: origToken.tsr,
			});
		}
		...
		if (attribs && attribs.length > 0) {
			...
			var htmlpre = token.name === "pre" && p && /^mw:html$/.test(p);
