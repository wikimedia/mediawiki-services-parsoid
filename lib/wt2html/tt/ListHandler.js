...
class ListHandler extends TokenHandler {
	static LIST_RANK() { return 2.49; } // before Sanitizer
	static ANY_RANK() { return 2.49 + 0.001; } // before Sanitizer
	...
	constructor(manager, options) {
		...
		this.manager.addTransform(
			(token, frame, prevToken) => this.onListItem(token, frame, prevToken),
			'ListHandler:onListItem', ListHandler.LIST_RANK(), 'tag', 'listItem'
		);
		this.manager.addTransform(
			(token, frame, prevToken) => this.onEnd(token, frame, prevToken),
			'ListHandler:onEnd', ListHandler.LIST_RANK(), 'end'
		);
		this.listFrames = [];
		this.reset();
	}
	...
	closeLists(token) {
		...
		if (this.listFrames.length === 0) {
			this.manager.removeTransform(ListHandler.ANY_RANK(), 'any');
		}
	...
	doListItem(bs, bn, token) {
		...
		if (!bs.length && this.listFrames.length === 0) {
			this.manager.addTransform(
				(token, frame, prevToken) => this.onAny(token, frame, prevToken),
				"ListHandler:onAny", ListHandler.ANY_RANK(), 'any');
		}
		...
		res.rank = ListHandler.ANY_RANK() + 0.01;
		this.currListFrame.solTokens = [];
		this.currListFrame.nlTk = null;
