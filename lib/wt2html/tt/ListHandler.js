...
class ListHandler extends TokenHandler {
	constructor(manager, options) {
		...
		this.manager.addTransform(
			...
			'ListHandler:onListItem', this.listRank, 'tag', 'listItem'
		);
		this.manager.addTransform(
			(token, frame, prevToken) => this.onEnd(token, frame, prevToken),
			'ListHandler:onEnd', this.listRank, 'end'
		);
		this.listFrames = [];
		this.reset();
	}
...
ListHandler.prototype.resetCurrListFrame = function() {
...
ListHandler.prototype.doListItem = function(bs, bn, token) {
	...
	if (!bs.length && this.listFrames.length === 0) {
		this.manager.addTransform(
			...
			"ListHandler:onAny", this.anyRank, 'any');
	}
