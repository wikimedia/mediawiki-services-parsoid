...
PegTokenizer.prototype.readSource = function() {
	var pegSrcPath = path.join(__dirname, 'pegTokenizer.pegjs');
	return fs.readFileSync(pegSrcPath, 'utf8');
};

PegTokenizer.prototype.parseTokenizer = function() {
	var src = this.readSource();
	return PEG.parser.parse(src);
};

PegTokenizer.prototype.compileTokenizer = function(ast) {
	...
	function cacheRuleHook(opts) {
		...
		if (opts.storeRefs.length) {
			cacheBits.store.push('    refs: {},');
		}
