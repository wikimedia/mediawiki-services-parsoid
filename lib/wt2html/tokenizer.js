...
PegTokenizer.prototype.compileTokenizer = function(ast, compileOpts = {}) {
	...
	var options = {
		...
		cacheInitHook: null,
		className: (compileOpts.className || 'Grammar'),
		allowedStartRules: [
			"start",
			"table_start_tag",
			"url",
		...
		allowLoops: true,
		// Don't report unreachable choices, because we deliberately add
		// assertions on unreachable paths.
		allowUselessChoice: true,
		headerComment: compileOpts.headerComment,
	};

	return compiler.compile(this.parseTokenizer(compileOpts), compiler.passes, options);
};
