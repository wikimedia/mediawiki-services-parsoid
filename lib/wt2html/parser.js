...
var supportedOptions = new Set([
	...
	// Any id assigned to the extension tag (ex: <references>)
	'extTagId',

	// Specific to ref extension, to prevent unwanted nesting
	// TODO(arlolra): Maybe generalize this, should the need arise
	'allowNestedRef',
...
function getCacheKey(cacheKey, options) {
	...
	if (options.extTag) {
		...
		if (options.allowNestedRef) {
			cacheKey += '::allowNestedRef';
		}
	}
	return cacheKey;
}
 ...
 */
ParserPipeline.prototype.process = function(input) {
	try {
