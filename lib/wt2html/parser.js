...
ParserPipelineFactory.prototype.makePipeline = function(type, options) {
	...
	const phpComponents = pipelineConfig && pipelineConfig.wt2html;
	const phpTokenTransformers = phpComponents && phpComponents.TT || null;
	...
	var stages = [];
	let PHPBuffer, PHPTokenTransformer, PHPPipelineStage;
		...
		if (stageData.constructor === String) {
			...
			if (phpComponents && (
				...
				if (stageData.length >= 3) {
					stage.transformers = stageData[2].map(T => T.name);
				}
			} else {
					...
					for (var j = 0; j < transforms.length; j++) {
						...
						if (phpTokenTransformers && phpTokenTransformers[T.name]) {
							...
							if (!PHPBuffer) {
								...
								stage.transformers.push(new PHPBuffer(stage, options));
							}
							stage.transformers.push(new PHPTokenTransformer(this.env, stage, T.name, options));
						} else {
