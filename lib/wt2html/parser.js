...
ParserPipelineFactory.prototype.makePipeline = function(type, options) {
	...
	const phpTokenTransformers = pipelineConfig && pipelineConfig.phpTokenTransformers || null;
	...
	for (var i = 0, l = recipe.length; i < l; i++) {
		...
		if (stageData.constructor === String) {
			...
			if (stageData.length >= 3) {
				...
				let PHPBuffer, PHPTransformer;
				for (var j = 0; j < transforms.length; j++) {
					const T = transforms[j];
					if (phpTokenTransformers && phpTokenTransformers.indexOf(T.name) >= 0) {
						...
						stage.transformers.push(new PHPTransformer(stage, T.name, options));
					} else {
