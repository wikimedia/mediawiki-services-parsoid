...
function ParserPipelineFactory(env) {
	...
	const configFile = path.resolve(__dirname, '../../pipeline.yaml');
	try {
		const yamlStr = fs.readFileSync(configFile, 'utf8');
			...
			if (stageData.length >= 3) {
				...
				for (var j = 0; j < transforms.length; j++) {
					...
					if (this.pipelineConfig &&
						(this.pipelineConfig.phpTokenTransformers || []).indexOf(T.name) >= 0
					) {
						if (!PHPBuffer) {
