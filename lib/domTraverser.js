...
DOMTraverser.prototype.callHandlers = function(node, atTopLevel, tplInfo) {
	var name = (node.nodeName || '').toLowerCase();
	var document = node.ownerDocument;

	for (var ix = 0; ix < this.handlers.length; ix++) {
		if (this.handlers[ix].name === null || this.handlers[ix].name === name) {
			var result = this.handlers[ix].run(node, atTopLevel, tplInfo);
			if (result !== true) {
				if (result === undefined) {
					this.env.log("error",
...
DOMTraverser.prototype.traverse = function(workNode, env, options, atTopLevel, tplInfo) {
	while (workNode !== null) {
		...
		var possibleNext = this.callHandlers(workNode, atTopLevel, tplInfo);
