...
DOMTraverser.prototype.callHandlers = function(node, atTopLevel) {
	var name = (node.nodeName || '').toLowerCase();
	var document = node.ownerDocument;
	for (var ix = 0; ix < this.handlers.length; ix++) {
		if (this.handlers[ix].name === null || this.handlers[ix].name === name) {
			var result = this.handlers[ix].run(node, atTopLevel);
			if ( result !== true ) {
				if ( result === undefined ) {
					this.env.log("error",
...
DOMTraverser.prototype.traverse = function( workNode, env, options, atTopLevel ) {
	...
	while ( workNode !== null ) {
		...
		result = this.callHandlers( workNode, atTopLevel );
		...
		if ( result !== true ) {
			...
			if ( DU.isElt(workNode) && workNode.childNodes.length > 0 ) {
				this.traverse( workNode.firstChild, env, options, atTopLevel );
			}
