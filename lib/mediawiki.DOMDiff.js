...
function nextNonTemplateSibling(env, node) {
	return DU.isTplOrExtToplevelNode(env, node) ? DU.skipOverEncapsulatedContent(node) : node.nextSibling;
}
...
DDP.doDOMDiff = function ( baseParentNode, newParentNode ) {
	...
	while ( baseNode && newNode ) {
		...
		if ( ! this.treeEquals(baseNode, newNode, false) ) {
			...
			if (!foundDiff) {
					...
					DU.getDataParsoid(savedNewNode).stx === DU.getDataParsoid(baseNode).stx)
				{
					...
					if (!DU.isTplOrExtToplevelNode(this.env, baseNode) &&
					...
					// there were deletions in the new subtree before 'savedNewNode'.
		...
		} else if (!DU.isTplOrExtToplevelNode(this.env, baseNode) && !DU.isTplOrExtToplevelNode(this.env, newNode)) {
...
DDP.markNode = function(node, change, blockNodeDeleted) {
	...
	if (change === 'deleted') {
		...
		} else if (DU.isText(node)) {
			if (change !== 'inserted') {
				this.env.log("error", "BUG! CHANGE-marker for text node is: " + change);
			}
			meta = DU.prependTypedMeta(node, 'mw:DiffMarker');
