		...
		if ( ! this.treeEquals(baseNode, newNode, false) ) {
			...
			if (!foundDiff) {
				if (!DU.isElt(savedNewNode)) {
					...
					if (!DU.isTplElementNode(this.env, baseNode) && !DU.isTplElementNode(this.env, savedNewNode)) {
...
DDP.markNode = function(node, change) {
	if (change === 'deleted') {
		...
		if (DU.isElt(node)) {
			...
			DU.prependTypedMeta(node, 'mw:DiffMarker');
		} else if (node.nodeType !== node.DOCUMENT_NODE && node.nodeType !== node.DOCUMENT_TYPE_NODE) {
