...
ParsoidLogger.prototype.registerLoggingBackends = function(defaultLogLevels, parsoidConfig, lintLogger) {
	...
	function buildTraceOrDebugFlag(parsoidFlags, logType) {
		var escapedFlags = Array.from(parsoidFlags).map(Util.escapeRegExp);
	...
	if (lintLogger && parsoidConfig.linting) {
		this.registerBackend(/^lint(\/.*)?/, lintLogger.linterBackend.bind(lintLogger));
		this.registerBackend(/^end(\/.*)/, lintLogger.logLintOutput.bind(lintLogger));
	}
};
