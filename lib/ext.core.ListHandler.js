...
ListHandler.prototype.onAny = function ( token, frame, prevToken ) {
	this.env.log("trace/list", "ANY:", function() { return JSON.stringify(token); });
...
ListHandler.prototype.onEnd = function( token, frame, prevToken ) {
	this.env.log("trace/list", "END:", function() { return JSON.stringify(token); });
...
ListHandler.prototype.closeLists = function(token) {
	...
	this.env.log("trace/list", "----closing all lists----");
	this.env.log("trace/list", "RET", tokens);
...
ListHandler.prototype.doListItem = function ( bs, bn, token ) {
	this.env.log("trace/list", "BEGIN:", function() { return JSON.stringify(token); });
	...
	if (prefix.length === bs.length && bn.length === bs.length) {
			...
			this.isDtDd( bs[prefixLen], bn[prefixLen] ) )
		{
			...
			this.env.log("trace/list", "    -> reduced nesting");
		...
		for (var i = prefixLen + prefixCorrection; i < bn.length; i++) {
			...
			if (i === prefixLen) {
				this.env.log("trace/list", "    -> increased nesting: first");
				listDP     = makeDP(prefixCorrection,prefixCorrection);
				listItemDP = makeDP(prefixCorrection,prefixLen+1);
			} else {
				this.env.log("trace/list", "    -> increased nesting: 2nd and higher");
				listDP     = makeDP(i,i);
	...
	this.env.log("trace/list", "RET:", function() { return JSON.stringify(res); });
	return res;
};
