...
PHPParseRequest.prototype._handleJSON = function ( error, data ) {
	...
	try {
		...
		if (data.parse.modules) {
			...
			for (i in data.parse.modules) { this.env.page.extensionModules.add(data.parse.modules[i]); }
		}
		if (data.parse.modulescripts) {
			if (!this.env.page.extensionModuleScripts) {
				this.env.page.extensionModuleScripts = new Set();
			}
			for (i in data.parse.modulescripts) { this.env.page.extensionModuleScripts.add(data.parse.modulescripts[i]); }
		}
		if (data.parse.modulestyles) {
			if (!this.env.page.extensionModuleStyles) {
				this.env.page.extensionModuleStyles = new Set();
			}
			for (i in data.parse.modulestyles) { this.env.page.extensionModuleStyles.add(data.parse.modulestyles[i]); }
		}
		if (data.parse.modulemessages) {
			if (!this.env.page.extensionModuleMessages) {
				this.env.page.extensionModuleMessages = new Set();
			}
		...
		if (data.parse.categories) {
			for (i in data.parse.categories) {
				...
				if (category.sortkey) {
					parsedHtml += "#" + encodeURIComponent(category.sortkey);
				}
				parsedHtml += '"/>';
			}
		}
