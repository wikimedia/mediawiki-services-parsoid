...
var MAX_LISTENERS = 50000;

var MW_API_PREPROCESSOR_TIMEOUT = 30 * 1000; // 30 seconds
var MW_API_EXTPARSE_TIMEOUT = 30 * 1000; // 30 seconds
var MW_API_SRC_FETCH_TIMEOUT = 40 * 1000; // 40 seconds
var MW_API_IMG_INFO_TIMEOUT = 40 * 1000; // 40 seconds
var MW_API_CONFIG_INFO_TIMEOUT = 40 * 1000; // 40 seconds
var PARSOID_CACHE_REQ_TIMEOUT = 10 * 1000; // 10 seconds

// 50 seconds (this is less than 100s VE timeout so we still finish)
var PARSOID_REPARSE_TIMEOUT = 50 * 1000;

// No retrying reqs. to varnish
var PARSOID_CACHE_MAX_RETRIES = 0;

// No retrying config requests
// FIXME: but why? seems like 1 retry is not a bad idea
var MW_API_CONFIG_INFO_MAX_RETRIES = 0;

var MW_API_MAX_RETRIES = 1;

var USER_AGENT = 'Parsoid/0.1';
...
function ApiRequest(env, title) {
	...
	this.retries = MW_API_MAX_RETRIES;
	this.env = env;
	this.title = title;
	this.queueKey = title;
	...
	if ( !data.query.pages ) {
		if ( data.query.interwiki ) {
			...
			iwstr = this.env.conf.wiki.interwikiMap.get(interwiki.iw).url ||
				this.env.conf.parsoid.interwikiMap.get(interwiki.iw) ||
				'/' + interwiki.iw + '/' + '$1';
...
function ParsoidCacheRequest( env, title, oldid, options ) {
	...
	if (options.evenIfNotCached) {
		this.requestOptions.timeout = PARSOID_CACHE_REQ_TIMEOUT + PARSOID_REPARSE_TIMEOUT;
	} else {
...
var ConfigRequest = function(uri, env, proxy) {
	...
	if (!uri) {
		this.retries = MW_API_CONFIG_INFO_MAX_RETRIES;
