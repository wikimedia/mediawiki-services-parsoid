...
function ParsoidConfig(localSettings, options) {
	...
	if (options.phpConfigFile) {
		let configFile;
		try {
			configFile = path.resolve(__dirname, '../..', options.phpConfigFile);
			this.pipelineConfig = yaml.load(fs.readFileSync(configFile, 'utf8'));
		} catch (e) {
			console.error('Error loading PHP pipeline config: ' + configFile);
			console.error(e);
			process.exit(1);
		}
	}
...
ParsoidConfig.prototype.loadWMFApiMap = function() {
	var insertInMaps = (proxyURI, site) => {
	...
	Object.keys(wmfSiteMatrix).forEach((key) => {
		...
		if (!Number.isNaN(Number(key))) {
			val.site.forEach(site => insertInMaps(undefined, site));
			...
			val.forEach(function(site) {
				...
				insertInMaps.call(this, prv ? null : undefined, site);
			});
		}
	});
};
