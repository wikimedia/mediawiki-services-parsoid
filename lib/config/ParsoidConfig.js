...
function ParsoidConfig(localSettings, options) {
	...
	if (this.useDefaultPerformanceTimer) {
		...
		Util.StatsD(this.txstatsdHost, this.txstatsdPort).then(function(pt) {
			...
			console.log('Failed to initialize performance timer: ' + err);
		});
	}
 ...
 */
ParsoidConfig.prototype.WIKITEXT_CONTENT_TYPE = 'text/plain; charset=utf-8; profile="mediawiki.org/specs/wikitext/1.0.0"';

/**
 * @property {string} HTML_CONTENT_TYPE
 */
ParsoidConfig.prototype.HTML_CONTENT_TYPE = 'text/html; charset=utf-8; profile="mediawiki.org/specs/html/1.2.0"';

/**
 * @property {string} DATA_PARSOID_CONTENT_TYPE
 */
ParsoidConfig.prototype.DATA_PARSOID_CONTENT_TYPE = 'application/json; charset=utf-8; profile="mediawiki.org/specs/data-parsoid/0.0.2"';
