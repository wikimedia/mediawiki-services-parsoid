...
var parseWikitextToDOM = Promise.async(function *(state, extArgs, leadingWS, wikitext, parseOpts) {
	...
	Sanitizer.applySanitizedArgs(manager.env, wrapper, extArgs);
...
// Dummy wrapper to deal with async-ness
var returnHtml = function(state, html) {
	// Pass an async signal since the ext-content won't be processed synchronously
	state.cb({ async: true });
	return Promise.resolve(DU.parseHTML(html));
};
