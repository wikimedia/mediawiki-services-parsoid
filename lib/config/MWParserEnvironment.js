...
var MWParserEnvironment = function(parsoidConfig, options) {
	...
	// This is temporary and will be replaced after the call to
...
MWParserEnvironment.prototype.bumpParserResourceUse = function(resource, count) {
	...
	if (n < 0) {
		// Crude
		this.log('error/limits/wt2html/' + resource, ' Too many ' + resource + 's. Aborting!');
		throw new Error('wt2html: Too many ' + resource + 's. Aborting!');
	}
...
MWParserEnvironment.prototype.bumpSerializerResourceUse = function(resource, count) {
	...
	if (n < 0) {
		// Crude
		this.log('error/limits/html2wt/' + resource, ' Too many ' + resource + 's. Aborting!');
		throw new Error('html2wt: Too many ' + resource + 's. Aborting!');
	}
