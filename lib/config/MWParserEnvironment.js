 ...
 * and resolves hose w.r.t the current page name so that title-handling code elsewhere
 * only deal with non-relative title strings.
 */
MWParserEnvironment.prototype.resolveTitle = function(urlDecodedStr, resolveOnly) {
	var namespace = 0; // Default to main namespace
...
MWParserEnvironment.prototype._makeTitle = function(urlDecodedText, ns, noExceptions) {
	try {
		if (this.page && /^(\#|\/|\.\.\/)/.test(urlDecodedText)) {
			ns = this.page.ns;
		}
		urlDecodedText = this.resolveTitle(urlDecodedText);
		return Title.newFromText(urlDecodedText, this.conf.wiki.siteInfo, ns);
	} catch (e) {
		if (noExceptions) {
			return null;
		} else {
...
MWParserEnvironment.prototype.makeTitleFromText = function(str, ns, noExceptions) {
	return this._makeTitle(Util.decodeURI(str), ns, noExceptions);
};

/* See: Title::newFromURL in mediawiki. */
MWParserEnvironment.prototype.makeTitleFromURLDecodedStr = function(str, ns, noExceptions) {
