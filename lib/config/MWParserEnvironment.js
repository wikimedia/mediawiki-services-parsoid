...
MWParserEnvironment.prototype.bumpIOTime = function(resource, time) {
	if (!this.ioProfile[resource]) {
		this.ioProfile[resource] = 0;
	}
	this.ioProfile[resource] += time;
};
...
function formatLine(k, v) {
	return k.padStart(35) + ': ' + JSON.stringify(v).padStart(7);
}

MWParserEnvironment.prototype._formatProfile = function(profile) {
	...
	for (var i = 0; i < outLines.length; i++) {
		k = outLines[i][0];
		v = outLines[i][1];
		let buf = formatLine(k, v);
		if (this.counts[k]) {
			buf += '; count: ' + JSON.stringify(this.counts[k]).padStart(6);
			buf += '; per-instance: ' +
				JSON.stringify(Math.round(100000 * v / this.counts[k]) / 100000).padEnd(10);
		}
		lines.push(buf);
	}
...
MWParserEnvironment.prototype.printTimeProfile = function() {
	...
	outLines.push("");
	outLines.push(formatLine('TOTAL PARSE TIME', endTime - this.startTime));
	outLines.push(formatLine('TOTAL IO TIME', ioOut.total));
	outLines.push(formatLine('TOTAL CPU(?) TIME', cpuOut.total));
	outLines.push("");
	outLines.push("CAVEATS:");
	outLines.push("* Multiple IO requests overlap with each other.");
	outLines.push("* CPU and IO activity overlap with each other.");
...
MWParserEnvironment.getParserEnv = Promise.async(function *(parsoidConfig, options) {
	...
	yield env.switchToConfig(options.prefix);
 ...
 * @return {Promise}
 */
MWParserEnvironment.prototype.switchToConfig = Promise.async(function *(prefix) {
	var env = this;
...
MWParserEnvironment.prototype.makeLink = function(title) {
	return Sanitizer.sanitizeTitleURI(this.page.relativeLinkPrefix + this._titleToString(title));
};
...
MWParserEnvironment.prototype.isValidLinkTarget = function(href) {
	...
	var hrefToken = Util.decodeURIComponent(Util.tokensToString(href));
 ...
 */
MWParserEnvironment.prototype.availableVersions = ['2.0.0', '1.8.0', '999.0.0'];
