...
MWParserEnvironment.prototype.bumpCount = function(resource) {
...
MWParserEnvironment.prototype.printTimeProfile = function() {
	...
	for (var i = 0; i < sortable.length; i++) {
		k = sortable[i][0];
		v = sortable[i][1];
		let buf = k.padStart(35) + ': ' + JSON.stringify(v).padStart(7);
		if (this.counts[k]) {
...
MWParserEnvironment.prototype.switchToConfig = Promise.async(function *(prefix) {
	...
	if (!prefix || !parsoidConfig.mwApiMap.has(prefix)) {
		...
		if (env.confCache[prefix]) {
			...
			resultConf = yield ConfigRequest.promise(uri, env, proxy);
		} else {
...
MWParserEnvironment.prototype.makeLink = function(title) {
	return Util.sanitizeTitleURI(this.page.relativeLinkPrefix + this._titleToString(title));
};
 ...
 */
MWParserEnvironment.prototype.availableVersions = ['1.7.0', '2.0.0'];
...
/**
 * If non-null, the language variant which the HTML will be converted to
 * (if wt2html) or from (if html2wt).
 */
MWParserEnvironment.prototype.variantLanguage = null;
...
MWParserEnvironment.prototype.resolveContentVersion = function(v) {
	for (var i = 0; i < this.availableVersions.length; i++) {
		...
		if (semver.satisfies(a, '^' + v) &&
				...
				// fail hard the `strictAcceptCheck`.
				semver.gte(v, '1.6.0')) {
			return a;
		}
	}
