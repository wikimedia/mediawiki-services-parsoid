 ...
 *   inTemplate    : if true, indicates pipeline is processing content of a template
 ...
 *   inBlockToken  : used to influence paragraph-wrapping for transclusion content
 *                   (in current usage, this looks like this is always true
 *                    for sub-pipelines. once html+tidy setup is completed for
 *                    parser tests, paragraph-wrapping code will get a fixup and
...
ParserPipelineFactory.prototype.getPipeline = function ( type, options ) {
	...
	if ( this.pipelineCache[cacheKey].length ) {
		...
		pipe.resetState(true); // signal that we are resetting a sub-pipeline
