...
function WikitextSerializer( options ) {
	...
	this.trace = this.env.log.bind(this.env, "trace/wts");
}
...
function getAutoURLNowikiTest(href) {
	if (/\(/.test(href)) {
		if (!autoURLNowikiTest1) {
			// Newlines terminate url matching -- so, look for non-newline chars only
			// in the last part of the regexp
			autoURLNowikiTest1 = new RegExp("^([" + src1 + src2 + "]|[" + src2 + "]+([^" + src2 + "\n]))");
		}
		...
		if (!autoURLNowikiTest2) {
			...
			// Newlines terminate url matching -- so, look for non-newline chars only
			// in the last part of the regexp
			autoURLNowikiTest2 = new RegExp("^([" + src1 + src3 + "]|[" + src3 + "]+([^" + src3 + "\n]))");
		}
	...
	switch( node.nodeType ) {
		case node.ELEMENT_NODE:
			...
			if (state.selserMode) {
				...
				this.trace("NODE:", node.nodeName + "; SOL: " + state.onSOL);
			}
		...
		case node.TEXT_NODE:
			...
			this.trace("TEXT:",
				function() { return JSON.stringify(node.nodeValue); },
				"; SOL:", state.onSOL);
...
WSP.serializeDOM = function( body, chunkCB, finalCB, selserMode ) {
	...
	try {
		...
		this.env.log("trace/wts", function() {
			return "--- DOM --- \n" + body.outerHTML + "\n-----------";
		});
		...
		var chunkCBWrapper = function (chunk, node) {
			state.emitSepAndOutput(chunk, node, chunkCB);
			state.env.log("trace/wts", "--->", "OUT:", function() { return JSON.stringify(chunk); });
