	...
	return wt.split(/\n|$/).map(function(line) {
		...
		for (var j=1; j<n; j+=2) {
			...
			if (p[j]===']]') {
				...
				if (stack.pop()!=='{{') { return line; }
			} else if (p[j][0]==="'" && stack[stack.length-1]===p[j]) {
				...
				if (/^<nowiki\/>'/.test(p[j-1])) {
					...
					// line  : ''foo''<nowiki/>'<nowiki/>''bar''
					// pieces: ['', foo, '', <nowiki/>''<nowiki/>, '', bar, '']
		...
		if (/^<nowiki\/>'/.test(p[n-1])) {
			// SSS FIXME: Later on, we can optimize this further if this
			// proves to be a big deal.
			//
			// We can remove a subset of nowikis even in this scenario.
			return line;
		}
		...
		if (nowiki_1_seen && !nowiki_1_failed && !nowiki_2_seen) {
			// Only one scenario seen. Safe to strip out all the <nowiki/>s
			return line.replace(/<nowiki\/>/g, '');
		} else if (!nowiki_1_seen && nowiki_2_seen && !nowiki_2_failed) {
			// Only one scenario seen. Safe to strip out all the <nowiki/>s
			return line.replace(/<nowiki\/>/g, '');
		} else {
			// Both scenarios seen. We can only strip out nowikis in one scenario.
			// For now, we will bail.
			//
			// SSS FIXME: With some more smarts, we can pick one of the two scenarios
