...
WSP.handleImage = function ( node, state, cb ) {
	...
	var linkDP = (linkElt && linkElt.hasAttribute( 'data-parsoid' )) ?
		DU.getDataParsoid(linkElt) : null;

	var filename;
	if ( imgElt.hasAttribute( 'resource' ) ) {
		filename = this.serializedAttrVal( imgElt, 'resource' );
		if ( filename.modified || !filename.value ) {
			filename = imgElt.getAttribute( 'resource' ).replace( /^(\.\.?\/)+/, '' );
		} else {
			filename = filename.value;
		}
	} else {
		var src = imgElt.getAttribute( 'src' );
		// XXX reconstruct filename from src?
		console.assert(false); // XXX
	}
	...
	if ( linkElt.tagName === 'SPAN' && !this.hasOpt( opts, 'link' ) ) {
		opts.push( {
			ck: 'link',
			v: '',
			ak: mwAliases.img_link[0]
		} );
	} else if ( linkElt.tagName === 'A' && !this.hasOpt( opts, 'link' ) ) {
		if ( linkDP && linkDP.sa && linkDP.sa.href ) {
			linkinfo = this.serializedAttrVal( linkElt, 'href' );

			if ( linkinfo.modified || !linkinfo.value ) {
				linkinfo.value = linkElt.getAttribute( 'href' ).replace( /^(\.\.?\/)+/, '' );
			}
		} else if ( linkDP === null ) {
			linkinfo = {
				value: linkElt.getAttribute( 'href' ).replace( /^(\.\.?\/)+/, '' ),
				modified: true
			};
		} else {
	...
	var val, valign, valignPos, halignSet = false, ix;
	var currentOpt;

	for ( ix = 0; ix < classes.length; ix++ ) {
		var currentClass = classes[ix];
		currentOpt = {};
	...
	if ( !DU.hasClass( node, 'mw-default-size' ) ) {
		...
		var htAttr = imgElt.getAttribute( 'height' );
		var wdAttr = imgElt.getAttribute( 'width' );

		// Detect image size change
		if ( !outerDP.img.h || !outerDP.img.w || outerDP.img.h.toString() !== htAttr.toString() || outerDP.img.w.toString() !== wdAttr.toString() ) {
			...
			outerDP.img.htset = true;
			outerDP.img.wdset = true;
	...
	// Handle caption
	var dmw = outerElt ? DU.getDataMw(outerElt) : {};
	var isFigure = (outerElt && outerElt.tagName === 'FIGURE');
	...
	var isDefaultSize; // XXX isDefaultSize is never used
	if ( DU.hasClass( imgElt, 'mw-default-size' ) ) {
		isDefaultSize = true;
	}

	var wikitext = '[[' + filename;
	...
	if ( opts ) {
		for ( ix = 0; ix < opts.length; ix++ ) {
			var curOpt = this.handleOpt( node, opts[ix].ck, opts[ix].ak || null, opts[ix].v || null, state, accumOpts );
			// XXX curOpt never used!
		}
	}
...
WSP.emitSeparator = function(state, cb, node) {
	...
	if (origSepUsable && node && prevNode) {
