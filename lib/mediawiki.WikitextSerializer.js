...
var getLinkRoundTripData = function( env, node, state ) {
	...
	if (node.childNodes.length >= 1 && DU.allChildrenAreText(node)) {
		...
		rtData.content.string = dp.content ||
			(dp.sa && dp.sa.href) ||
...
WSP.handleImage = function ( node, state, cb ) {
	...
	var resource = this.serializedAttrVal( imgElt, 'resource' );
	...
	var alt = this.serializedAttrVal( imgElt, 'alt' );

	// Fetch the lang (if any)
	var lang = this.serializedAttrVal( imgElt, 'lang' );
	...
	var wh = this.serializedAttrVal( imgElt, 'height' ),
		...
		sizeUnmodified = !ww.modified && !wh.modified,
...
WSP.linkHandler = function(node, state, cb) {
	...
	if ( linkData.type !== null && linkData.target.value !== null  ) {
		...
		if (/^mw:WikiLink$/.test( linkData.type ) ||
			...
			if ( this._isSimpleWikiLink(env, dp, target, linkData) ) {
				// Simple case
