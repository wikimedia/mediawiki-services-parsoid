...
WSP._serializeTextNode = function(node, state, cb) {
	...
	if (state.escapeText && (escapes.prefix || escapes.suffix)) {
		state.escapeText = false;
		if (escapes.prefix) {
			cb(ConstrainedText.cast(escapes.prefix, node), node);
		}
		state.escapeText = true;
		cb(ConstrainedText.cast(res, node), node);
		state.escapeText = false;
		if (escapes.suffix) {
			cb(ConstrainedText.cast(escapes.suffix, node), node);
		}
	} else {
		cb(new ConstrainedText({ text: res, node: node, prefix: escapes.prefix, suffix: escapes.suffix }), node);
		state.escapeText = false;
	}
