...
WSP.handleImage = function ( node, state, cb ) {
	...
	var resource;
	if ( imgElt.hasAttribute( 'resource' ) ) {
		resource = this.serializedAttrVal( imgElt, 'resource' );
		if ( resource.modified || !resource.value ) {
			resource.value = imgElt.getAttribute( 'resource' );
			resource.fromsrc = false;
		}
	} else {
		// from non-parsoid HTML: try to reconstruct resource from src?
		var src = imgElt.getAttribute( 'src' );
		if (!src) {
			console.error( "WARNING: In WSP.handleImage, img does not have resource or src: ");
			console.error( node.outerHTML );
			return cb( '', node );
		}
		if (/^https?:/.test(src)) {
	...
	if ( linkElt && linkElt.hasAttribute('href') ) {
		link = this.serializedAttrVal( linkElt, 'href' );
		if ( (!link.fromsrc) &&
			 linkElt.getAttribute('href') === imgElt.getAttribute('resource')) {
			// default link: same place as resource
			link = resource;
		} else if ( link.modified || !link.value ) {
			link.value = linkElt.getAttribute( 'href' );
			link.fromsrc = false;
		}
		if ( !link.fromsrc ) {
	...
	if (imgElt.hasAttribute('alt')) {
		alt = this.serializedAttrVal( imgElt, 'alt' );
		if ( alt.modified || !alt.value ) {
			alt.value = imgElt.getAttribute( 'alt' );
			alt.fromsrc = false;
		}
	}
	...
	var type = (outerElt.getAttribute('typeof') || '').
		match(/(?:^|\s)(mw:Image\S*)/);
	switch ( type ? type[1] : null ) {
		case 'mw:Image/Thumb':
			nopts.push( {
				ck: 'thumbnail',
				ak: mwAliases.img_thumbnail
			} );
			break;
	...
	if ( !(outerElt && outerElt.classList.contains('mw-default-size')) ) {
		outerDP.img = outerDP.img || {};

		// Get the user-specified width/height from wikitext
		var wh = this.serializedAttrVal( imgElt, 'height' ),
			ww = this.serializedAttrVal( imgElt, 'width' ),
			bbox = null,
			sizeString;

		var sizeStringOpt = outerDP.optList ?
			outerDP.optList.find(function(o) { return o.ck === 'width'; }) : null;

		if (sizeStringOpt) {
			sizeString = sizeStringOpt.ak;
		}

		if (!ww.modified && !wh.modified && sizeString) {
...
WSP._buildExtensionWT = function(state, node, dataMW) {
	...
	if (!dataMW.body) {
		...
		if (typeof dataMW.body.html === 'string') {
			...
			this.env.log("error", "ERROR: extension src unavailable for: " + node.outerHTML );
		}
			...
			handle: function () {
				...
				if (/(?:^|\s)mw:Transclusion(?=$|\s)/.test(typeOf)) {
					...
					if (dataMW) {
						...
						self.env.log("error", "ERROR: No data-mw for" + node.outerHTML );
						src = dp.src;
					}
					...
					self.env.log("error", "ERROR: Should not have come here!");
				}
				...
				if (src !== undefined) {
					...
					var errors = ["ERROR: No handler for: " + node.outerHTML];
					errors.push("Serializing as HTML.");
					self.env.log("error", errors);
...
WSP.mergeConstraints = function (oldConstraints, newConstraints) {
	...
	if (res.min > res.max) {
		...
		res.max = res.min;
		console.error('Incompatible constraints (merge):', res, this.loggableConstraints(oldConstraints), this.loggableConstraints(newConstraints));
	}
	return res;
};
