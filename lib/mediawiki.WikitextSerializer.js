...
function stripUnnecessaryQuoteNowikis(wt) {
	// no-quotes OR matched quote segments with 5/3/2 quotes OR single-quote char
	//
	...
	// - allow [[..]]
	var testRE = /^[^']+$|^[^']*(('''''(\[\[(\w|\s|\|)+\]\]|[^\[\{<'])+'''''|'''(\[\[(\w|\s|\|)+\]\]|[^\[\{<'])+'''|''(\[\[(\w|\s|\|)+\]\]|[^\[\{<'])+''|')([^']+|$))+('|$)$/;

	return wt.split(/\n|$/).map(function(line) {
		// Optimization: skip test if there are no <nowiki/>s here to remove.
		if (!/<nowiki\/>/.test(line)) {
			return line;
		}

		// * Strip out nowiki-protected strings since we are only interested in
		//   quote sequences that correspond to <i>/<b> tags.
		// * Find segments separated by <nowiki/>s.
		// * If all the segments contain balanced i/b tags, and the <nowiki/>
		//   separated a quote and an i/b tag, we can remove all the <nowiki/>s
		var pieces = line.replace(/<nowiki>.*?<\/nowiki>/g, '').split(/<nowiki\/>/);
		var n = pieces.length;
		for (var i = 0; i < n; i++) {
			// Since we are okay with single quotes in the middle, strip those
			// out so as not to have to deal with them in the testRE regexp above.
			// We need to leave a trailing ' behind since we test for it below.
			var p = pieces[i].replace(/(^|[^'])'(?=[^'])/g, "$1");
			if (!testRE.test(p) ||
				// All but the first piece should start with ''
				(i > 0 && !/^''/.test(p)) ||
				// All but the last piece should end in a single ' char
				// since that is the only scenario we are optimizing for here
				(i < n-1 && !/(^|[^'])'$/.test(p)))
			{
				return line;
			}
