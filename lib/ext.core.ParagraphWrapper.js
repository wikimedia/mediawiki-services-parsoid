...
ParagraphWrapper.prototype._getTokensAndReset = function (res) {
	...
	this.nlWsTokens = [];
	this.newLineCount = 0;
	this.env.log("trace/p-wrap", "---->  ", resToks);
	return resToks;
};
...
ParagraphWrapper.prototype.onNewLineOrEOF = function (  token, frame, cb ) {
	this.env.log("trace/p-wrap", "NL    |", function() { return JSON.stringify(token); });
	...
	if (token.constructor === EOFTk) {
		...
		this.hasOpenHTMLPTag = false;
		this.reset();
		this.env.log("trace/p-wrap", "---->  ", function() { return JSON.stringify(res); });
		return { tokens: res };
	} else {
		this.newLineCount++;
		this.nlWsTokens.push(token);
...
ParagraphWrapper.prototype.processPendingNLs = function (isBlockToken) {
	...
	this.env.log("trace/p-wrap", "NL-count:", newLineCount);
...
ParagraphWrapper.prototype.onAny = function ( token, frame ) {
	this.env.log("trace/p-wrap", "any   |", function() { return JSON.stringify(token); });
