...
ParagraphWrapper.prototype.reset = function() {
	...
	//
	//   non-nl-tokens        (from previous lines if newline-token-count > 0)
	//   newline-token-count  (>= 0)
...
ParagraphWrapper.prototype._getTokensAndReset = function (res) {
	...
	if (!res && this.newLineCount > 0) {
		this.env.log("error/p-wrap",
			"Failed assertion in _getTokensAndReset: newline-count:", this.newLineCount,
...
ParagraphWrapper.prototype.closeOpenPTag = function(out) {
	if ( this.hasOpenPTag ) {
		...
		for ( var i = out.length - 1; i > -1; i-- ) {
			if ( out[i].name === "meta" ) {
				var typeOf = out[i].getAttribute("typeof");
			...
			// Not a transclusion meta, check sol.
			if ( !Util.isSolTransparent(out[i]) ) {
				// Leave sol-transparent tags out of the p-wrapping
				out.splice( i+1, 0, new EndTagTk('p') );
				break;
			}
		}
	...
	if (this.currLine.hasWrappableTokens) {
		...
		for (var i = 0, n = toks.length; i < n; i++) {
			...
			if (Util.isBlockToken(t)) {
				...
				this.closeOpenPTag(newToks);
			} else if (blockTagCount === 0 && !Util.isSolTransparent(t) && !this.hasOpenPTag) {
