...
WikiLinkHandler.prototype.renderFile = function (token, frame, cb, target)
{
	...
	function handleResponse( linkTitle, err, data ) {
		...
		if ( oHash.lang ) {
			img.addNormalizedAttribute( 'lang', oHash.lang, langBackup );
		}
		...
		if ( hasImageLink ) {
			if ( oHash.link && urlParser.tokenizeURL( oHash.link )) {
				...
				innerContain.addNormalizedAttribute( 'href', linkTitle.makeLink(), linkBackup );
			}
		}

		// zero-length string indicates that height (width) option was
		// missing in the wikitext
		if ( height ) {
			img.addNormalizedAttribute(
				'height', String( height ),
				(oHash.height===null) ? '' : oHash.height
			);
		}
	...
	for( i = 0, l = content.length; i<l; i++ ) {
		...
		if ( optInfo.ck === 'link' ) {
			// Holds the shadow info for the link option until we can save it
			linkBackup = oContent.vsrc;
		}

		if ( optInfo.ck === 'lang' ) {
			// Holds the shadow info for the lang option until we can save it
