 ...
 */
function handleSize( info, dataAttribs ) {
	var width, height;
	if ( info.height ) {
		height = info.height;
	}
...
WikiLinkHandler.prototype.handleImageInfo = function( cb, token, opts, err, data ) {
	if (err || !data) {
	...
	var	env = this.manager.env,
		title = opts.title.v,
		format = opts.format && opts.format.v,
	...
	var size = handleSize( info, dataAttribs );
	...
	if ( hasImageLink ) {
		if ( opts.link ) {
			...
			if (this.urlParser.tokenizeURL( opts.link.v )) {
				...
				title = Title.fromPrefixedText( env, opts.link.v );
	...
	if ( opts.caption !== undefined ) {
		if ( useFigure ) {
			...
			dataMWObj.caption = opts.caption.src;
			dataMW = JSON.stringify(dataMWObj);
		}
	}

	if (dataMW) {
		container.addAttribute("data-mw", dataMW);
	}
...
WikiLinkHandler.prototype.renderFile = function (token, frame, cb, target)
{
	...
	for( i = 0, l = content.length; i<l; i++ ) {
		...
		if ( oText.constructor !== String ) {
			...
			maybeOText = stringifyOptionTokens( oText, '', env );
			if ( maybeOText !== null ) {
				oText = maybeOText;
			}
		}
		...
		if ( optInfo.s === true ) {
			...
			if ( optInfo.ck === 'width' ) {
				var maybeSize = optInfo.v.match( /^(\d*)(?:x(\d+))?\s*$/ );
				if ( maybeSize !== null ) {
	...
	if ( queueKey in env.pageCache ) {
		this.handleImageInfo( cb, token, opts, null, env.pageCache[ queueKey ], cb );
		return;
	}
	...
	env.requestQueue[ queueKey ].once( 'src', this.handleImageInfo.bind( this, cb, token, opts ) );
	cb( { async: true } );
};
