...
WikiLinkHandler.prototype.onRedirect = function(token, frame, cb) {
	var rlink = new SelfclosingTagTk('link', [], Util.clone(token.dataAttribs));
	...
	var tokens = [rlink];
	if (target.title && target.title.ns.isCategory() && !target.fromColonEscapedText) {
...
WikiLinkHandler.prototype.handleImageInfo = function(cb, token, title, opts, optSources, err, data) {
	...
	var setupDataMW = function(obj, str) {
