...
function isRecognizedSpanWrapper(type) {
	return type &&
		type.split(/\s+/).find(function(t) { return genContentSpanTypes.has(t); }) !== undefined;
}
...
function hoistLinks( node, rtl ) {
	...
	while ( sibling ) {
		...
		if ( !DU.isSolTransparentLink( sibling ) ) {
			if ( fragment ) {
				// soak up the remaining non-content nodes (exclude sibling)
				move = node[first];
				while ( move !== sibling ) {
					fragment.insertBefore( move, rtl ? fragment.firstChild : null );
					move = node[first];
				}
		...
		// hoist everything up to the link (include sibling)
		move = null;
		while ( move !== sibling ) {
			move = node[first];
			fragment.insertBefore( move, rtl ? fragment.firstChild : null );
	...
	return {
		handle: function(node, state, cb) {
			...
			if ( node.childNodes.length ) {
				...
				if ( beforeFragment && beforeFragment.childNodes.length ) {
					state.serializeChildren( beforeFragment, cb, null );
				}
			}
			...
			if ( afterFragment && afterFragment.childNodes.length ) {
				state.serializeChildren( afterFragment, cb, null );
			}
		},
		sepnls: {
...
function isTplListWithoutSharedPrefix( node ) {
	...
	// If the first part is a string, template ranges were expanded to include
	// this list element. That may be trouble.
	var dataMw = DU.getDataMw( node );
	if ( !dataMw.parts || typeof dataMw.parts[0] !== "string" ) {
		return false;
	}
	// Less than two bullets indicates that a shared prefix was not assigned to
	// this element. A safe indication that we should call getListsBullets on
	...
	th: {
		handle: function (node, state, cb, wrapperUnmodified) {
			var dp = DU.getDataParsoid( node ), res,
				...
				attrSepSrc = usableDP ? dp.attrSepSrc : null;
		...
		sepnls: {
			before: function (node, otherNode) {
				if ( DU.isSolTransparentLink(node) &&
					((DU.isNewElt(node) && otherNode.nodeName !== 'BODY') ||
