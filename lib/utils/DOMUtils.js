...
DOMUtils = DU = {
	...
	storeDataAttribs: function(node, discardDataParsoid, keepTmp) {
		console.assert(!(discardDataParsoid && keepTmp));
		if (!DU.isElt(node)) {
			return;
		}
	...
	isInlineImage: function(n) {
		return DU.isElt(n) && (/\bmw:Image\b/).test(n.getAttribute("typeof")) &&
			n.nodeName === 'SPAN';
	},
			...
			while (node) {
				if (DU.isElt(node)) {
					...
					if ((/(?:^|\s)(?:mw:(?:Transclusion(?=$|\s)|Extension\/))/
						...
						if (key) {
							expAccum[key] = DU.makeExpansion(nodes);
						}
						node = lastItem(nodes);
					} else {
	...
	applyDataParsoid: function(doc, dp) {
...
DOMUtils.ppToXML = function(node, options) {
	DU.visitDOM(node, DU.storeDataAttribs, false, options && options.keepTmp);
...
DOMUtils.extractDpAndSerialize = function(node, options) {
	...
	var dpScriptElt = (DU.isBody(node) ? node.ownerDocument : node).getElementById('mw-data-parsoid');
 ...
 * @param {Object} [options]
 */
DOMUtils.dumpDOM = function(root, title, options) {
	options = options || {};
