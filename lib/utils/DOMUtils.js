...
DU = DOMUtils = {
	...
	storeDataAttribs: function(node, options) {
		...
		if (DU.validDataMw(node)) {
			if (options.storeInPageBundle && options.env &&
					// The pagebundle didn't have data-mw before 2.x
					semver.satisfies(options.env.contentVersion, '^2.0.0')) {
				data = data || {};
	...
	getWrapperTokens: function(nodes, opts) {
		function makeWrapperForNode(node, wType) {
			var wrapperName;
			if (wType === 'BLOCK' && !DU.isBlockNode(node)) {
				wrapperName = 'DIV';
	...
	isDOMFragmentWrapper: function(node) {
		function hasRightType(n) {
			return (/(?:^|\s)mw:DOMFragment(?=$|\s)/).test(n.getAttribute("typeof"));
		}
		function previousSiblingIsWrapper(sibling, abt) {
			return sibling &&
				DU.isElt(sibling) &&
	 ...
	 * @returns {String} DOM-escaped comment
	 */
	 ...
	 * @returns {String} Wikitext-escaped comment
	 */
	 ...
	 * @returns {Number} The wikitext length necessary to encode this comment,
...
DOMUtils.dumpDOM = function(rootNode, title, options) {
	options = options || {};
	if (options.storeDiffMark) { console.assert(options.env); }
...
DOMUtils.normalizeHTML = function(source) {
	try {
		var body = DU.normalizeIEW(DU.parseHTML(source).body);
...
DOMUtils.normalizeOut = function(out, parsoidOnly) {
	...
	var body = DU.normalizeIEW(out, stripTypeof, parsoidOnly);
	...
	.then(function(result) {
		...
		wikiLinks.forEach(function(a) {
			...
			if (data === undefined) {
				...
				var title = env.makeTitleFromURLDecodedStr(k, undefined, true);
				if (title !== null) {
					var ns = title.getNamespace();
					if (ns.isUser() || ns.isUserTalk()) {
						var key = ':' + title._key.replace(/_/g, ' ');
...
DOMUtils.stripSectionTags = function(node) {
	...
	while (n) {
		...
		if (DU.isElt(n)) {
			...
			// Strip <section> tags (except if they have template info)
			if (n.nodeName === 'SECTION' && !DU.findFirstEncapsulationWrapperNode(n)) {
