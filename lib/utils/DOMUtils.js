...
DU = DOMUtils = {
	...
	isRenderingTransparentNode: function(node) {
		...
		return DU.isComment(node) ||
			...
			(node.nodeName === 'META' &&
				// FIXME: This might no longer be necessary after
				// changes to Cite to remove special-case handling
				// to pass marker metas.
				//
				// Similar to Util.isSolTransparent, we treat all mw:Extension/*
				// metas as non-SOL.  This is necessary since we do p-wrapping
				// on the dom before fragment unpacking, unlike in the php parser.
