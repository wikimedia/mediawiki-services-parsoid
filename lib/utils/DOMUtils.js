...
DU = DOMUtils = {
	...
	mkTagTk: function(node) {
		...
		return new TagTk(node.nodeName.toLowerCase(), attribKVs, this.getDataParsoid(node));
	},
	...
	mkEndTagTk: function(node) {
		var attribKVs = this.getAttributeKVArray(node);
		return new EndTagTk(node.nodeName.toLowerCase(), attribKVs, this.getDataParsoid(node));
	},
