...
class ContentUtils {
	...
	static reinsertFosterableContent(node) {
		if (!DOMUtils.isElt(node)) {
			return;
		}

		const dataFosteredAttr = node.getAttribute('data-fostered');
		if (dataFosteredAttr) {
			const tunneledContent = JSON.parse(dataFosteredAttr);
			let c = tunneledContent.shift();
			let idx = 0;
			let child = node.firstChild;
			while (c) {
				console.assert(child, 'Bad DOM encountered. Cannot reinsert fosterable content!');
				const next = child.nextSibling;
				if (c.idx === idx) {
					console.assert(DOMUtils.isComment(child), 'Expected placeholder comment!');
					if (c.text) {
						node.replaceChild(node.ownerDocument.createTextNode(c.content), child);
					} else {
						// parse & transfer over
						const fcBody = DOMUtils.parseHTML(c.content).body;
						DOMUtils.migrateChildrenBetweenDocs(fcBody, node, child);
						node.removeChild(child);
					}
					c = tunneledContent.shift();
				}
				child = next;
				idx++;
			}
	...
	static ppToDOM(env, html, options) {
		...
		if (options.reinsertFosterableContent) {
			DOMUtils.visitDOM(node, (node, ...args) => {
				// untunnel fostered content
				this.reinsertFosterableContent(node);
			...
			// load data attribs
			DOMUtils.visitDOM(node,
				(...args) => DOMDataUtils.loadDataAttribs(...args),
				options.markNew);
		}
