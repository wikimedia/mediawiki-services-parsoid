...
class ContentUtils {
	...
	static stripUnnecessaryWrappersAndFallbackIds(node) {
		var n = node.firstChild;
		while (n) {
			var next = n.nextSibling;
			if (DOMUtils.isElt(n)) {
				// Recurse into subtree before stripping this
				this.stripUnnecessaryWrappersAndFallbackIds(n);
	...
	static dumpDOM(rootNode, title, options) {
		options = options || {};
		if (options.storeDiffMark || options.dumpFragmentMap) {
			console.assert(options.env);
		}
