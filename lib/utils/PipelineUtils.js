...
var PipelineUtils = {
	...
	getWrapperTokens: function(nodes, opts) {
		...
		if (!DU.isElt(node)) {
		...
		} else if (node.hasChildNodes() || wrapperName !== node.nodeName) {
			...
			if (wrapperName === node.nodeName) {
				const dp = Object.assign({}, DU.getDataParsoid(node), {
					tsr: undefined,
				});
				DU.setDataParsoid(workNode, dp);
			}
		} else {
			workNode = node;
		}
	 ...
	 *    See encapsulateExpansionHTML's doc. for more info about these options.
	 */
	buildDOMFragmentTokens: function(env, token, body, opts) {
	...
	makeExpansion: function(env, nodes) {
		nodes.forEach(function(n) {
			DU.visitDOM(n, DU.storeDataAttribs);
		});
