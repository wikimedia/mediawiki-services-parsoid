...
class Bag {
	...
	getObject(docId) {
		return this.dataobject.get(docId);
	}
...
class DOMDataUtils {
	...
	static hasTypeOf(node, type) {
		...
		return typeOfs.split(/\s+/g).includes(type);
	}
	...
	static applyPageBundle(doc, pb) {
		DOMUtils.visitDOM(doc.body, (node) => {
			if (DOMUtils.isElt(node)) {
				...
				if (pb.parsoid && pb.parsoid.ids && pb.parsoid.ids.hasOwnProperty(id)) {
					this.setJSONAttribute(node, 'data-parsoid', pb.parsoid.ids[id]);
				}
				if (pb.mw && pb.mw.ids && pb.mw.ids.hasOwnProperty(id)) {
