...
class DOMDataUtils {
	...
	static getNodeData(node) {
		...
		const env = node.ownerDocument.env;
		const docId = node.getAttribute(this.DataObjectAttrName());
		const dataobject = env.dataobject.get(docId);
	...
	static setNodeData(node, data) {
		const env = node.ownerDocument.env;
		const docId = String(env.docId);
		env.dataobject.set(docId, data);
	...
	static storeInPageBundle(node, env, data) {
		...
		var pb = document.env.pagebundle;
