...
class DOMDataUtils {
	...
	static storeInPageBundle(node, env, data) {
		// FIXME: DOMDataUtils.f() instead of this.f() is to work around T209772
		var uid = node.getAttribute('id');
		var document = node.ownerDocument;
		var pb = DOMDataUtils.getDataParsoid(document).pagebundle;
		...
		if (!uid) {
			...
			DOMDataUtils.addNormalizedAttribute(node, 'id', uid, origId);
		}
	...
	static injectPageBundle(doc, obj) {
		...
		DOMDataUtils.addAttributes(script, {
			id: 'mw-pagebundle',
	...
	static applyPageBundle(doc, pb) {
		DOMUtils.visitDOM(doc.body, function(node) {
			if (DOMUtils.isElt(node)) {
				var id = node.getAttribute('id');
				if (pb.parsoid.ids.hasOwnProperty(id)) {
					DOMDataUtils.setJSONAttribute(node, 'data-parsoid', pb.parsoid.ids[id]);
				}
				if (pb.mw && pb.mw.ids.hasOwnProperty(id)) {
					...
					if (node.getAttribute('data-mw') === null) {
						DOMDataUtils.setJSONAttribute(node, 'data-mw', pb.mw.ids[id]);
					}
				}
			}
		});
	...
	static loadDataAttribs(node, markNew) {
		...
		var dp = DOMDataUtils.getJSONAttribute(node, 'data-parsoid', {});
		if (markNew) {
			if (!dp.tmp) { dp.tmp = {}; }
		...
		DOMDataUtils.setDataMw(node, DOMDataUtils.getJSONAttribute(node, 'data-mw', undefined));
	...
	static storeDataAttribs(node, options) {
		...
		if (!discardDataParsoid) {
			...
			if (options.storeInPageBundle) {
				...
				DOMDataUtils.setJSONAttribute(node, 'data-parsoid', dp);
			}
		}
		...
		if (DOMDataUtils.validDataMw(node)) {
			if (options.storeInPageBundle && options.env &&
				...
				data = data || {};
				data.mw = DOMDataUtils.getDataMw(node);
			} else {
				DOMDataUtils.setJSONAttribute(node, 'data-mw', DOMDataUtils.getDataMw(node));
			}
		}
		// Store pagebundle
		if (data !== null) {
			DOMDataUtils.storeInPageBundle(node, options.env, data);
		}
	}
}
