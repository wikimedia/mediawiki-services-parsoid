...
var Util = {
	...
	setTemplatingAndProcessingFlags: function(parsoidConfig, opts) {
		...
		if (opts.apiURL) {
			...
			parsoidConfig.mwApi.push({ prefix: 'customwiki', uri: opts.apiURL });
		}
		...
		if (opts.lint) {
			...
			if (!parsoidConfig.linter) {
				// Late require since we have a circular require involved
				var ParsoidConfig = require('../config/ParsoidConfig.js').ParsoidConfig;
				parsoidConfig.linter = Util.clone(ParsoidConfig.prototype.linter);
			}
	...
	*/
	...
	sanitizeTitleURI: function(title) {
		...
		if (anchor !== null) {
			title += '#' + Sanitizer.escapeId(anchor, { noninitial: true });
		}
		return title;
	},
	...
	sanitizeURI: function(s) {
		...
		s = host + path;
		if (anchor !== null) {
			s += '#' + Sanitizer.escapeId(anchor, { noninitial: true });
		}
		return s;
	},
