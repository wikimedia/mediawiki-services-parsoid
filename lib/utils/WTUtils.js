...
class WTUtils {
	...
	static isRenderingTransparentNode(node) {
		...
		return DOMUtils.isComment(node) ||
			...
			(node.nodeName === 'META' &&
				...
				(/(mw:StartTag)|(mw:EndTag)/.test(node.getAttribute('typeof') || '') || DOMDataUtils.getDataParsoid(node).stx !== 'html')) ||
	...
	static fosterCommentData(typeOf, attrs) {
	...
	static reinsertFosterableContent(env, node) {
