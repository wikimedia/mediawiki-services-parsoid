...
var escapeWikiLinkContentString = function(linkTarget, state, node, suppressLinkTest) {
	...
	if (!suppressLinkTest &&
		(!state.env.isValidLinkTarget(linkTarget) || /[\|]/.test(linkTarget)))
	{
...
function serializeAsWikiLink(node, state, linkData, cb) {
		...
		if (prevNode && DU.isElt(prevNode) && DU.hasNodeName(prevNode, "link") &&
			...
			contentParts = splitLinkContentString(
					Util.decodeURI( targetParts[2] )
						.replace( /%23/g, '#' )
						// gwicke: verify that spaces are really double-encoded!
						.replace( /%20/g, ' '),
					dp );
	...
	if ( linkData.isRedirect ) {
		...
		if (target.modified || !target.fromsrc) {
			...
			escapedRes = escapeWikiLinkContentString(linkTarget, state, node);
		...
		cb( new WikiLinkText( linkData.prefix + '[[' + linkTarget + ']]', node, wiki, linkData.type ), node );
		return;
		...
		if (!target.modified && !linkData.contentModified) {
			...
			escapedRes = escapeWikiLinkContentString(linkData.content.string,
					state, node);
			linkTarget = addColonEscape(env, escapedRes.linkTarget, linkData);
		...
		cb( new WikiLinkText( linkData.prefix + '[[' + linkTarget + ']]' + linkData.tail, node, wiki, linkData.type ), node );
		return;
	} else {
		...
		if ( linkData.contentNode ) {
			contentSrc = state.serializeLinkChildrenToString(
					...
					state.serializer.wteHandlers.wikilinkHandler, false);
			...
			if (linkData.content.fromsrc) {
				...
				// Don't test for bad-title text since we are not
				// using that escapedRes.linkTarget for the output.
				escapedRes = escapeWikiLinkContentString(linkData.content.string || '',
						state, node, !!"suppressLinkTest");
...
function serializeAsExtLink(node, state, linkData, href, cb) {
	var env = state.env,
		wiki = env.conf.wiki,
		target = linkData.target,
		dp = DU.getDataParsoid(node);
			...
			(target.modified || linkData.contentModified || dp.stx === 'url'))
	{
		...
		var magicLinkMatch = wiki.ExtResourceURLPatternMatcher.match(href);
		// Fully serialize the content
		contentStr = state.serializeLinkChildrenToString(node,
				state.serializer.wteHandlers.aHandler, false);
...
var linkHandler = function(node, state, cb) {
	...
	if ( linkData.type !== null && linkData.target.value !== null  ) {
		...
		if (/\b(mw:ExtLink|mw:PageProp\/Language)\b/.test(linkData.type)) {
			...
			if (interWikiMatch &&
				...
				if (oldPrefix && (
						...
						(wiki.interwikiMap.get(oldPrefix[1].toLowerCase().trim().replace(/^:/, '')) || {}).url ===
						(wiki.interwikiMap.get(interWikiMatch[0].toLowerCase().replace(/^:/, '')) || {}).url
						))
				{
		...
		if (/^mw:WikiLink$/.test(linkData.type) || Util.solTransparentLinkRegexp.test(linkData.type)) {
