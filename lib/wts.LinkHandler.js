		...
		if (/\b(mw:ExtLink|mw:PageProp\/Language)\b/.test(linkData.type)) {
			...
			if (interWikiMatch &&
				...
				if (oldPrefix && (
						...
						(env.conf.wiki.interwikiMap[oldPrefix[1].toLowerCase().trim().replace(/^:/, '')] || {}).url ===
				...
				} else if (env.conf.wiki.iwp.replace(/wiki/, '') === interWikiMatch[0].substr(1)) {
					// - conf.wiki.iwp will be "enwiki", "dewiki", etc.
