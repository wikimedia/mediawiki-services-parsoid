...
function serializeAsExtLink(node, state, linkData, cb) {
	...
	if (contentStr &&
		...
		var magicLinkMatch = wiki.ExtResourceURLPatternMatcher.match(decodeURI(linkData.origHref));
...
var linkHandler = function(node, state, cb) {
	...
	if (wiki.ExtResourceURLPatternMatcher.match(decodeURI(linkData.origHref))) {
