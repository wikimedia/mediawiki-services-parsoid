...
function findTopLevelNonOverlappingRanges(document, env, docRoot, tplRanges) {
		...
		while (n !== docRoot) {
			...
			if ( data && data.tmp_tplRanges ) {
				if (n !== r.start) {
					...
					// Record a range in which 'r' is nested.
	...
	for (i = 0; i < numRanges; i++) {
		...
		env.log("trace/tplwrap/merge", function() {
				...
				msg += "\nrange " + r.id + "; r-start: " + r.start.outerHTML + "; DP: " + JSON.stringify(DU.getDataParsoid(r.start));
				msg += "\nrange " + r.id + "; r-end: " + r.end.outerHTML + "; DP: " + JSON.stringify(DU.getDataParsoid(r.end));
				msg += "\n----------------------------------------------";
				return msg;
			});
