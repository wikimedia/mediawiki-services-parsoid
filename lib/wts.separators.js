...
function makeSeparator(state, sep, nlConstraints) {
	...
	if (state.serializer.debugging) {
		...
		state.serializer.trace('makeSeparator', sep, origSep, minNls, sepNlCount, constraints);
	}
...
function mergeConstraints(env, oldConstraints, newConstraints) {
	...
	if (res.min > res.max) {
		...
		res.max = res.min;
		env.log("warning", 'Incompatible constraints (merge):', res, loggableConstraints(oldConstraints), loggableConstraints(newConstraints));
	}
	return res;
}
...
function makeSepIndentPreSafe(state, sep, nlConstraints) {
	...
	if (state.serializer.debugging) {
		var constraints = Util.clone(nlConstraints);
		constraints.constraintInfo = undefined;
		state.serializer.trace('makePreSafe  ', sep, constraints);
	}
