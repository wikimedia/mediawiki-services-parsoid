	...
	if (sepNlsHandlerB) {
		...
		if (cb.min !== undefined) {
			if (nlConstraints.max !== undefined && nlConstraints.max < cb.min) {
				...
				env.log("warning", "Incompatible constraints 1:", nodeA.nodeName,
		...
		if (cb.max !== undefined) {
			if (nlConstraints.min !== undefined && nlConstraints.min > cb.max) {
				// Conflict, warn and let nodeB win.
				env.log("warning", "Incompatible constraints 2:", nodeA.nodeName,
						nodeB.nodeName, loggableConstraints(nlConstraints));
...
var handleSeparatorText = function(node, state) {
	...
	if (!state.inIndentPre && DU.isText(node)) {
		if (node.nodeValue.match(separatorREs.pureSepRE)) {
			...
			var match = node.nodeValue.match(separatorREs.sepPrefixWithNlsRE);
			if (match) {
				state.sep.src = (state.sep.src || '') + match[0];
...
function makeSeparator(state, sep, nlConstraints) {
	...
	if (minNls > 0 && sepNlCount < minNls) {
		...
		if (sepType === 'parent-child' &&
				!DU.isContentNode(DU.firstNonDeletedChildNode(nodeA)) &&
				!(Consts.ChildTableTags.has(nodeB.nodeName) &&
...
var WS_COMMENTS_SEP_REPLACE_REGEXP = new RegExp(WS_COMMENTS_SEP_TEST_REGEXP, 'g');
...
function makeSepIndentPreSafe(state, sep, nlConstraints) {
	...
	//
	// SSS FIXME: Quick hack for now. We need to probably add this
	// info to mediawiki.wikitext.constants.js or parsoid-config and
	// also see what to do about the noPre option in the wt2html pipeline
	// which has the same purpose.
	if (state.serializer.options.extName !== 'ref' && !state.inIndentPre &&
		(NL_WS_COMMENTS_SEP_REGEXP.test(sep)
		|| (constraintInfo.onSOL && WS_COMMENTS_SEP_TEST_REGEXP.test(sep)))
	) {
		...
		if (nodeB && DU.precedingSpaceSuppressesIndentPre(nodeB, origNodeB)) {
			...
			while (nodeB && (DU.isMarkerMeta(nodeB, "mw:DiffMarker") ||
					DU.emitsSolTransparentSingleLineWT(state.env, nodeB))) {
		...
		var stripLeadingSpace = constraintInfo.onSOL && nodeB && Consts.SolSpaceSensitiveTags.has(nodeB.nodeName);
...
var buildAndEmitSep = function(state, cb, node) {
	 ...
	 *     (see the extensive comment in emitSepAndOutput in wts.SerializerState.js)
	 *
	...
	var origSepUsable = node && prevNode && node !== prevNode &&
		src && (state.rtTestMode ||
			...
			!DU.nextToDeletedBlockNodeInWT(state.env, prevNode, true) &&
	...
	if (origSepUsable) {
		if (!DU.isElt(prevNode)) {
			...
			if (!prevNode.nextSibling &&
				...
				dsrA = DU.getDataParsoid(prevNode.parentNode).dsr;
					...
					(state.rtTestMode || // no changes in rt testing
		...
		if (!dsrA) {
			...
			dsrB = DU.isElt(node) ? DU.getDataParsoid(node).dsr : null;
		}
