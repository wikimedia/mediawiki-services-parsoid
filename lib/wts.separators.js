...
function makeSepIndentPreSafe(state, sep, nlConstraints) {
		...
		(constraintInfo.onSOL && sep.match(/ +(<!--(?:[^\-]|-(?!->))*-->[^\n]*)?$/g)))))
	{
		...
		if (sepType && DU.precedingSpaceSuppressesIndentPre(nodeB, origNodeB)) {
			...
			// If nodeA uses HTML syntax, obviously it is not zero width!
			while (DU.isZeroWidthWikitextElt(nodeA)) {
