...
DOMUtils = DU = {
	...
	traverseWithTplOrExtInfo: function( func, node, env, options, atTopLevel, tplInfo ) {
		...
		while ( node ) {
			...
			if (DU.isElt(node)) {
				...
				// Strictly speaking, the !tplInfo check isn't necessary since
				...
				if ( !tplInfo && DU.isTplOrExtToplevelNode(node) &&
					...
					tplInfo = {
						first: node,
						last: DU.getAboutSiblings( node, about ).last(),
						dsr: DU.getDataParsoid( node ).dsr,
						// Give funcs a chance to clear the tplInfo after
						// processing.
						clear: false
					};
				}
			...
			// node.nextSibling returns a node or null, so explicitly test
			...
			if ( possibleNext !== undefined ) {
				...
				DU.traverseWithTplOrExtInfo( func, node.firstChild, env, options, atTopLevel, tplInfo );
			}

			// Clear the template info after reaching the last about sibling.
			// The case for clearing (or walking past the last about sibling)
			// in the subtree seems dubiously useful and error prone,
			// explicitly leaving it out.
	...
	isFirstEncapsulationWrapperNode: function(node) {
		return DU.isElt(node) && (/(?:^|\s)mw:(?:Transclusion(?=$|\s)|Param(?=$|\s)|Extension\/[^\s]+)/).test(node.getAttribute('typeof'));
	},
	...
	getWTSource: function( env, node ) {
		...
		return dsr && Util.isValidDSR(dsr) ? env.page.src.substring(dsr[0], dsr[1]) : null;
	},
	...
	//
	// The wikitext comment production is very simple: <!-- starts a comment,
	...
	if (needsWt) {
		steps.push(function() {
			...
			return TemplateRequest.setPageSrcInfo(
				...
				// An alternative would be to audit all the uses to handle
		...
		if (useCache) {
			...
			steps.push(function() {
				return env.pipelineFactory.parse(
					...
					env.page.dom = DU.parseHTML(DU.serializeNode(doc)).body;
				}, function(err) {
