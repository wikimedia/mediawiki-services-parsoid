...
LogData.prototype.stack = function(){
	if (this._cache.stack === undefined) {
		// FIXME: We need to strip stack entries from calls inside the logger
		// from the new Error().stack.
		this._cache.stack = this.flatLogObject().stack || this._getStack();
	}
...
LogData.prototype._announceLocation = function () {
	...
	var location = this.logType + ' in ' + this._env.conf.wiki.iwp + ':' + this._env.page.name;
	if (this._env.page.meta &&
		this._env.page.meta.revision &&
		this._env.page.meta.revision.revid) {
		location += ' with oldid: ' + this._env.page.meta.revision.revid;
	}
	return location;
};
