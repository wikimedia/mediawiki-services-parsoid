 ...
 * @param {string} apiConf.proxyURI The URL of a proxy to use for API requests, or
 ...
 * @param {object} apiConf.proxyHeaders Headers to add when proxying.
 */
ParsoidConfig.prototype.setInterwiki =  // Alias for backwards compat.
ParsoidConfig.prototype.setMwApi = function(prefix, apiConf) {
	var apiURI, proxyURI, proxyHeaders;
	if (typeof apiConf === 'string') {
		apiURI = apiConf;
	} else {
		apiURI = apiConf.uri;
		proxyURI = apiConf.proxyURI;
		proxyHeaders = apiConf.proxyHeaders;
	}

	this.mwApiMap.set(prefix, apiURI);
	this.reverseMwApiMap.set(url.parse(apiURI).host, prefix);
...
ParsoidConfig.prototype.removeMwApi = function(prefix) {
	...
	var u = url.parse(this.mwApiMap.get(prefix));
