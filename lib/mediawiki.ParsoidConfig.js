...
ParsoidConfig.prototype.registerLoggingBackends = function(defaultLogLevels, logger) {
	...
	if (this.traceFlags) {
		logger.registerTracer(buildTraceOrDebugFlag(this.traceFlags, "trace"));
	}
	if (this.debugFlags) {
		logger.registerTracer(buildTraceOrDebugFlag(this.debugFlags, "debug"));
	}
	if (this.linting){
		var linter = new Linter(this);
