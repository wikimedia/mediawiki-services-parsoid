	...
	for (var i = 0, n = tokens.length; i < n; i++) {
		...
		if (t.constructor === SelfclosingTagTk) {
			var type = t.getAttribute("typeof"),
				typeMatch = type ? type.match(includeRE) : null;
			if (typeMatch) {
...
function splitTokens(env, token, nlTkPos, tokens, wrapTemplates) {
	...
	for (var i = 0, l = tokens.length; i < l; i++) {
		...
		if (i === nlTkPos) {
			...
			if (wrapTemplates && t.constructor === SelfclosingTagTk) {
				var type = t.getAttribute("typeof"),
					typeMatch = type && type.match(/(mw:(Transclusion|Param|Extension|Includes\/)(.*)?$)/);
	...
	if (wrapTemplates && startMeta) {
		...
		// Updating tsr[0] is sufficient, but debug output will be
		// easier to make sense of if both are updated.
		dp.tsr = token.dataAttribs.tsr;
		metaTokens = [startMeta];

		return { metaTokens: metaTokens, preNLBuf: buf, postNLBuf: postNLBuf };
	} else {
		return { preNLBuf: tokens };
	}
}
...
AttributeExpander.prototype._returnAttributes = function( token, cb, expandedAttrs ) {
	...
	for (i = 0, l = oldAttrs.length; i < l; i++) {
		...
		// (where newlines show up in attributes of html tags and wikitext tables).
		//
		...
		if (expandedK) {
			...
			if (Array.isArray(oldA.k)) {
				if (!(expandedK.constructor === String && /(^|\s)mw:maybeContent(\s|$)/.test(expandedK))) {
					nlTkPos = nlTkIndex(nlTkOkay, expandedK, this.options.wrapTemplates);
					if (nlTkPos !== -1) {
						// Scenario 1 from the documentation comment above.
						updatedK = splitTokens(env, token, nlTkPos, expandedK, this.options.wrapTemplates);
						...
						updatedK = stripMetaTags(expandedK, this.options.wrapTemplates);
			...
			if ( expandedK.constructor === String && Array.isArray(oldA.v) ) {
				if (!expandedK.match(/^mw:/)) {
					nlTkPos = nlTkIndex(nlTkOkay, attrValTokens, this.options.wrapTemplates);
					if (nlTkPos !== -1) {
						// Scenario 1 from the documentation comment above.
						updatedV = splitTokens(env, token, nlTkPos, attrValTokens, this.options.wrapTemplates);
						attrValTokens = updatedV.preNLBuf;
						postNLToks = updatedV.postNLBuf;
						metaTokens = updatedV.metaTokens;
					} else {
						// Scenario 2 from the documentation comment above.
						updatedV = stripMetaTags(attrValTokens, this.options.wrapTemplates);
	...
	if (modified) {
		...
		if ( !token.getAttribute( 'about' ) && tmpDataMW.size > 0) {
			...
			Util.expandValuesToDOM(manager.env, manager.frame, vals, function(err, eVals) {
				...
				cb( { tokens: metaTokens.concat([token], postNLToks || []) });
			});
