...
TemplateHandler.prototype.getEncapsulationInfo = function (state, chunk) {
	...
	if (!chunk || !chunk.length || chunk[0].constructor !== String) {
		// We only need to return the meta tag with the info
		chunk = [new SelfclosingTagTk( 'meta', attrs, dataParsoid )];
	} else {
		// Try to encapsulate all the simple strings in a <span> and add the
		// info to it
		var firstToken = chunk[0],
			stringTokens = [chunk.shift()];
