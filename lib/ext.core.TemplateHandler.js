...
TemplateHandler.prototype.onTemplate = function(token, frame, cb) {
	...
	if (env.conf.parsoid.usePHPPreProcessor && env.conf.wiki.apiURI !== null) {
		if (this.options.wrapTemplates) {
			...
			if (env.transclusionCache[text]) {
				...
				var toks = DU.encapsulateExpansionHTML(env, token, expansion, opts);
	...
	Util.processContentInPipeline(
		...
		param.wt,
		{
			...
			documentCB: function(html) {
				// Remove DSR from direct children
				// XXX: this is likely incorrect; we should remove DSR
				// from *all* children (traversing down the tree) or
				// from none at all; see comments in PS6 of
				// https://gerrit.wikimedia.org/r/#/c/228009/6/lib/ext.core.LinkHandler.js
				for (var node = html.body.firstChild; node; node = node.nextSibling) {
					if (!DU.isElt(node)) { continue; }
					var dp = DU.getDataParsoid(node);
					if (dp.dsr) {
						dp.dsr = undefined;
						// If data-parsoid only had dsr, remove it completely
						if (Object.keys(dp).every(function(k) {
							return dp[k] === undefined;
						})) {
...
TemplateHandler.prototype._onDocument = function(state, cb, doc) {
	...
	var addWrapperAttrs = function(firstNode) {
		...
		// Update data-mw
		DU.storeDataMw(firstNode, Object.assign(state.wrapperDataMw || {}, argDict));
