...
TemplateHandler.prototype.onTemplateArg = function (token, frame, cb) {
	...
	if (this.options.wrapTemplates) {
		...
		newCB = function(res) {
			...
			toks = tplHandler.getEncapsulationInfo(state).concat(toks);
			toks.rank = tplHandler.rank;
			toks[0].dataAttribs.tmp.tplarginfo = JSON.stringify(tplHandler.getArgInfo(state));
