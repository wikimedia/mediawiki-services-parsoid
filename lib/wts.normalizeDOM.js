...
function normalizeNode(env, node) {
	...
	var emptyTag = (isHeading || Consts.WTQuoteTags.has(node.nodeName)) &&
	...
	var anchorEndsInSpace = (node.nodeName === 'A') && DU.isText(last) &&
			last.nodeValue.match(/\s+$/);
	if (anchorEndsInSpace) {
		last.nodeValue = last.nodeValue.substring(0, anchorEndsInSpace.index);
		if (!DU.isText(next)) {
			var txt = doc.createTextNode('');
			node.parentNode.insertBefore(txt, next);
			next = txt;
		}
		next.nodeValue = anchorEndsInSpace[0] + next.nodeValue;
	}
