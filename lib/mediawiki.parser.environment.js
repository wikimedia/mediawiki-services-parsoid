...
var MWParserEnvironment = function ( parsoidConfig, wikiConfig ) {
	...
	this.traceTest = (function() {
		...
		if (self.conf.parsoid.traceFlags !== null) {
			escapedTraceFlags = self.conf.parsoid.traceFlags.map(Util.escapeRegExp);
			traceTypes = "|trace\/(" + escapedTraceFlags.join("|") + ")";
		}

		if (self.conf.parsoid.debug) {
			debugOn = "|debug";
		}

		return new RegExp("error|warning" + traceTypes + debugOn);
	})();
};
...
MWParserEnvironment.prototype.log = function (logType, info) {
	var self = this;

	var announceLocation = function (logType) {
		if (logType === "error" || logType === "warning") {
			var location = logType + ' in ' + self.conf.wiki.iwp + ':' + self.page.name;
			if (self.page.revision && self.page.revision.revid) {
				location += ' with oldid: ' + self.page.revision.revid;
			}
			console.warn(location);
		}
	};

	var emitMessage = function (info) {
		var messageString;
		if (info.constructor === Object) {
			messageString = info.msg;
		} else if (info.constructor === Array) {
			messageString = info.join("\n");
		} else if (info.constructor === Function) {
			messageString = info();
		} else {
			messageString = info;
		}
	...
	if (this.traceTest.test(logType)) {
		announceLocation(logType);
		emitMessage(info);
	}
};
