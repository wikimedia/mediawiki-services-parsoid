...
MWParserEnvironment.prototype.log = function (logType) {
	...
	var announceLocation = function (logType) {
		var location = logType + ' in ' + this.conf.wiki.iwp + ':' + this.page.name;
		if (this.page.revision && this.page.revision.revid) {
			location += ' with oldid: ' + this.page.revision.revid;
		}
		log(location);
	}.bind(this);

	var emitMessage = function (args) {
		var f = flatten(args, 'top level');
		var messageString = f.msg;
		if (includeStackTrace) {
			var stack = f.stack || (new Error().stack);
			messageString += "\n" + stack;
		}
		log(messageString);
	};
	...
	if (this.traceTest.test(logType)) {
		...
		emitMessage(Array.prototype.slice.call(arguments, 1));
	}
};
