...
MWParserEnvironment.prototype.getAPIProxy = function( prefix ) {
	...
	// defaultAPIProxyURI)
	Object.assign(apiProxy, this.conf.parsoid.apiProxies.get(prefix));
	...
	if (!prefix) {
		...
		if (env.confCache[prefix]) {
			...
			if (localConfig && localConfig.query) {
				getConfigPromise = Promise.resolve(localConfig.query);
			} else {
				getConfigPromise = Promise.reject(
					new Error('Could not read valid config from file: ' +
						 localConfigFile));
			}
		}
	}
