...
function hoistTransclusionInfo(env, child, tdNode) {
	...
	var dataMW = DU.getDataMw(child),
		parts = dataMW.parts,
		dp = DU.getDataParsoid( tdNode ),
		childDP = DU.getDataParsoid( child );
...
TableFixups.prototype.collectAttributishContent = function(env, node, templateWrapper) {
	...
	while (child) {
		if (DU.isComment(child)) { /* jshint noempty:false */
			...
			// to do here!
			return buildRes();
		} else if (transclusionNode && /^mw:(Nowiki|Entity)$/.test(child.getAttribute("typeof"))) {
	...
	while (child) {
		...
		if (newCell) {
			...
			var cellName = node.nodeName.toLowerCase(),
				hasSpanWrapper = !DU.isText(child),
				match;
