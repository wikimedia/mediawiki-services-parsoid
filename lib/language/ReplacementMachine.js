...
class ReplacementMachine {
	...
	constructor(baseLanguage, ...codes) {
		...
		this.machine = new Map(codes.map(c => [c, {
			convert: ReplacementMachine.loadJSON(`trans-${c}`),
			bracket: new Map(codes.map(cc => [
				cc,
				ReplacementMachine.loadJSON(
	 ...
	 * @private
	 */
	static loadJSON(filename, bracket) {
		const json = require(`./fst/${filename}.json`);
		return FST.compile(json, bracket);
	}
