...
function docFragToString(docFrag, force) {
	if (!force) {
		for (let child = docFrag.firstChild; child; child = child.nextSibling) {
			if (!DU.isText(child)) { return null; /* unsafe */ }
		}
	}
...
class ConversionTraverser extends DOMTraverser {
	 ...
	 */
	constructor(env, toLang, guesser, machine) {
		super(env);
	...
	aHandler(node, env, atTopLevel, tplInfo) {
		...
		if (rel === 'mw:WikiLink') {
			...
			if (DU.usesURLLinkSyntax(node)) {
				// Don't convert children of syntactic "free links"
				return node.nextSibling;
			}
			// It appears that all other external link text is also
			// marked to protect it from conversion (see
			// Parser.php::replaceExternalLinks).  That seems suboptimal?
			return node.nextSibling;
		}
		return true;
	}
	...
	lcHandler(node, env, atTopLevel, tplInfo) {
		if (!DU.hasTypeOf(node, 'mw:LanguageVariant')) {
			return true; /* not language converter markup */
		}
		const dmv = DU.getJSONAttribute(node, 'data-mw-variant', {});
		if (dmv.disabled) {
...
class LanguageConverter {
	...
	static baseToVariant(env, rootNode) {
		...
		const toLangCode = env.variantLanguage;
		let guesser;
		...
		if (false) { // eslint-disable-line no-constant-condition
			guesser = new ConstantLanguageGuesser(pageLangCode);
		} else {
			guesser = new MachineLanguageGuesser(
				langconv.getMachine(), rootNode, toLangCode
			);
		}
		new ConversionTraverser(env, toLangCode, guesser, langconv.getMachine())
