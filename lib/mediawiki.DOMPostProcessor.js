...
var handleLIHack = require('./dom.t.handleLIHack.js').handleLIHack;
...
function DOMPostProcessor(env, options) {
	...
	domVisitor.addHandler('a', handleLinkNeighbours.bind(null, env));
	domVisitor.addHandler(null, unpackDOMFragments.bind(null, env));
	...
	domVisitor.addHandler('li', handleLIHack.bind(null, env));
	// 3. Fix up issues from templated table cells and table cell attributes
	domVisitor.addHandler('td', tableFixer.stripDoubleTDs.bind(tableFixer, env));
	domVisitor.addHandler('td', tableFixer.handleTableCellTemplates.bind(tableFixer, env));
	domVisitor.addHandler('th', tableFixer.handleTableCellTemplates.bind(tableFixer, env));
	this.processors.push(domVisitor.traverse.bind(domVisitor));
