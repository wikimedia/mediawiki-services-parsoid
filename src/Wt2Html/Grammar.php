...
private function parsetable_attribute_name($silence, $boolParams, &$param_preproc, &$param_th) {
  ...
  for (;;) {
    ...
    // start seq_3
    $p12 = $this->currPos;
    $r13 = $this->parsexmlish_tag($silence, $boolParams, $param_preproc, $param_th);
    // x <- $r13
    if ($r13===self::$FAILED) {
      $r11 = self::$FAILED;
      goto seq_3;
...
private function parseinclude_limits($silence, $boolParams, &$param_preproc, &$param_th) {
  ...
  $r11 = $this->parsexmlish_tag_opened($silence, ($boolParams & ~0x800) | 0x1000, $param_preproc, $param_th);
  // il <- $r11
  if ($r11===self::$FAILED) {
    $this->currPos = $p3;
    $r1 = self::$FAILED;
