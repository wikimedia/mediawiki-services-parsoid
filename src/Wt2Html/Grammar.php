...
class Grammar extends \WikiPEG\PEGParserBase {
  ...
  private function parseinclude_limits($silence, $boolParams, $param_templatedepth, &$param_preproc, &$param_th) {
    ...
    $this->savedPos = $this->currPos;
    $r11 = $this->a78($r10, /*sol_il*/($boolParams & 0x2000) !== 0);
    if ($r11) {
      $r11 = false;
    } else {
      $r11 = self::$FAILED;
    ...
    if ($r1!==self::$FAILED) {
      $this->savedPos = $p2;
      $r1 = $this->a79($r10, /*sol_il*/($boolParams & 0x2000) !== 0);
    }
    // free $p3
    $cached = ['nextPos' => $this->currPos, 'result' => $r1];
      if ($saved_preproc !== $param_preproc) $cached["\$preproc"] = $param_preproc;
  ...
  private function parsexmlish_tag_opened($silence, $boolParams, $param_templatedepth, &$param_preproc, &$param_th) {
    ...
    $this->savedPos = $this->currPos;
    $r6 = $this->a88($r4, $r5, /*extTag*/($boolParams & 0x800) !== 0, /*isBlock*/($boolParams & 0x400) !== 0);
    if ($r6) {
      $r6 = false;
    } else {
      $r6 = self::$FAILED;
    ...
    if ($r1!==self::$FAILED) {
      $this->savedPos = $p2;
      $r1 = $this->a89($r4, $r5, /*extTag*/($boolParams & 0x800) !== 0, /*isBlock*/($boolParams & 0x400) !== 0, $r7, $r8);
    }
    // free $p3
    $cached = ['nextPos' => $this->currPos, 'result' => $r1];
      if ($saved_preproc !== $param_preproc) $cached["\$preproc"] = $param_preproc;
