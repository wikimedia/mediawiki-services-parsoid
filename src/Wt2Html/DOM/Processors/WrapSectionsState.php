		...
		if ( !$this->env->getPageConfig()->getSuppressTOC() ) {
			if ( $enoughToc ) {
				...
				if ( !$tocBS ) {
					...
					$headingTocIP = $this->findTOCInsertionPoint( DOMCompat::getBody( $this->doc ) );
					if ( $headingTocIP === null ) {
						// should not happen, but nothing to do here!
						return;
					}

					// NOTE: Given how <section>s are computed in this file, headings
					// will never have previous siblings. So, we look at $sectionTocIP's
					// previous siblings always.
					$sectionTocIP = $headingTocIP->parentNode;

					$insertionContainer = $sectionTocIP->previousSibling;
					...
					$syntheticOffset = DOMDataUtils::getDataParsoid( $headingTocIP )->dsr->start ?? null;
					if ( $syntheticOffset !== null ) {
						...
						$dp->dsr = new DomSourceRange(
							...
							source: DOMDataUtils::getDataParsoid( $headingTocIP )->dsr->source
						);
					}
				}
			}
