...
class TableFixups {
	 ...
	 */
	public function stripDoubleTDs( $node, $env ) {
...
|| // Hacky work-around for nested templates
	...
	public function isSimpleTemplatedSpan( $node ) {
		return $node->nodeName === 'SPAN'
&& DOMDataUtils::hasTypeOf( $node, 'mw:Transclusion' )
&& DOMUtils::allChildrenAreTextOrComments( $node );
	}

	public function hoistTransclusionInfo( $env, $child, $tdNode ) {
	 ...
	 * character.
	 */
	public function collectAttributishContent( $env, $node, $templateWrapper ) {
		$buf = [];
		$nowikis = [];
...
&& DOMUtils::allChildrenAreTextOrComments( $child )
	 ...
	 */
	public function reparseTemplatedAttributes( $env, $node, $templateWrapper ) {
	...
	public function needsReparsing( $node ) {
	...
	public function handleTableCellTemplates( $node, $env ) {
