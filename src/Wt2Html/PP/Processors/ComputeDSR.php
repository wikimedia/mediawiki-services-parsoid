...
class ComputeDSR {
	public function tsrSpansTagDOM( $n, $parsoidData ) {
		// - tags known to have tag-specific tsr
		// - html tags with 'stx' set
		$name = strtolower( $n->nodeName );
		return !WtTagsWithLimitedTSR[ $name ]
|| WTUtils::hasLiteralHTMLMarker( $parsoidData );
	}

	public function acceptableInconsistency( $opts, $node, $cs, $s ) {
		/**
		...
		if ( $node->nodeName === 'A' && ( WTUtils::usesURLLinkSyntax( $node ) || WTUtils::usesMagicLinkSyntax( $node ) ) ) {
			return true;
		} elseif ( $opts->attrExpansion && DOMUtils::isBody( $node ) ) {
			return true;
		} else {
			return false;
		}
	...
	public function computeListEltWidth( $li ) {
	...
	public function computeATagWidth( $node, $dp ) {
		/* -------------------------------------------------------------
		...
		if ( !$dp ) {
			...
			if ( WTUtils::usesWikiLinkSyntax( $node, $dp ) && !WTUtils::hasExpandedAttrsType( $node ) ) {
				if ( $dp->stx === 'piped' ) {
					$href = ( $dp->sa ) ? $dp->sa->href : null;
					if ( $href ) {
						return [ count( $href ) + 3, 2 ];
					} else {
						return null;
					}
				} else {
			...
			} elseif ( $dp->tsr && WTUtils::usesExtLinkSyntax( $node, $dp ) ) {
				return [ $dp->targetOff - $dp->tsr[ 0 ], 1 ];
			} elseif ( WTUtils::usesURLLinkSyntax( $node, $dp ) || WTUtils::usesMagicLinkSyntax( $node, $dp ) ) {
				return [ 0, 0 ];
			} else {
				return null;
			}
	...
	public function computeTagWidths( $widths, $node, $dp ) {
		if ( $dp->tagWidths ) {
			return $dp->tagWidths;
		}

		$stWidth = $widths[ 0 ];
		$etWidth = ( gettype( $widths[ 1 ] ) === 'number' ) ? $widths[ 1 ] : null;
		...
		if ( WTUtils::hasLiteralHTMLMarker( $dp ) ) {
			...
			if ( $nodeName === 'TR' && !$dp->startTagSrc ) {
				...
				$wtTagWidth = Consts\WtTagWidths::get( $nodeName );
				if ( $stWidth === null ) {
	 ...
	 * @param {Object} opts
	 */
	public function computeNodeDSR( $env, $node, $s, $e, $dsrCorrection, $opts ) {
		function trace() use ( &$env ) {
			$args = $arguments;
			$env->log( 'trace/dsr', function () use ( &$args ) {
					$buf = '';
					foreach ( $args as $i => $___ ) {
						$buf += ( gettype( $args[ $i ] ) === 'string' ) ? $args[ $i ] : json_encode( $args[ $i ] );
					}
					return $buf;
			}
			);
...
&& Consts\WTQuoteTags::has( $child->nodeName )
			...
			$env->log( 'trace/dsr', function () use ( &$child, &$DOMUtils ) {
					$i = array_search( $child, $child->parentNode->childNodes ); // slow, for debugging only
...
. ( ( DOMUtils::isElt( $child ) ) ?
						...
						json_encode( $child->data ) )
. ' with ' . json_encode( [ $cs, $ce ] );
			}
			);
			...
			if ( $cType === $node::TEXT_NODE ) {
				if ( $ce !== null ) {
					...
					$cs = $ce - count( $child->data ) - WTUtils::indentPreDSRCorrection( $child );
				}
			} elseif ( $cType === $node::COMMENT_NODE ) {
				if ( $ce !== null ) {
					// decode html entities & reencode as wikitext to find length
					$cs = $ce - WTUtils::decodedCommentLength( $child );
				}
			} elseif ( $cType === $node::ELEMENT_NODE ) {
				...
				if ( !$rtTestMode && $ce !== null && $dp->autoInsertedEnd && DOMUtils::isQuoteElt( $child ) ) {
					$correction = ( 3 + count( $child->nodeName ) );
				...
				if ( $child->nodeName === 'META' ) {
					...
					} elseif ( preg_match( '/^mw:Placeholder(\/\w*)?$/', $cTypeOf ) && $ce !== null && $dp->src ) {
						$cs = $ce - count( $dp->src );
					} else {
...
. '; subtree-[cs,ce]=' . json_encode( [ $ccs, $cce ] );
				...
				if ( $cs !== null || $ce !== null ) {
					...
					$env->log( 'trace/dsr', function () use ( &$cTypeOf, &$child, &$dp, &$env, &$cs, &$ce ) {
							$str = '     UPDATING ' . $child->nodeName
. ' with ' . json_encode( [ $cs, $ce ] ) . '; typeof: ' . $cTypeOf;
...
&& !WTUtils::isTplStartMarkerMeta( $child )
					...
					while ( $newCE !== null && $sibling && !WTUtils::isTplStartMarkerMeta( $sibling ) ) {
						...
						if ( $nType === $node::TEXT_NODE ) {
							$newCE = $newCE + count( $sibling->data ) + WTUtils::indentPreDSRCorrection( $sibling );
						} elseif ( $nType === $node::COMMENT_NODE ) {
							$newCE += WTUtils::decodedCommentLength( $sibling );
						} elseif ( $nType === $node::ELEMENT_NODE ) {
...
|| ( $siblingDP->dsr[ 0 ] !== null && $siblingDP->tsr && $siblingDP->dsr[ 0 ] < $newCE )
							...
							$env->log( 'trace/dsr', function () use ( &$newCE, &$sibling, &$siblingDP, &$env ) {
...
&& strtoupper( $endTagInfo->nodeName ) === $child->previousSibling->nodeName
				) {
			...
			if ( $isMarkerTag ) {
				...
				if ( DOMUtils::isText( $prevChild ) && DOMUtils::isText( $nextChild ) ) {
					...
					if ( $ce !== null ) {
						...
						$cs = $ce - count( $prevText ) - WTUtils::indentPreDSRCorrection( $prevChild );
						$ce = $cs;
					}
		...
		// Detect errors
		if ( $s !== null && $s !== null && $cs !== $s && !$this->acceptableInconsistency( $opts, $node, $cs, $s ) ) {
	...
	public function run( $rootNode, $env, $options ) {
		...
		$startOffset = ( $options->sourceOffsets ) ? $options->sourceOffsets[ 0 ] : 0;
		$endOffset = ( $options->sourceOffsets ) ? $options->sourceOffsets[ 1 ] : count( $env->page->src );
