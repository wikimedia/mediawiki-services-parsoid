...
38 => ["type" => "other", "description" => "parsoid_fragment_marker"],
...
55 => ["type" => "other", "description" => "parsoid_fragment_marker_literal"],
...
88 => ["type" => "literal", "value" => "{{#parsoid\x00fragment:", "description" => "\"{{#parsoid\\u0000fragment:\""],
89 => ["type" => "class", "value" => "[0-9]", "description" => "[0-9]"],
...
96 => ["type" => "other", "description" => "double_underscore"],
...
return $this->traceCall(function($silence, &$param_th, &$param_preproc) {
  $key = json_encode([334, $param_th, $param_preproc]);
  $bucket = $this->currPos;
...
return $this->traceCall(function($silence, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([400, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([624, $boolParams & 0x3bbf, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_th, &$param_preproc, &$param_headingIndex) {
  $key = json_encode([408, $param_th, $param_preproc, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_headingIndex) {
  $key = json_encode([480, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_th, &$param_headingIndex, &$param_preproc) {
  $key = json_encode([344, $param_th, $param_headingIndex, $param_preproc]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_th, &$param_preproc, &$param_headingIndex) {
  $key = json_encode([406, $boolParams & 0x3bff, $param_tagType, $param_th, $param_preproc, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([644, $boolParams & 0x39ff, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_preproc_sol, &$param_headingIndex) {
  $key = json_encode([482, $param_preproc_sol, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, $param_tagType, &$param_th, &$param_headingIndex) {
  $key = json_encode([421, $boolParams & 0x3bff, $param_tagType, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_preproc) {
  $key = json_encode([412, $param_preproc]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_th, &$param_headingIndex) {
  $key = json_encode([420, $boolParams & 0x3bff, $param_tagType, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function(&$param_preproc_sol) {
  $key = json_encode([485, $param_preproc_sol]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([664, $boolParams & 0x3baf, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([423, $boolParams & 0x3bff, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([372, $boolParams & 0x3baf, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_th, &$param_preproc, &$param_headingIndex) {
  $key = json_encode([434, $boolParams & 0x3bff, $param_tagType, $param_th, $param_preproc, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_th, &$param_headingIndex, &$param_preproc) {
  $key = json_encode([466, $boolParams & 0x3bff, $param_tagType, $param_th, $param_headingIndex, $param_preproc]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, &$param_preproc_sol, &$param_headingIndex, $param_preproc_stop) {
  $key = json_encode([488, $boolParams & 0x400, $param_preproc_sol, $param_headingIndex, $param_preproc_stop]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, &$param_preproc_sol, &$param_headingIndex) {
  $key = json_encode([492, $boolParams & 0x400, $param_preproc_sol, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([638, $boolParams & 0x3baf, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, $param_tagType, &$param_th, &$param_headingIndex, &$param_preproc) {
  $key = json_encode([467, $boolParams & 0x3bff, $param_tagType, $param_th, $param_headingIndex, $param_preproc]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([370, $boolParams & 0x3baf, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([402, $boolParams & 0x3bff, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_th, &$param_preproc, &$param_headingIndex) {
  $key = json_encode([438, $boolParams & 0x3bfb, $param_tagType, $param_th, $param_preproc, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, &$param_preproc, $param_tagType, &$param_th, &$param_headingIndex) {
  $key = json_encode([468, $boolParams & 0x3bff, $param_preproc, $param_tagType, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_preproc_sol, &$param_headingIndex, $param_preproc_stop) {
  $key = json_encode([506, $param_preproc_sol, $param_headingIndex, $param_preproc_stop]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($param_tagType) {
  $key = json_encode([637, $param_tagType]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $param_tagType) {
  $key = json_encode([530, $param_tagType]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([430, $boolParams & 0x3b8b, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, &$param_preproc, $param_tagType, &$param_th, &$param_headingIndex) {
  $key = json_encode([469, $boolParams & 0x3bff, $param_preproc, $param_tagType, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, &$param_preproc) {
  $key = json_encode([436, $param_preproc]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_headingIndex) {
  $key = json_encode([608, $boolParams & 0x3bff, $param_tagType, $param_preproc, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([473, $boolParams & 0x3bff, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([369, $boolParams & 0x3bff, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([474, $boolParams & 0x39f7, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([460, $boolParams & 0x3b7f, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([462, $boolParams & 0x3a7f, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([590, $boolParams & 0x2bfe, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
...
return $this->traceCall(function($silence, $boolParams, $param_tagType, &$param_preproc, &$param_th, &$param_headingIndex) {
  $key = json_encode([592, $boolParams & 0x3baf, $param_tagType, $param_preproc, $param_th, $param_headingIndex]);
  $bucket = $this->currPos;
  $cached = $this->cache[$bucket][$key] ?? null;
  if ($cached) {
    $this->currPos = $cached->nextPos;
