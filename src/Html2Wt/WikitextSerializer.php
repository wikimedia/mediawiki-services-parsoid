...
class WikitextSerializer {
	...
	private function serializePart(
		...
		if ( preg_match( '/.*data-parsoid\/0.0.1"$/', $this->env->getPageConfig()->dpContentType ) ) {
	...
	private function serializeDOMNode( DOMElement $node, DOMHandler $domHandler ) {
		...
		if ( $state->selserMode
			...
			&& ( $dp->dsr[1] > $dp->dsr[0]
				...
				|| ( $dp->dsr[1] === $dp->dsr[0]
					&& ( preg_match( '/^(p|br)$/', $node->nodeName )
						|| DOMDataUtils::getDataMw( $node )->autoGenerated ) )
			...
			if ( !DiffUtils::hasDiffMarkers( $node, $this->env ) ) {
				...
				$this->trace( 'ORIG-src with DSR', function () use ( $dp, $out ) {
					return '[' . $dp->dsr[0] . ',' . $dp->dsr[1] . '] = ' . json_encode( $out );
				} );
	...
	public function serializeDOM( DOMElement $body, bool $selserMode = false ) {
		...
		if ( $this->env->getSiteConfig()->hasDumpFlag( 'dom:post-normal' ) ) {
