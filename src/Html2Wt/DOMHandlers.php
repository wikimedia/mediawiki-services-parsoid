...
var tagHandlers = JSUtils.mapObject({
	...
	th: {
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			var nextTh = DOMUtils.nextNonDeletedSibling(node);
			var nextUsesRowSyntax = nextTh && DOMDataUtils.getDataParsoid(nextTh).stx === 'row';
	...
	td: {
		handle: Promise.async(function *(node, state, wrapperUnmodified) {
			...
			var nextTd = DOMUtils.nextNonDeletedSibling(node);
			var nextUsesRowSyntax = nextTd && DOMDataUtils.getDataParsoid(nextTd).stx === 'row';
