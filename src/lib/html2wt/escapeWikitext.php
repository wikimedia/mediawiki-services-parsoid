...
class WikitextEscapeHandlers {
	...
	escapeLinkContent(str, solState, node) {
		var state = this.serializer.state;
		...
		state.wteHandlerStack.push(this.serializer.wteHandlers.wikilinkHandler);
		state.inLink = true;
		var res = this.serializer.wteHandlers.escapeWikiText(str, { node: node });
		state.inLink = false;
		state.wteHandlerStack.pop();
