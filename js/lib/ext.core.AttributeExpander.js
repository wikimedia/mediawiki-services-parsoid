...
AttributeExpander.prototype._returnAttributes = function ( token, cb, newAttrs )
{
	...
	if (modified) {
		...
		// template, this check can be relaxed to allow that.
		...
		if ( !token.getAttribute( 'about' ) && tmpDataMW.size > 0) {
			...
			async.map(
				...
				function(k_or_v, cb1) {
					// Build to DOM
					if (k_or_v.html && k_or_v.html.constructor === Array) {
						Util.processAttributeToDOM(
							manager,
							k_or_v.html,
							function(dom) {
								k_or_v.html = dom.body.innerHTML;
								cb1(null, k_or_v);
							}
						);
					} else {
